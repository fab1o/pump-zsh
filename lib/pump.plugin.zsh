...
function load_settings_() {
  ...
  PUMP_PUSH_SIGNED=$(sed -n "s/^PUMP_PUSH_SIGNED${i}=\\([^ ]*\\)/\\1/p" "$PUMP_SETTINGS_FILE" 2>/dev/null)
  if [[ "$PUMP_PUSH_SIGNED" -ne 0 && "$PUMP_PUSH_SIGNED" -ne 1 ]]; then
    PUMP_PUSH_SIGNED=""
  fi
...
function macdown() {
  ...
  if (( macdown_is_h )); then
    print "  ${yellow_cor}macdown${reset_cor} : download full installers of macos"
    return 0;
  fi
...
function fix() {
  ...
  if (( fix_is_h )); then
    print "  ${yellow_cor}fix ${low_yellow_cor}[<folder>]${reset_cor} : run fix script or format + lint"
    return 0;
  fi
...
function refix() {
  set +x
  eval "$(parse_flags_ "$0" "npr" "q" "$@")"
  (( refix_is_debug )) && set -x

  if (( refix_is_h )); then
    print "  ${yellow_cor}refix ${low_yellow_cor}[<folder>]${reset_cor} : reset last commit then run fix lint and format"
    print "  --" >&1
    print "  ${yellow_cor}refix -n${reset_cor} : create a new commit instead of amending"
    print "  ${yellow_cor}refix -p${reset_cor} : then repush"
    print "  ${yellow_cor}refix -r${reset_cor} : then recommit"
    print "  ${yellow_cor}refix -q${reset_cor} : quiet, no output"
    return 0;
  fi
...
function proj_prs_() {
  ...
  if (( proj_prs_is_a && proj_prs_is_u )); then
    ...
    proj_prs_au_ "$proj_repo" "$pr_approval_min" "$title_criteria"
    print ""
  ...
  if (( proj_prs_is_a )); then
    ...
    local prs=("${(@f)$(select_prs_ "$title_criteria" "$proj_repo" "pull requests to approve")}")
    if (( $? == 130 )); then return 130; fi
...
function proj_prs_au_() {
  ...
  local pr_list=()
  if command -v gum &>/dev/null; then
    pr_list=("${(@f)$(gum spin --title="fetching pull requests..." -- gh pr list --repo "$proj_repo" --json number,title --jq '.[] | "\(.number)\t\(.title)"' | grep -i "$title_criteria"  2>/dev/null)}")
  else
    pr_list=("${(@f)$(gh pr list --repo "$proj_repo" --json number,title --jq '.[] | "\(.number)\t\(.title)"' | grep -i "$title_criteria"  2>/dev/null)}")
  fi
  ...
  for pr in "${pr_list[@]}"; do
    ...
    if (( approval_count < pr_approval_min )); then
      if (( user_has_approved )); then
        print " pr $pr_number_link has $pr_approval_min ✓ and you also approved it"
      else
      ...
      if (( user_has_approved )); then
        ...
        print " pr $pr_number_link has $approval_count ✓, but you did not approve"
      fi
    fi
  done
}
...
function revert() {
  ...
  if (( revert_is_h )); then
    ...
    print "  --" >&1
    print "  ${yellow_cor}revert -m <parent_number>${reset_cor} : to revert a merge commit"
    print "  ${yellow_cor}revert -a${reset_cor} : --abort"
    print "  ${yellow_cor}revert -c${reset_cor} : --continue"
    print "  ${yellow_cor}revert -s${reset_cor} : --signoff"
    return 0;
  fi
      ...
      if [[ -n "$2" && $2 != -* ]]; then
        if [[ $2 =~ ^[0-9a-f]{7,40}$ ]]; then
          ...
          if [[ -n "$1" && $1 =~ ^[0-9]+$ ]]; then
            ...
            print " run ${yellow_cor}revert -h${reset_cor} to see usage" >&2
            return 1;
          fi
        elif [[ $2 =~ ^[0-9]+$ ]]; then
          num="$2"
...
function recommit() {
  ...
  if (( recommit_is_h )); then
    ...
    print "  ${yellow_cor}recommit -q${reset_cor} : quiet, no output"
    return 0;
  fi
...
function glog() {
  ...
  if (( glog_is_h )); then
    print "  ${yellow_cor}glog ${low_yellow_cor}[<branch>] [<folder>]${reset_cor} : see log commits of a given branch"
    print "  ${yellow_cor}glog -<number>${reset_cor} : limit log commits e.g.: glog -10"
    print "  ${yellow_cor}glog -c ${low_yellow_cor}<branch>${reset_cor} : see log commits after head of a given branch"
    print "  ${yellow_cor}glog -b${reset_cor} : see log commits after head of base branch"
    print "  ${yellow_cor}glog -m${reset_cor} : see log commits after head of main branch"
  ...
  if [[ -n "$3" && $3 != -* ]]; then
    if (( glog_is_f )); then
      ...
      print " fatal: not valid arguments: $@" >&2
      print " run ${yellow_cor}glog -h${reset_cor} to see usage" >&2
      return 1;
    fi
  ...
  if [[ -n "$format" ]]; then
    ...
    git -C "$folder" --no-pager log $branch_arg --oneline --decorate ${flags[@]} $@
  fi
}
...
function push() {
  ...
  if [[ -z "$PUMP_PUSH_SIGNED" ]]; then
    confirm_ "signed all ${yellow_cor}push${reset_cor} and ${yellow_cor}pushf${reset_cor} ?"
    local RET=$?
    if (( RET == 130 || RET == 2 )); then return 130; fi

    if (( RET == 0 )); then
      update_setting_ "PUMP_PUSH_SIGNED" 1
    else
      update_setting_ "PUMP_PUSH_SIGNED" 0
    fi
  fi
  ...
  if (( PUMP_PUSH_SET_UPSTREAM && PUMP_PUSH_SIGNED )); then
    eval "$(parse_flags_ "$0" "tfnv" "q" "$@")"
  elif (( PUMP_PUSH_SET_UPSTREAM )); then
    eval "$(parse_flags_ "$0" "tfnvs" "q" "$@")"
  elif (( PUMP_PUSH_SIGNED )); then
    eval "$(parse_flags_ "$0" "tfnvu" "q" "$@")"
  else
    eval "$(parse_flags_ "$0" "tfnvus" "q" "$@")"
  fi
  ...
  if (( push_is_h )); then
    ...
    (( ! PUMP_PUSH_SIGNED )) && print "  ${yellow_cor}push -s${reset_cor} : --signed=if-asked"
...
function pushf() {
  ...
  if [[ -z "$PUMP_PUSH_SIGNED" ]]; then
    confirm_ "signed all ${yellow_cor}pushf${reset_cor} and ${yellow_cor}push${reset_cor} ?"
    local RET=$?
    if (( RET == 130 || RET == 2 )); then return 130; fi
  ...
  if (( pushf_is_h )); then
    ...
    (( ! PUMP_PUSH_SIGNED )) && print "  ${yellow_cor}pushf -s${reset_cor} : --signed=if-asked"
...
function repush() {
  ...
  if (( repush_is_h )); then
    ...
    print "  ${yellow_cor}repush -q${reset_cor} : quiet, no output"
    return 0;
  fi
...
function co() {
  ...
  if (( co_is_x )); then # search an exact term
...
function proj_handler() {
  ...
  if [[ " ${sub_cmds[*]} " != *" $1 "* ]]; then
    if (( proj_handler_is_h )); then
      ...
      print "  ${yellow_cor}${proj_cmd} prs -a ${low_yellow_cor}[<criteria>]${reset_cor} : approve pull requests by criteria"
      print "  ${yellow_cor}${proj_cmd} prs -au ${low_yellow_cor}[<criteria>]${reset_cor} : auto approve pull requests by criteria"
...
function commit() {
  ...
  if (( commit_is_h )); then
    ...
    print "  ${yellow_cor}commit -s${reset_cor} : --signoff"
    return 0;
  fi
...
function help_general_() {
  ...
  [[ "$(uname)" == "Darwin" ]] && printf "  ${low_yellow_cor}%-$spaces${reset_cor} = %s \n" "macdown" "download macos installers"
