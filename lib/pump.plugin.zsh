...
function proj_prs_() {
  ...
  if [[ -z "$proj_repo" ]]; then
    ...
    print " run ${yellow_cor}$proj_cmd -e${reset_cor} to set the repository" >&2
    return 1;
  fi
  ...
  if (( proj_prs_is_a && proj_prs_is_u )); then
    ...
    proj_prs_au_ "$proj_repo" "$pr_approval_min" "$search_term"
    print ""

    if confirm_ -a "approve prs every $_interval min?"; then
      while true; do
        print "sleeping for $_interval minutes..."
        sleep $(( 60 * _interval ))
...
function proj_prs_r_() {
  ...
  if [[ -n "$_not_merged" ]]; then
    ...
    for branch in "${_not_merged[@]}"; do
      print "    ◦ ${red_cor}$branch${reset_cor}"
    done
  fi
}
...
function renb() {
  ...
  if (( renb_is_h )); then
    print "  ${yellow_cor}renb <new_branch_name> ${low_yellow_cor}[<folder>]${reset_cor} : rename current local branch"
    print "  --" >&1
    print "  ${yellow_cor}renb -r${reset_cor} : rename current upstream branch"
    return 0;
  fi
...
function fetch() {
  ...
  if (( fetch_is_h )); then
    print "  ${yellow_cor}fetch ${low_yellow_cor}[<branch>] [<folder>]${reset_cor} : fetch upstream changes"
    print "  --" >&1
...
function push() {
  ...
  if [[ -z "$PUMP_PUSH_SET_UPSTREAM" ]]; then
    confirm_ "create an upstream counterpart branch on ${yellow_cor}push${reset_cor} and ${yellow_cor}pushf${reset_cor}?"
    local RET=$?
    if (( RET == 130 || RET == 2 )); then return 130; fi
...
function pushf() {
  ...
  if [[ -z "$PUMP_PUSH_SET_UPSTREAM" ]]; then
    confirm_ "create an upstream counterpart branch on ${yellow_cor}pushf${reset_cor} and ${yellow_cor}push${reset_cor}?"
    local RET=$?
    if (( RET == 130 || RET == 2 )); then return 130; fi
...
function pull() {
  ...
  if (( pull_is_h )); then
    print "  ${yellow_cor}pull ${low_yellow_cor}[<branch>] [<folder>]${reset_cor} : pull from upstream"
    print "  --" >&1
...
function reseta() {
  ...
  if (( reseta_is_h )); then
    ...
    print "  --" >&1
    print "  ${yellow_cor}reseta -o${reset_cor} : erase every change and match HEAD to upstream"
...
function reseto() {
  ...
  if (( reseto_is_h )); then
    print "  ${yellow_cor}reseto ${low_yellow_cor}[<folder>]${reset_cor} : erase every change and match HEAD to upstream"
    print "  --" >&1
...
function glr() {
  ...
  if (( glr_is_h )); then
    print "  ${yellow_cor}glr ${low_yellow_cor}[<folder>]${reset_cor} : list upstream branches"
    print "  ${yellow_cor}glr <branch> ${low_yellow_cor}[<folder>]${reset_cor} : list upstream branches matching branch"
    return 0;
  fi
...
function rebase() {
  ...
  if (( rebase_is_h )); then
    print "  ${yellow_cor}rebase ${low_yellow_cor}[<base_branch>] [<folder>]${reset_cor} : apply the commits from your branch on top of the HEAD of base branch"
    print "  --" >&1
    print "  ${yellow_cor}rebase -d${reset_cor} : apply the commits from your branch on top of the HEAD of default branch"
    print "  ${yellow_cor}rebase -l${reset_cor} : rebase on top of local branch instead of base branch"
...
function merge() {
  ...
  if (( merge_is_h )); then
    print "  ${yellow_cor}merge ${low_yellow_cor}[<base_branch>] [<folder>]${reset_cor} : merge from base branch"
    print "  --" >&1
    print "  ${yellow_cor}merge -d${reset_cor} : merge from default branch"
    print "  ${yellow_cor}merge -l${reset_cor} : merge from local branch instead of base branch"
...
function delb() {
  ...
  if (( delb_is_h )); then
    print "  ${yellow_cor}delb ${low_yellow_cor}[<branch>] [<folder>]${reset_cor} : delete local branches"
    print "  --" >&1
    print "  ${yellow_cor}delb -e <branch>${reset_cor} : delete an exact branch, no lookup"
    print "  ${yellow_cor}delb -r${reset_cor} : also delete upstream branches"
...
function proj_handler() {
  ...
  if [[ " ${sub_cmds[*]} " != *" $1 "* ]]; then
    if (( proj_handler_is_h )); then
      ...
      print "  ${yellow_cor}${proj_cmd} gha -a ${low_yellow_cor}[<workflow>]${reset_cor} : check status of workflow every $_interval min"
      print "  --" >&1
      ...
      (( single_mode )) && print "  ${yellow_cor}${proj_cmd} jira -vv${reset_cor} : view ticket status of all your branches"
      (( ! single_mode )) && print "  ${yellow_cor}${proj_cmd} jira -vv${reset_cor} : view ticket status of all your folders"
      print "  --" >&1
      print "  ${yellow_cor}${proj_cmd} prs${reset_cor} : open pull requests in github"
      print "  ${yellow_cor}${proj_cmd} prs -a ${low_yellow_cor}[<search_term>]${reset_cor} : approve pull requests"
      print "  ${yellow_cor}${proj_cmd} prs -au ${low_yellow_cor}[<search_term>]${reset_cor} : approve prs every $_interval min"
      print "  ${yellow_cor}${proj_cmd} prs -r${reset_cor} : rebase/merge all your open pull requests"
      print "  ${yellow_cor}${proj_cmd} prs -s${reset_cor} : set assignee for all pull requests"
      print "  ${yellow_cor}${proj_cmd} prs -sa${reset_cor} : set assignee for all prs every $_interval min"
      print "  --" >&1
      ...
      print "  ${yellow_cor}${proj_cmd} revs -dd${reset_cor} : delete all code reviews"
      print "  --" >&1
