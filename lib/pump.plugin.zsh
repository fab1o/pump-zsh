...
typeset -g bold_gray_cor=$'\e[1;38;5;244m'
...
function parse_args_() {
  ...
  for s in "${specs[@]}"; do
    ...
    if [[ "$type" != *:* ]]; then sub_type="";sub_type_char="";sub_type_okz=""; fi
    ...
    if [[ -n "$type_okz" ]]; then
      if [[ "$type_char" == "f" ]]; then
        ...
        if [[ "$type_char" != "${last_type[1]}" ]]; then
          print " ${red_cor}fatal: parse_args_ array type must be the last argument in the spec${reset_cor}" >&2
          return 1;
        fi

        if [[ -z "$sub_type_char" ]]; then
          print " ${red_cor}fatal: parse_args_ array type must have a subtype specified after a second colon (e.g. a:t or a:b)${reset_cor}" >&2
          return 1;
        fi
  ...
  if [[ "$last_type" == "a" ]] && (( arg_count > max_arg )); then
    ...
    local last_is_default="${is_defaults[-1]}"
  ...
  while (( spec_idx <= max_arg )); do
    ...
    local is_default="${is_defaults[$spec_idx]}"
    ...
    if (( arg_idx > $# )) || [[ "${value}" == -* ]]; then
      if (( ! optional )); then
        print " fatal: $name argument is required" >&2
        local proj_cmd="${cmd%% *}"
        print " run: ${hi_yellow_cor}$proj_cmd -h${reset_cor} to see usage" >&2
      ...
      if (( is_default )); then
        case "$type" in
          b)
            values+=("$(get_my_branch_ 2>/dev/null)")
            ;;
          f)
            values+=("$(pwd)")
            ;;
    ...
    if (( ! is_usable )) && (( ! is_valid || maybe_required )); then
      print " fatal: not a valid $name argument: $value" >&2
      local proj_cmd="${cmd%% *}"
      print " run: ${hi_yellow_cor}$proj_cmd -h${reset_cor} to see usage" >&2
    ...
    if (( ! is_usable )); then
      ...
      repeat $repeat_count; do
        empty_args+=("__EMPTY__")
      done

      parse_args_ "${original_arg_count}" "$cmd" "$spec" "${values[@]}" "${empty_args[@]}" "${@:$arg_idx}"
  ...
  # output: values separated by TAB, with original_arg_count at the end
  local output=""
  local val=""
  for val in "${values[@]}"; do
    output+="${val}${TAB}"
  done
  for val in "${a_values[@]}"; do
    if [[ "$val" == "${a_values[-1]}" ]]; then
      output+="${val}${TAB}"
    else
      output+="${val}${NUL}"
    fi
...
function input_number_() {
  ...
  while true; do
    local typed_value=""
    if (( input_number_is_k )); then
      typed_value="$(input_type_mandatory_ -k "$header" "$placeholder" "$max" "$value")"
    else
      typed_value="$(input_type_mandatory_ "$header" "$placeholder" "$max" "$value")"
    fi
    if (( $? == 130 )); then return 130; fi
...
function check_proj_() {
  ...
  if [[ -z "$i" ]]; then
    print " fatal: check_proj_ i is required" >&2
    return 1;
  fi
...
function get_pkg_field_online_() {
  ...
  if [[ -n "$branch" ]]; then
    package_json="$(gh api repos/${repo_name}/contents/package.json\?ref=$branch --jq '.download_url // empty')"
  else
    package_json="$(gh api repos/${repo_name}/contents/package.json --jq '.download_url // empty')"
  fi
...
function detect_pkg_manager_online_() {
  ...
  if [[ -n "$branch" ]]; then
    package_json="$(gh api ${url}/package.json\?ref=$branch --jq '.download_url // empty')"
  else
    package_json="$(gh api ${url}/package.json --jq '.download_url // empty')"
  fi
  ...
  if [[ -n "$package_json" ]]; then
    ...
    if [[ -n "$branch" ]]; then
      if gh api $url/package-lock.json?ref=$branch --silent &>/dev/null; then
        manager="npm"
      elif gh api $url/yarn.lock?ref=$branch --silent &>/dev/null; then
        manager="yarn"
...
function save_pkg_manager_() {
  ...
  local pkg_manager="$(detect_pkg_manager_ "$proj_folder" "$proj_repo" 2>/dev/null)"
...
function save_proj_() {
  ...
  local pkg_name="$(get_pkg_name_ -o "$TEMP_PUMP_FOLDER" "$TEMP_PUMP_REPO" "$TEMP_PUMP_SINGLE_MODE" 2>/dev/null)"
  update_config_ $i "PUMP_PKG_NAME" "$pkg_name" &>/dev/null
...
function determine_target_branch_() {
  ...
  local label=""
  if [[ -n "$branch" ]]; then
    label="target branch for ${pink_cor}$branch${reset_cor}"
  else
    label="target branch"
  fi
...
function is_branch_existing_local_() {
  ...
  if get_local_branch_ "$branch" "$proj_folder" &>/dev/null; then
    return 0;
  fi
...
function is_branch_existing_remote_() {
  ...
  if get_remote_branch_ "$branch" "$proj_folder" &>/dev/null; then
    return 0;
  fi
...
function is_branch_existing_() {
  ...
  if get_remote_branch_ "$branch" "$proj_folder" &>/dev/null || get_local_branch_ "$branch" "$proj_folder" &>/dev/null; then
    return 0;
  fi
...
function get_long_name_() {
  ...
  if is_remote_branch_name_ "$branch" "$folder" &>/dev/null; then
    echo "$branch"
    return 0;
  fi
  ...
  echo "refs/remotes/${remote_name}/${short_branch}"
}
...
function find_base_branch_() {
  ...
  local base_branch=""
  base_branch="$(gum spin --title="finding base branch..." -- zsh -ic "get_base_branch_ \"$my_branch\" \"$folder\" 2>/dev/null")"
  if (( $? != 0 )); then
  ...
  if [[ -n "$base_branch" ]]; then
    if (( find_base_branch_is_l )); then
      echo "$(get_long_name_ "$base_branch" "$folder")"
    else
      echo "$(get_short_name_ "$base_branch" "$folder")"
    fi
  fi
}
...
function get_default_branch_() {
  ...
  if [[ -n "$default_branch" ]]; then
    if (( get_default_branch_is_l )); then
      echo "$(get_long_name_ "$default_branch" "$folder")"
    else
      echo "$(get_short_name_ "$default_branch" "$folder")"
    fi
    return 0;
  fi
...
function select_branches_() {
  ...
  if (( select_branches_is_a )); then
    if (( select_branches_is_c )); then # co command      
      ...
      output="$(git -C "$folder" branch --all --list "${remote_name}/${search_text}" --list "$search_text" --sort="$sort_git" -i --no-column --format='%(refname:short)' 2>/dev/null \
        | sed "s#^$remote_name/##" \
        | grep -v 'detached' \
        | grep -v 'HEAD' \
        | grep -vFx "$remote_name" \
        | sed '/^$/d' \
        | LC_ALL=C sort $sed_sort
      )" 

    elif (( select_branches_is_x )); then
      if is_remote_branch_name_ "$search_arg" "$folder"; then
        search_arg="${search_arg#$remote_name/}"
      fi
      ...
      output="$(git -C "$folder" branch --all --list "$search_text" --sort="$sort_git" -i --no-column --format='%(refname:short)' 2>/dev/null \
        | grep -v 'detached' \
        | grep -v 'HEAD' \
        | grep -vFx "$remote_name" \
        | sed '/^$/d' \
    ...
    output="$(git -C "$folder" branch --all --list "${remote_name}/${search_text}" --sort="$sort_git" -i --no-column --format='%(refname:short)' 2>/dev/null \
      | grep -vFx "$remote_name" \
    )"
    ...
    output="$(git -C "$folder" branch --list "$search_text" --sort="$sort_git" -i --no-column --format='%(refname:short)' 2>/dev/null \
      | grep -v 'detached' \
      | grep -v 'HEAD' \
      | sed '/^$/d'
    )"
...
function load_default_config_() {
  ...
  if (( i == 0 )); then
    ...
    PUMP_PKG_MANAGER[0]="$(detect_pkg_manager_ "${PUMP_FOLDER[0]}" "${PUMP_REPO[0]}" 2>/dev/null)"
    PUMP_PKG_NAME[0]="$(get_pkg_name_ "${PUMP_FOLDER[0]}" "${PUMP_REPO[0]}" 1 2>/dev/null)"
  fi
...
function load_global_settings_() {
  ...
  if [[ "$PUMP_SKIP_DETECT_NODE" != "0" && "$PUMP_SKIP_DETECT_NODE" != "1" ]]; then
    ...
    update_setting_ "PUMP_SKIP_DETECT_NODE" "$PUMP_SKIP_DETECT_NODE" &>/dev/null
  fi
  ...
  if [[ "$PUMP_NODE_REQ_SAME_MINOR" != "0" && "$PUMP_NODE_REQ_SAME_MINOR" != "1" ]]; then
    ...
    update_setting_ "PUMP_NODE_REQ_SAME_MINOR" "$PUMP_NODE_REQ_SAME_MINOR" &>/dev/null
  fi
  ...
  if [[ -z "$PUMP_UPDATE_DAY" || ! "$PUMP_UPDATE_DAY" =~ ^[0-9]+$ ]] || (( PUMP_UPDATE_DAY < 1 || PUMP_UPDATE_DAY > 7 )); then
    ...
    update_setting_ "PUMP_UPDATE_DAY" "$PUMP_UPDATE_DAY" &>/dev/null
  fi
}
...
function fix() {
  ...
  parse_args_ "$0" "folder:fz" "$@" 1>/dev/null
  if (( $? != 0 )); then return 1; fi
...
function refix() {
  ...
  if (( refix_is_h )); then
    ...
    print "  ${hi_yellow_cor}refix -q${reset_cor} : --quiet"
    return 0;
  fi
  ...
  local is_quiet="$( (( ${argv[(Ie)--quiet]} || refix_is_q )) && echo 1 || echo 0)"

  local is_amend=0
  local cannot_amend=0
  local message="style: lint and format"
  ...
  if (( is_amend )); then
    ...
    if [[ -z "$last_message" ]]; then
      print " ${red_cor}fatal: no previous commit, cannot refix${reset_cor}" >&2
      return 1;
    fi
  ...
  if (( is_amend )); then
    if ! commit -ef "$message" "$@"; then return 1; fi
  else
    if ! commit -f "$message" "$@"; then return 1; fi
  fi
...
function add() {
  ...
  if (( add_is_h )); then
    ...
    print "  --"
    print "  ${hi_yellow_cor}add -f${reset_cor} : skip confirmation"
    return 0;
  fi
...
function rem() {
  ...
  if (( rem_is_h )); then
    ...
    print "  --"
    print "  ${hi_yellow_cor}rem -f${reset_cor} : skip confirmation"
    return 0;
  fi
...
function reset1() {
  ...
  local arg_count=0
  local output=""
  output="$(parse_args_ "$0" "folder:fz" "$@")"
  if (( $? != 0 )); then return 1; fi
  IFS=$TAB read -r folder arg_count <<< "$output"
...
function read_commits_() {
  ...
  if [[ -z "$target_branch" ]]; then
    local target_branch="$(get_base_branch_ -l "$folder" 2>/dev/null)"
    if [[ -z "$target_branch" ]]; then
...
function pr() {
  ...
  if (( pr_is_h )); then
    ...
    print "  ${hi_yellow_cor}pr -lc${reset_cor} : set label type: devops or ci or chore"
    return 0;
  fi
  ...
  local arg_count=0
  local output=""
  output="$(parse_args_ "$0" "title:to,folder:fz" "$@")"
  if (( $? != 0 )); then return 1; fi
  IFS=$TAB read -r title folder arg_count <<< "$output"
  ...
  if (( pr_is_t )); then
    ...
    if [[ -z "$pump_pkg_manager" ]]; then
      pump_pkg_manager="$(detect_pkg_manager_ "$folder" 2>/dev/null)"
      if [[ -z "$pump_pkg_manager" ]]; then
        print " fatal: missing package manager, run: ${hi_yellow_cor}refresh${reset_cor}" >&2
        return 1;
      fi
...
function proj_run_() {
  ...
  local arg_count=0
  local output=""
  output="$(parse_args_ "run" "proj:po,script:to,folder:fz" "$@")"
  if (( $? != 0 )); then return 1; fi
  IFS=$TAB read -r proj_arg script_arg folder_arg arg_count <<< "$output"
...
function proj_setup_() {
  ...
  local arg_count=0
  local output=""
  output="$(parse_args_ "run" "proj:po,folder:fz" "$@")"
  if (( $? != 0 )); then return 1; fi
  IFS=$TAB read -r proj_arg folder_arg arg_count <<< "$output"
...
function clone_repo_() {
  ...
  if [[ -z "$header" ]]; then
    header="cloning... ${pink_cor}$repo_name${reset_cor}"
  fi
...
function proj_rev_() {
  ...
  local output=""
  output="$(parse_args_ "$proj_cmd rev" "proj:p,branch:bo" "$@")"
  if (( $? != 0 )); then return 1; fi
  IFS=$TAB read -r proj_cmd branch_arg _ <<< "$output"
  ...
  local branch="" jira_key="" pr_number="" pr_title="" pr_url="" pr_state=""
  ...
  # proj_rev_ -j select jira key
    ...
    jira_key="$(select_jira_ticket_ -y $i "$branch_arg" "" "to review")"
    if (( $? == 130 )); then return 130; fi
  ...
  # proj_rev_ -r select by jira key in release
    ...
    jira_key="$(select_jira_key_by_release_ -y $i "$branch_arg" "" "to review")"
    if (( $? == 130 )); then return 130; fi
    if [[ -z "$jira_key" ]]; then return 1; fi
  ...
  # proj_rev_ -b select branch
    ...
    if [[ -z "$branch" ]]; then
      if command -v gh &>/dev/null; then
        local output="" pr_branch=""
        output="$(select_pr_ -odg "$branch_arg" "$proj_repo" "pull request to review")"
        if (( $? == 130 )); then return 130; fi
        if (( $? == 0 )); then
          IFS=$TAB read -r pr_number pr_title pr_url pr_branch pr_state _ <<< "$output"
        fi
  ...
  if [[ -z "$branch" ]]; then
    if [[ -n "$full_rev_folder" ]]; then
      if [[ -d "$full_rev_folder" ]]; then
        print " cannot determine branch for folder-only review: $branch_arg" >&2
        cd "$full_rev_folder"
      else
        print " review folder does not exist: $full_rev_folder" >&2
      fi
    else
      print " cannot determine branch to review" >&2
    fi
    print " run: ${hi_yellow_cor}$proj_cmd rev -h${reset_cor} to see usage" >&2
    return 1;
  fi
  ...
  if [[ -z "$pr_number" || -z "$pr_title" || -z "$pr_url" ]]; then
    local output=""
    output="$(get_pr_ -omc "$branch" "$proj_repo")"
    if (( $? == 0 )); then
      IFS=$TAB read -r pr_number pr_title pr_url pr_state _ <<< "$output"
    fi

    if [[ -z "$pr_number" ]]; then
      pr_number="$(read_pump_value_ "PR_NUMBER" "$full_rev_folderv")"
      pr_title="$(read_pump_value_ "PR_TITLE" "$full_rev_folderv")"
      pr_url="$(read_pump_value_ "PR_LINK" "$full_rev_folder")"
    fi
  fi
  ...
  if [[ -n "$pr_title" ]]; then
    print " opening pull request... ${hi_cyan_cor}$pr_title${reset_cor}"
  elif [[ -n "$pr_url" ]]; then
    print " opening pull request... ${hi_cyan_cor}$pr_url${reset_cor}"
  else
    print " opening branch... ${hi_cyan_cor}$branch${reset_cor}"
  fi
  ...
  if is_folder_git_ "$full_rev_folder" &>/dev/null; then
    ...
    if [[ "$branch" == "$local_branch" ]]; then
      ...
      if pull -r "$full_rev_folder" --quiet; then
        ...
        local output=""
        output="$(get_my_branch_status_ "$branch" "$remote_branch" "$full_rev_folder")"
        if (( $? == 0 )); then
    ...
    if ! clone_repo_ -o "$proj_repo" "$full_rev_folder" "cloning... ${pink_cor}$branch${reset_cor}"; then
      print " ${red_cor}fatal: failed to clone branch: $branch${reset_cor}" >&2
      return 1;
    fi
    ...
    if command -v gh &>/dev/null; then
      ...
      if [[ -n "$pr_target_branch" ]]; then
        git -C "$full_rev_folder" config branch.$branch.pump-merge $pr_target_branch
      fi
    fi
  ...
  if [[ -n "$pr_number" && -n "$pr_title" && -n "$pr_url" ]]; then
    ...
    update_pump_file_ "PR_LINK" "$pr_url" "$full_rev_folder"
  fi
...
function proj_clone_() {
  ...
  local proj_cmd="$1"
  local branch_arg=""
  local target_branch_arg=""
  local work_type=""

  local arg_count=0
  local output=""
  output="$(parse_args_ "$proj_cmd clone" "proj:p,branch:bk,target_branch:bk,work_type:tk" "$@")"
  if (( $? != 0 )); then return 1; fi
  IFS=$TAB read -r proj_cmd branch_arg target_branch_arg work_type arg_count <<< "$output"
  ...
  if (( single_mode )); then
    ...
    if [[ -z "$git_folder" ]]; then
      ...
      if [[ -n "$branch_arg" ]]; then
        ...
        if [[ "$branch_arg" != "$first_branch" ]]; then
    ...
    if (( is_first_branch )); then
      ...
      if [[ -z "$branch_arg" ]]; then
        branch_arg="$(input_branch_name_ "branch name")"
        if (( $? == 130 )); then return 130; fi
      ...
      if [[ -n "$jira_key" && "${branch_arg:t:u}" == "${jira_key:u}" ]]; then
      ...
      if [[ -n "$jira_key" ]]; then
        ...
        while true; do
          ...
          if is_branch_existing_ "$branch_arg" "$proj_folder" &>/dev/null; then
            if is_folder_git_ "$folder_to_clone" &>/dev/null; then break; fi
          else
            break;
          fi
        done
  ...
  if ! is_folder_git_ "$folder_to_clone" &>/dev/null; then
    if ! clone_repo_ -o "$proj_repo" "$folder_to_clone" "cloning... ${pink_cor}$branch_arg${reset_cor}"; then
      print " ${red_cor}fatal: failed to clone repository: $proj_repo${reset_cor}" >&2
      return 1;
    fi
  fi
  ...
  if [[ -n "$jira_key" ]]; then
    local pump_jira_key="$(read_pump_value_ "JIRA_KEY" "$folder_to_clone")"

    if [[ -z "$pump_jira_key" ]]; then
      update_pump_file_ "JIRA_KEY" "$jira_key" "$folder_to_clone"
    fi

    local pump_jira_title="$(read_pump_value_ "JIRA_TITLE" "$folder_to_clone")"
  ...
  if [[ -n "$branch_arg" && "$first_branch" != "$target_branch" && "$branch_arg" != "$first_branch" && "$branch_arg" != "main" && "$branch_arg" != "master" ]]; then
    ...
    if [[ -z "$target_branch" ]]; then
      ...
      if [[ -z "$target_branch" ]]; then
        target_branch="$(determine_target_branch_ -dbm "$branch_arg" "$folder_to_clone" "$proj_cmd" "$first_branch")"
      fi
    fi
  fi
  ...
  if [[ -n "$target_branch" ]]; then
    if git -C "$folder_to_clone" switch "$target_branch" &>/dev/null; then
      ...
      git -C "$folder_to_clone" config branch.$branch_arg.pump-merge $target_branch
  ...
  if [[ -n "$branch_arg" ]]; then
    ...
    if [[ -z "$remote_branch_arg" ]]; then
      ...
      print " branch create: ${pink_cor}$branch_arg${reset_cor} but not in $remote_name"
    else
  ...
  if [[ -n "${PUMP_SETUP[$i]}" ]]; then
    ...
    print "    ${white_cor}edit PUMP_SETUP_$i in your pump.json file to customize the setup script${reset_cor}"
  fi
...
function proj_pull_() {
  ...
  parse_args_ "$proj_cmd pull" "proj:p" "$@" 1>/dev/null
  if (( $? != 0 )); then return 1; fi
...
function proj_prs_() {
  ...
  local arg_count=0
  local output=""
  output="$(parse_args_ "$proj_cmd prs" "proj:p,search:to,interval:no" "$@")"
  if (( $? != 0 )); then return 1; fi
  IFS=$TAB read -r proj_cmd search_text interval arg_count <<< "$output"

  shift $arg_count

  if [[ -z "$interval" ]]; then
    if [[ -z "$PUMP_INTERVAL" ]]; then
      PUMP_INTERVAL="$(input_number_ "type the interval in minutes" "20" 2)"
      if (( $? == 130 )); then return 130; fi
      if [[ -n "$PUMP_INTERVAL" ]]; then
        update_setting_ "PUMP_INTERVAL" "$PUMP_INTERVAL"
      else
        PUMP_INTERVAL="20"
      fi
    fi
  ...
  if (( ! proj_prs_is_a && ! proj_prs_is_l )) && [[ -n "$search_text" ]]; then
  ...
  if (( proj_prs_is_a )); then
    ...
    while true; do
      local pr_list=""
      pr_list="$(list_prs_ -o "$search_text" "$proj_repo")"
      if (( $? == 130 )); then return 130; fi
      if [[ -z "$pr_list" ]]; then break; fi
      ...
      for line in "${(@f)pr_list}"; do
        ...
        approve_pr_ "$pr_number" "$pr_title" "$pr_url" "$pr_branch" "$pr_state" "$pr_author" "$is_draft" "$has_dnm_label" "$approval_count" "$user_has_approved" "$pr_approval_min" "$current_user" "$@"
        local RET=$?
        if (( RET == 130 )); then return 130; fi
  ...
  if (( proj_prs_is_l_l )); then
    ...
    local releases="$(select_jira_releases_ $i "$jira_proj" "$search_text" "$jira_api_token")"
    ...
    for release in "${(@f)releases}"; do
      proj_prs_l_ -f $i "$release" "$search_text" "" "$@"
  ...
  for line in "${(@f)pr_list}"; do
    ...
    print_pr_status_ "$pr_number" "$pr_title" "$pr_url" "$pr_branch" "$pr_state" "$pr_author" "$is_draft" "$has_dnm_label" "$approval_count" "$user_has_approved" "$pr_approval_min" "$current_user"
  done
}
...
function proj_prs_l_() {
  ...
  for label in "${choose_labels[@]}"; do
    if [[ -z "$(gum spin --title="fetching labels..." -- gh label list --repo "$proj_repo" --search "$label" 2>/dev/null)" ]]; then
...
function select_prs_() {
  ...
  if (( select_prs_is_1 )); then
    prs="$(choose_one_ "$header" "${pr_choices[@]}")"
  else
    prs="$(choose_multiple_ "$header" "${pr_choices[@]}")"
  fi
  if (( $? == 130 )); then return 130; fi
...
function proj_bkp_() {
  ...
  parse_args_ "$proj_cmd bkp" "proj:p" "$@" 1>/dev/null
  if (( $? != 0 )); then return 1; fi
...
function proj_dbkp_() {
  ...
  local output=""
  output="$(parse_args_ "$proj_cmd bkp" "proj:p,folder:fo" "$@")"
  if (( $? != 0 )); then return 1; fi
  IFS=$TAB read -r proj_cmd folder_arg _ <<< "$output"
...
function proj_tag_() {
  ...
  local output=""
  output="$(parse_args_ "$proj_cmd tag" "proj:p,tag:to" "$@")"
  if (( $? != 0 )); then return 1; fi
  IFS=$TAB read -r proj_cmd tag _ <<< "$output"
...
function proj_tags_() {
  ...
  parse_args_ "$proj_cmd tags" "proj:p,n:no" "$@" 1>/dev/null
  if (( $? != 0 )); then return 1; fi
...
function proj_dtag_() {
  ...
  parse_args_ "$proj_cmd tag" "proj:p,tag:to" "$@" 1>/dev/null
  if (( $? != 0 )); then return 1; fi
...
function proj_rel_() {
  ...
  local params="pro:p,branch:b,tag:to,title:to"

  if [[ "$proj_cmd" == "$CURRENT_PUMP_SHORT_NAME" ]] && is_folder_git_ &>/dev/null; then
    params="pro:p,branch:bz,tag:to,title:to"
  fi

  local output=""
  output="$(parse_args_ "$proj_cmd rel" "$params" "$@")"
  if (( $? != 0 )); then return 1; fi
  IFS=$TAB read -r proj_cmd branch tag title _ <<< "$output"
...
function proj_rels_() {
  ...
  parse_args_ "$proj_cmd rels" "proj:p,limit:no" "$@" 1>/dev/null
  if (( $? != 0 )); then return 1; fi
...
function proj_drel_() {
  ...
  parse_args_ "$proj_cmd rel" "proj:p,tag:to" "$@" 1>/dev/null
  if (( $? != 0 )); then return 1; fi
...
function proj_jira_() {
  ...
  local arg_count=0
  local output=""
  output="$(parse_args_ "$proj_cmd jira" "proj:p,key:jk,status:to" "$@")"
  if (( $? != 0 )); then return 1; fi
  IFS=$TAB read -r proj_cmd search_key jira_status arg_count <<< "$output"
  ...
  if (( proj_jira_is_v )) && [[ -n "$search_key" ]]; then
    update_jira_status_ -v $i "$search_key" "" ""
    return $?;
  fi
  ...
  if (( proj_jira_is_v || proj_jira_is_c || proj_jira_is_a || proj_jira_is_e || proj_jira_is_r || proj_jira_is_t || proj_jira_is_s || proj_jira_is_d )); then
    ...
    if (( proj_jira_is_x )); then
      ...
      if (( single_mode )); then
        local branch_found="$(proj_jira_find_branch_ "$search_key" "$label" 2>/dev/null)"
        if (( $? == 130 )); then return 130; fi

        jira_key="$(extract_jira_key_ "$branch_found")"

      else
        local choosen_folder="$(proj_jira_find_folder_ $i "$search_key" "$label" "$proj_folder" 2>/dev/null)"
        if (( $? == 130 )); then return 130; fi
    ...
    if [[ -z "$jira_key" ]]; then
      jira_key="$(select_jira_ticket_ $i "$search_key" "$jira_status" "$label" "$@")"
      if (( $? == 130 )); then return 130; fi
      if [[ -z "$jira_key" ]]; then return 1; fi
    fi

    update_jira_status_ $i "$jira_key" "$jira_status" "$@"
  ...
  if (( proj_jira_is_x )); then
    ...
    jira_key="$(select_jira_ticket_ -o $i "$search_key" "" "$label")"
    if (( $? == 130 )); then return 130; fi
    if [[ -z "$jira_key" ]]; then return 1; fi
  fi
  ...
  if (( single_mode )); then
    ...
    local branch_found="$(select_branch_ -jli "$jira_key" "branch to open" "$proj_folder" 2>/dev/null)"
    if (( $? == 130 )); then return 130; fi
  ...
  if (( proj_jira_is_f )); then
    update_jira_status_ -of $i "$jira_key" "$jira_in_progress"
  else
    update_jira_status_ -o $i "$jira_key" "$jira_in_progress"
  fi
...
function check_jira_() {
  ...
  if (( check_jira_is_p )); then
    if (( PUMP_JIRA_ALERT < 2 && ! check_jira_is_i )) && ! check_jira_cli_; then
      (( PUMP_JIRA_ALERT++ ))
      update_setting_ $i "PUMP_JIRA_ALERT" "$PUMP_JIRA_ALERT" &>/dev/null
    fi
  ...
  if (( check_jira_is_s )); then
    ...
    local colors=("${bold_yellow_cor}" "${bold_blue_cor}" "${bold_magenta_cor}" "${bold_green_cor}" "${bold_dark_green_cor}" "${bold_orange_cor}" "${bold_pink_cor}" "${bold_red_cor}" "${bold_cyan_cor}")
    ...
    if (( check_jira_is_d || check_jira_is_s_s )); then
      ...
      if [[ -z "${STATUS_COLOR_MAP[$jira_todo]}" ]]; then STATUS_COLOR_MAP[$jira_todo]="$cor"; fi
    ...
    if (( check_jira_is_o || check_jira_is_s_s )); then
      ...
      if [[ -z "${STATUS_COLOR_MAP[$jira_in_progress]}" ]]; then STATUS_COLOR_MAP[$jira_in_progress]="$cor"; fi
    ...
    if (( check_jira_is_r || check_jira_is_s_s )); then
      ...
      if [[ -z "${STATUS_COLOR_MAP[$jira_in_review]}" ]]; then STATUS_COLOR_MAP[$jira_in_review]="$cor"; fi
    ...
    if (( check_jira_is_t || check_jira_is_s_s )); then
      ...
      if [[ -z "${STATUS_COLOR_MAP[$jira_in_test]}" ]]; then STATUS_COLOR_MAP[$jira_in_test]="$cor"; fi
      ...
      if [[ -z "${STATUS_COLOR_MAP[$jira_ready_for_test]}" ]]; then STATUS_COLOR_MAP[$jira_ready_for_test]="$cor"; fi
    ...
    if (( check_jira_is_a || check_jira_is_s_s )); then
      ...
      if [[ -z "${STATUS_COLOR_MAP[$jira_almost_done]}" ]]; then STATUS_COLOR_MAP[$jira_almost_done]="$cor"; fi
    ...
    if (( check_jira_is_e || check_jira_is_s_s )); then
      ...
      if [[ -z "${STATUS_COLOR_MAP[$jira_done]}" ]]; then STATUS_COLOR_MAP[$jira_done]="$cor"; fi
    ...
    if (( check_jira_is_c || check_jira_is_s_s )); then
      ...
      if [[ -z "${STATUS_COLOR_MAP[$jira_canceled]}" ]]; then STATUS_COLOR_MAP[$jira_canceled]="$cor"; fi
    ...
    if (( check_jira_is_b || check_jira_is_s_s )); then
      ...
      if [[ -z "${STATUS_COLOR_MAP[$jira_blocked]}" ]]; then STATUS_COLOR_MAP[$jira_blocked]="$cor"; fi
...
function proj_jira_release_() {
  ...
  local arg_count=0
  local output=""
  output="$(parse_args_ "$proj_cmd jira release" "proj:p,key:jk,status:to" "$@")"
  if (( $? != 0 )); then return 1; fi
  IFS=$TAB read -r proj_cmd search_key jira_status arg_count <<< "$output"
  ...
  local jira_key="$(select_jira_key_by_release_ $i "$search_key" "$jira_status" "$label" "$@")"
  if (( $? == 130 )); then return 130; fi
  if [[ -z "$jira_key" ]]; then return 1; fi
...
function update_jira_status_() {
  ...
  local current_status="" work_type="" assignee_name="" current_assignee_email="" summary=""
  ...
  IFS=$TAB read -r current_status work_type assignee_name current_assignee_email summary _ <<< "$_output"
  ...
  if (( update_jira_status_is_v )); then
    ...
    summary="$(truncate_ $summary 70)"
    
    display_line_ "" "${cor}" 70
    print " ${gray_cor}ticket: ${cor}$jira_link${reset_cor}"
    print " ${gray_cor}status: ${cor}$current_status${reset_cor}"
    print " ${gray_cor} title:${reset_cor} ${bold_cor}$summary${reset_cor}"
    print " ${gray_cor}  type:${reset_cor} $work_type"
    print " ${gray_cor}assign:${reset_cor} $assignee_name"
    ...
    for line in "${(@f)pr_list}"; do
      ...
      print " ${gray_cor}pull r: $(print_pr_status_ "$pr_number" "$pr_title" "$pr_url" "$pr_branch" "$pr_state" "$pr_author" "$is_draft" "$has_dnm_label" "$approval_count" "$user_has_approved" "$pr_approval_min" "$current_user")"
    done
  ...
  if (( update_jira_status_is_o || update_jira_status_is_r )); then
    if [[ "$current_assignee_email" == "$current_jira_user_email" ]]; then
      ...
      local output=""
      if [[ -n "$current_assignee_email" ]]; then
        local _RET=0
        if (( ! update_jira_status_is_f )); then
          confirm_ "work item ${jira_key} is assigned to ${yellow_cor}$current_assignee_email${reset_cor} - re-assign it to you?"
          _RET=$?
        fi
        if (( _RET == 130 || _RET == 2 )); then return 130; fi
        if (( _RET == 0 )); then
  ...
  if [[ -n "$current_assignee_email" && "$current_assignee_email" != "$current_jira_user_email" ]]; then
  ...
  local _RET=0
  if (( ! update_jira_status_is_f )); then
    confirm_ "transition of work item ${jira_key} assigned to ${yellow_cor}$current_assignee_email${reset_cor} to status: ${cor}${jira_status}${reset_cor}?"
    _RET=$?
  fi
  if (( _RET == 130 || _RET == 2 )); then return 130; fi
...
function select_jira_ticket_() {
  ...
  if [[ "$jira_key" == "more..." ]]; then
    jira_key="$(select_jira_ticket_ -n $i "" "$label" "$@")"
  fi
...
function renb() {
  ...
  if (( renb_is_h )); then
    print "  ${hi_yellow_cor}renb ${yellow_cor}<new_branch> [<folder>]${reset_cor} : rename current branch"
    print "  --"
  ...
  local output=""
  output="$(parse_args_ "$0" "new_branch:b,folder:fz" "$@")"
  if (( $? != 0 )); then return 1; fi
  IFS=$TAB read -r new_branch folder _ <<< "$output"
  ...
  if ! normalize_branch_name_ "$new_branch" 1>/dev/null; then return 1; fi
...
function chp() {
  ...
  if (( chp_is_h )); then
    print "  ${hi_yellow_cor}chp ${yellow_cor}[<hash>] [<folder>]${reset_cor} : cherry-pick commit"
  ...
  if [[ -z "$hash_arg" ]]; then
    ...
    commit="$(git -C "$folder" --no-pager log -1 --pretty=%s $hash_arg 2>/dev/null)"
  fi
...
function revert() {
  ...
  local arg_count=0
  local output=""
  output="$(parse_args_ "$0" "hash:ho,parent_number:no,folder:fz" "$@")"
  if (( $? != 0 )); then return 1; fi
  IFS=$TAB read -r hash parent_number folder arg_count <<< "$output"
...
function conti() {
  ...
  if (( conti_is_h )); then
    ...
    print "  --"
    print "  ${hi_yellow_cor}conti -n${reset_cor} : --no-commit"
    return 0;
  fi
...
function fetch() {
  ...
  local arg_count=0
  local output=""
  output="$(parse_args_ "$0" "branch:bo,folder:fz" "$@")"
  if (( $? != 0 )); then return 1; fi
  IFS=$TAB read -r branch folder arg_count <<< "$output"
...
function gconf() {
  ...
  if (( gconf_is_h )); then
    ...
    print "  --"
    print "  ${hi_yellow_cor}gconf -r <entry>${reset_cor} : remove an entry from the configuration"
    print "  --"
    print "  ${hi_yellow_cor}gconf -e${reset_cor} : display current effective configuration (mix of all)"
    print "  ${hi_yellow_cor}gconf -a${reset_cor} : display all scopes (local, global, system)"
    print "  --"
    print "  ${hi_yellow_cor}gconf -g${reset_cor} : global configuration"
    print "  ${hi_yellow_cor}gconf -l${reset_cor} : local configuration"
    print "  ${hi_yellow_cor}gconf -s${reset_cor} : system configuration"
    return 0;
  fi
  ...
  local output=""
  output="$(parse_args_ "$0" "entry:to,folder:fz" "$@")"
  if (( $? != 0 )); then return 1; fi
  IFS=$TAB read -r entry folder _ <<< "$output"
...
function glog() {
  ...
  if (( glog_is_h )); then
    ...
    print "  ${hi_yellow_cor}glog -s${reset_cor} : --stat"
    return 0;
  fi
  ...
  if (( glog_is_b )); then
    ...
    output="$(parse_args_ "$0" "branch:b,folder:fz" "$@")"
    if (( $? != 0 )); then return 1; fi
    IFS=$TAB read -r branch folder arg_count <<< "$output"
  else
    output="$(parse_args_ "$0" "format:to,branch:bo,folder:fz" "$@")"
    if (( $? != 0 )); then return 1; fi
    IFS=$TAB read -r format branch folder arg_count <<< "$output"
  fi
  ...
  if (( glog_is_b || glog_is_c )); then
    ...
    if [[ -z "$branch" ]]; then
      branch="$(get_base_branch_ -l "" "$folder")"
      if [[ -z "$branch" ]]; then
    ...
    print " showing commits of ${cyan_cor}$branch${reset_cor} after head of ${hi_cyan_cor}${branch}${reset_cor}"
    print ""
...
function repush() {
  ...
  if (( repush_is_h )); then
    ...
    print "  --"
    print "  ${hi_yellow_cor}repush -i${reset_cor} : only repush staged changes"
    print "  ${hi_yellow_cor}repush -c${reset_cor} : create message using conventional commits standard"
    print "  ${hi_yellow_cor}repush -j${reset_cor} : create message using jira if available"
    print "  ${hi_yellow_cor}repush -n${reset_cor} : prompt for a bigger commit message"
    print "  ${hi_yellow_cor}repush -q${reset_cor} : --quiet"
  ...
  local arg_count=0
  local output=""
  output="$(parse_args_ "$0" "message:to,folder:fz" "$@")"
  if (( $? != 0 )); then return 1; fi
  IFS=$TAB read -r message folder arg_count <<< "$output"
...
function pull() {
  set +x
  eval "$(parse_flags_ "$0" "trmfopq" "xw" "$@")"
  (( pull_is_debug )) && set -x

  if (( pull_is_h )); then
    print "  ${hi_yellow_cor}pull ${yellow_cor}[<folder>]${reset_cor} : update current branch with its configured upstream"
    print "  ${hi_yellow_cor}pull [<origin/upstream>] <branch> ${yellow_cor}[<folder>]${reset_cor} : update from this exact branch, no guessing"
    print "  --"
  ...
  local arg_count=0
  local output=""
  output="$(parse_args_ "$0" "remote_name:ro,branch:bz,folder:fz" "$@")"
  if (( $? != 0 )); then return 1; fi
  IFS=$TAB read -r remote_name branch folder arg_count <<< "$output"
...
function reseta() {
  ...
  if (( reseta_is_h )); then
    ...
    print "  --"
    print "  ${hi_yellow_cor}reseta -o${reset_cor} : erase every change and match HEAD to upstream"
  ...
  local arg_count=0
  local output=""
  output="$(parse_args_ "$0" "branch_or_commit:to,folder:fz" "$@")"
  if (( $? != 0 )); then return 1; fi
  IFS=$TAB read -r branch_or_commit folder arg_count <<< "$output"
...
function workflow_run_() {
  local proj_repo="$1"
  local workflow="$2"
  local search_text="$(trim_ "$3")"

  local repo_name="$(get_repo_name_ "$proj_repo" 2>/dev/null)"

  local colors=("${bold_blue_cor}" "${bold_magenta_cor}" "${bold_green_cor}" "${bold_cyan_cor}" "${bold_orange_cor}" "${bold_pink_cor}" "${bold_dark_green_cor}")
  ...
  local result="$(gum spin --title="checking workflow status... $workflow" -- gh run list --repo "$proj_repo" \
    ...
    --jq "[.[] | select(.displayTitle | contains(\"$search_text\"))] | .[0] | \"\(.databaseId)${TAB}\(.url)${TAB}\(.displayTitle)${TAB}\(.headBranch)${TAB}\(.updatedAt)${TAB}\(.conclusion // empty)\""
  )"

  if [[ -z "$result" ]]; then
    print -n -- " could not find any workflows named \"$workflow\"" >&2
    ...
    if [[ -n "$search_text" ]]; then
      ...
      if [[ -z "$result" ]]; then
        print -n -- " with any title" >&2
      else
        print -n -- " with title \"$search_text\"" >&2
      fi
    fi

    print -n -- " in: $repo_name" >&2
    print "" >&2
    if [[ -n "$result" ]]; then
      return 2;
    else
  ...
  display_line_ "" "${cor}" 55

  print " ${gray_cor}titled: ${cor}$link${reset_cor} @ ${cor}$pkg_version${reset_cor} : ${stat_cor}$stat${reset_cor}"
  # print " ${gray_cor}titled: ${cor}$link${reset_cor}"
  # print " ${gray_cor}pkg v.: ${cor}$pkg_version${reset_cor}"
  # print " ${gray_cor}status: ${stat_cor}$stat${reset_cor}"
  print " ${gray_cor}tagged:${reset_cor} $tag"
  print " ${gray_cor}branch:${reset_cor} $tag_branch"
  # print " ${gray_cor}author:${reset_cor} $author"
  ...
  print " ${gray_cor}posted:${reset_cor} $author @ $updated_at"
  # print " ${gray_cor}posted:${reset_cor} $updated_at"
}
...
function workflow_runs_() {
  ...
  if [[ -z "$@" ]]; then
    workflow_run_ "$proj_repo" "$workflow" ""
    return $?;
  fi
  ...
  if (( count == ${#@} )); then
    print " no workflow runs found with any of the specified search texts, showing most recent run of workflow: $workflow"
    workflow_run_ "$proj_repo" "$workflow" ""
    return $?;
  fi
}
...
function proj_gha_() {
  ...
  if (( proj_gha_is_d && proj_gha_is_a )); then
    output="$(parse_args_ "$proj_cmd gha" "proj:p,interval:nz:20,search_texts:a:to" "$@")"
    if (( $? != 0 )); then return 1; fi
    IFS=$TAB read -r proj_cmd interval search_texts arg_count <<< "$output"

  elif (( proj_gha_is_d )); then
    output="$(parse_args_ "$proj_cmd gha" "proj:p,search_texts:a:to" "$@")"
    if (( $? != 0 )); then return 1; fi
    IFS=$TAB read -r proj_cmd search_texts arg_count <<< "$output"
  
  elif (( proj_gha_is_a )); then
    output="$(parse_args_ "$proj_cmd gha" "proj:p,workflow_arg:to,interval:nz:20,search_texts:a:to" "$@")"
    if (( $? != 0 )); then return 1; fi
    IFS=$TAB read -r proj_cmd workflow_arg interval search_texts arg_count <<< "$output"

  else
    output="$(parse_args_ "$proj_cmd gha" "proj:p,workflow_arg:to,search_texts:a:to" "$@")"
    if (( $? != 0 )); then return 1; fi
    IFS=$TAB read -r proj_cmd workflow_arg search_texts arg_count <<< "$output"
  fi

  search_texts=("${(0)search_texts}")

  # print ""
  # print "proj_cmd = [$proj_cmd]"
  # print "workflow_arg = [$workflow_arg]"
  ...
  #         interval="$arg"
  #       else
  #         print " warning: ignoring numeric argument (intervals require -a): $arg" >&2
  #       fi
  #     else
  #       search_texts+=("$(trim_ -s "$arg")")
  #     fi
  #   fi
  # done
  ...
  if (( proj_gha_is_d )); then
    workflow_arg="${PUMP_GHA_DEPLOY[$i]}"
  fi
  ...
  if [[ -z "$workflow_arg" ]]; then
    ...
    workflow_arg="$(choose_one_ "$label" "${(@f)workflow_choices}")"
    if (( $? == 130 )); then return 130; fi
    if [[ -z "$workflow_arg" ]]; then return 1; fi

    if (( proj_gha_is_d )); then
      local is_save=1
      if (( ! proj_gha_is_f )); then
        is_save=0
        confirm_ "save \"$workflow_arg\" as the default deployment workflow for ${proj_cmd}?"
        local _RET=$?
      ...
      if (( is_save )); then
        update_config_ $i "PUMP_GHA_DEPLOY" "$workflow_arg"
        PUMP_GHA_DEPLOY[$i]="$workflow_arg"
      fi
    fi
  fi

  while true; do
    if ! workflow_runs_ "$proj_repo" "$workflow_arg" "${search_texts[@]}"; then
      if (( proj_gha_is_d )) && [[ -n "$workflow_arg" ]]; then
        PUMP_GHA_DEPLOY[$i]=""
        proj_gha_ -fd "$proj_cmd" "$interval" "${search_texts[@]}"
        return $?;
      else
        return 1;
      fi
...
function co() {
  ...
  if (( co_is_h )); then
    ...
    print "  --"
    print "  ${hi_yellow_cor}co -a ${yellow_cor}[<branch>] [<folder>]${reset_cor} : switch to remote branch"
    print "  ${hi_yellow_cor}co -l ${yellow_cor}[<branch>] [<folder>]${reset_cor} : switch to local branch"
    print "  --"
    print "  ${hi_yellow_cor}co -pr ${yellow_cor}[<pr>] [<folder>]${reset_cor} : switch to pull request (detached branch)"
    print "  --"
    print "  ${hi_yellow_cor}co -b <branch> ${yellow_cor}[<base_branch>] [<folder>]${reset_cor} : create new branch off of chosen base branch"
    print "  ${hi_yellow_cor}co -c <branch> ${yellow_cor}[<base_branch>] [<folder>]${reset_cor} : create new branch off of current branch"
    print "  ${hi_yellow_cor}co -e <branch> ${yellow_cor}[<base_branch>] [<folder>]${reset_cor} : switch to an exact branch, no lookup"
    print "  --"
    print "  ${hi_yellow_cor}co -u ${yellow_cor}[<branch>] [<folder>]${reset_cor} : --set-upstream-to (set up tracking information)"
    return 0;
  fi
  ...
  local arg_count=0
  local output=""
  output="$(parse_args_ "$0" "branch:bo,base_branch:bo,folder:fo" "$@")"
  if (( $? != 0 )); then return 1; fi
  IFS=$TAB read -r branch base_branch folder arg_count <<< "$output"
  ...
  if (( co_is_a )); then
    ...
    if [[ -n "$branch" ]]; then
      branch_choice="$(select_branch_ -aic "$branch" "branch to switch" "$folder")"
    else
      local my_branch="$(get_my_branch_ -l "$folder" 2>/dev/null)"
      local my_branch_short="$(get_my_branch_ "$folder" 2>/dev/null)"

      branch_choice="$(select_branch_ -ac "" "branch to switch" "$folder" "$my_branch" "$my_branch_short")"
    fi
    if (( $? == 130 )); then return 130; fi
  ...
  if (( co_is_l )); then
    ...
    if [[ -n "$branch" ]]; then
      branch_choice="$(select_branch_ -lic "$branch" "local branch to switch" "$folder")"
    else
      local my_branch="$(get_my_branch_ -l "$folder" 2>/dev/null)"
      local my_branch_short="$(get_my_branch_ "$folder" 2>/dev/null)"
  
      branch_choice="$(select_branch_ -lc "" "local branch to switch" "$folder" "$my_branch" "$my_branch_short")"
    fi
    if (( $? == 130 )); then return 130; fi
    if [[ -z "$branch_choice" ]]; then return 1; fi
  ...
  if (( co_is_b || co_is_c )); then
    ...
    if [[ -n "$my_branch" && "$branch" == "$my_branch" ]]; then
      print " fatal: branch already exists: $branch" >&2
      return 1;
    fi
  ...
  if (( co_is_x )); then
    ...
    if git -C "$folder" switch "$base_branch" --quiet &>/dev/null; then
      if ! git -C "$folder" switch -c "$branch" "$@"; then
        return 1;
      fi
  ...
  if (( co_is_e && ! co_is_x )); then
    ...
    if ! git -C "$folder" switch "$branch" "$@"; then
      if ! get_branch_status_ "$folder" &>/dev/null; then
  ...
  if [[ -n "$branch" ]]; then
    ...
    branch_choice="$(select_branch_ -lc "" "local branch to switch" "$folder" "$my_branch" "$my_branch_short" 2>/dev/null)"
    if [[ -z "$branch_choice" ]]; then
      branch_choice="$(select_branch_ -ac "" "local branch to switch" "$folder" "$my_branch" "$my_branch_short")"
    fi
  fi
  if (( $? == 130 )); then return 130; fi
...
function base() {
  ...
  if git -C "$folder" switch "$base_branch"; then
    fetch "$base_branch" "$folder" --quiet
  fi
}
...
function rebase() {
  ...
  if (( rebase_is_f )); then
    output="$(parse_args_ "$0" "base_branch:b,folder:fz" "$@")"
  else
    output="$(parse_args_ "$0" "base_branch:bo,folder:fz" "$@")"
  fi
  if (( $? != 0 )); then return 1; fi
  IFS=$TAB read -r base_branch folder arg_count <<< "$output"
  ...
  if (( rebase_is_w )); then
    local selected_branches=($(select_branches_ -l "" "branches to rebase" "$folder"))
  ...
  if get_branch_status_ "$folder" &>/dev/null; then
    ...
    if ! git -C "$folder" pull --quiet; then
      print " fatal: failed to pull latest changes from remote before rebasing" >&2
      return 1;
    fi
  fi
...
function merge() {
  ...
  if (( merge_is_f )); then
    output="$(parse_args_ "$0" "base_branch:b,strategy:to,folder:fz" "$@")"
  else
    output="$(parse_args_ "$0" "base_branch:bo,strategy:to,folder:fz" "$@")"
  fi
  if (( $? != 0 )); then return 1; fi
  IFS=$TAB read -r base_branch strategy folder arg_count <<< "$output"
  ...
  if (( merge_is_w )); then
    local selected_branches=($(select_branches_ -l "" "branches to merge" "$folder"))
    if [[ -z "$selected_branches" ]]; then return 1; fi
  ...
  if get_branch_status_ "$folder" &>/dev/null; then
    ...
    # pull latest changes from remote
    if ! git -C "$folder" pull --quiet; then
      print " fatal: failed to pull latest changes from remote before merging" >&2
      return 1;
    fi
  fi
...
function delb() {
  ...
  if (( delb_is_h )); then
    ...
    print "  ${hi_yellow_cor}delb -e <branch> ${yellow_cor}[<folder>]${reset_cor} : delete an exact branch, no lookup"
    print "  --"
    print "  ${hi_yellow_cor}delb -a${reset_cor} : find and delete local and remote branches"
    print "  ${hi_yellow_cor}delb -r${reset_cor} : find and delete remote branches"
    print "  ${hi_yellow_cor}delb -f${reset_cor} : skip confirmation (cannot be used with -r or -a)"
  ...
  if (( delb_is_a )); then
    ...
    if (( delb_is_e || delb_is_x )); then
      ...
      excluded_branches+=("$my_branch")
      excluded_branches+=("$my_branch_short")
    fi
  elif (( delb_is_r )); then
    flags+=(-r)
    if (( delb_is_e || delb_is_x )); then
  ...
  selected_branches=($(select_branches_ "${flags[@]}" "$branch" "branches to delete" "$folder" "${excluded_branches[@]}"))
...
function get_pkg_name_() {
  ...
  if [[ -n "$folder" ]]; then
    ...
    if [[ -z "$pkg_name" ]] && (( get_pkg_name_is_o )); then
...
function detect_node_version_() {
  ...
  if [[ -n "$nvm_use_v" && "$nvm_use_v" != "$nvm_version" ]]; then
...
function pro_c_() {
  ...
  confirm_ "remove any temporary folder for ${blue_cor}$proj_cmd${reset_cor}?"
  local RET=$?
  if (( RET == 130 )); then return 130; fi
  if (( RET == 0 )); then
    remove_proj_folders_ -ct "$proj_cmd" "$proj_folder"
  fi

  confirm_ "remove any review folder for ${blue_cor}$proj_cmd${reset_cor}?"
  local RET=$?
  if (( RET == 130 )); then return 130; fi
  if (( RET == 0 )); then
    remove_proj_folders_ -r "$proj_cmd" "$proj_folder"
  fi
...
function pro() {
  set +x
  eval "$(parse_flags_ "$0" "aerulndiscH" "xf" "$@")"
  (( pro_is_debug )) && set -x
  ...
  if (( pro_is_h )); then
    ...
    print "  ${hi_yellow_cor}pro -r ${yellow_cor}[<name...>]${reset_cor} : remove project"
  ...
  if (( pro_is_i )); then
    ...
    for i in {1..9}; do
      if [[ -n "${PUMP_FOLDER[$i]}" && -n "${PUMP_SHORT_NAME[$i]}" ]]; then
        ...
        printf "  ${cyan_cor}%-$spaces${reset_cor} %s \n" "mode:" "${mode_cor}${new_mode}${reset_cor}"
  ...
  if (( pro_is_u )); then
    ...
    if (( pro_is_H )); then
      i="$(find_proj_index_ -oe "$proj_arg"  "project to reset settings for")"
      (( i )) || return 1;
    ...
    local all_global_settings=(
      "PUMP_SKIP_DETECT_NODE    =${PUMP_SKIP_DETECT_NODE}"
      "PUMP_CODE_EDITOR         =${PUMP_CODE_EDITOR}"
      "PUMP_INTERVAL            =${PUMP_INTERVAL}"
      "PUMP_JIRA_ALERT          =${PUMP_JIRA_ALERT}"
      "PUMP_MERGE_TOOL          =${PUMP_MERGE_TOOL}"
      "PUMP_NODE_REQ_SAME_MINOR =${PUMP_NODE_REQ_SAME_MINOR}"
    ...
    local all_project_settings=(
      "PUMP_COMMIT_SIGNOFF      =$(truncate_ ${PUMP_COMMIT_SIGNOFF[$i]} 90)"
      "PUMP_GHA_DEPLOY          =$(truncate_ ${PUMP_GHA_DEPLOY[$i]} 90)"
      "PUMP_JIRA_ALMOST_DONE    =$(truncate_ ${PUMP_JIRA_ALMOST_DONE[$i]} 90)"
      "PUMP_JIRA_API_TOKEN      =$(truncate_ ${PUMP_JIRA_API_TOKEN[$i]} 90)"
      "PUMP_JIRA_BLOCKED        =$(truncate_ ${PUMP_JIRA_BLOCKED[$i]} 90)"
      "PUMP_JIRA_CANCELED       =$(truncate_ ${PUMP_JIRA_CANCELED[$i]} 90)"
      "PUMP_JIRA_DONE           =$(truncate_ ${PUMP_JIRA_DONE[$i]} 90)"
      "PUMP_JIRA_IN_PROGRESS    =$(truncate_ ${PUMP_JIRA_IN_PROGRESS[$i]} 90)"
      "PUMP_JIRA_IN_REVIEW      =$(truncate_ ${PUMP_JIRA_IN_REVIEW[$i]} 90)"
      "PUMP_JIRA_IN_TEST        =$(truncate_ ${PUMP_JIRA_IN_TEST[$i]} 90)"
      "PUMP_JIRA_READY_FOR_TEST =$(truncate_ ${PUMP_JIRA_READY_FOR_TEST[$i]} 90)"
      "PUMP_JIRA_PROJECT        =$(truncate_ ${PUMP_JIRA_PROJECT[$i]} 90)"
      "PUMP_JIRA_STATUSES       =$(truncate_ ${PUMP_JIRA_STATUSES[$i]} 90)"
      "PUMP_JIRA_TODO           =$(truncate_ ${PUMP_JIRA_TODO[$i]} 90)"
      "PUMP_JIRA_WORK_TYPES     =$(truncate_ ${PUMP_JIRA_WORK_TYPES[$i]} 90)"
      "PUMP_NVM_USE_V           =$(truncate_ ${PUMP_NVM_USE_V[$i]} 90)"
      "PUMP_PR_APPEND           =$(truncate_ ${PUMP_PR_APPEND[$i]} 90)"
      "PUMP_PR_APPROVAL_MIN     =$(truncate_ ${PUMP_PR_APPROVAL_MIN[$i]} 90)"
      "PUMP_PR_REPLACE          =$(truncate_ ${PUMP_PR_REPLACE[$i]} 90)"
      "PUMP_SCRIPT_FOLDER       =$(truncate_ ${PUMP_SCRIPT_FOLDER[$i]} 90)"
      "PUMP_SETUP               =$(truncate_ ${PUMP_SETUP[$i]} 90)"
    )
  ...
  if (( pro_is_r )); then
    ...
    for proj_arg in "${proj_args[@]}"; do
      ...
      confirm_ "remove project: ${blue_cor}$proj_arg${reset_cor}?"
      local _RET=$?
      if (( _RET == 130 || _RET == 2 )); then return 130; fi
      if (( _RET == 1 )); then continue; fi 

      confirm_ "also remove project temp and revs folders (backup folders are not removed)?"
      _RET=$?
      if (( _RET == 130 || _RET == 2 )); then return 130; fi
      if (( _RET == 0 )); then
        remove_proj_folders_ -rct "${PUMP_SHORT_NAME[$i]}" "${PUMP_FOLDER[$i]}"
      fi
  ...
  if [[ "$proj_arg" == "$CURRENT_PUMP_SHORT_NAME" ]]; then
    if [[ "$PWD/" != "$OLDPWD/"* && "$OLDPWD/" != "$PWD/"* ]]; then
      local jira_key="$(read_pump_value_ "JIRA_KEY")"
      local jira_title="$(read_pump_value_ "JIRA_TITLE")"

      # if [[ -z "$jira_key" ]]; then jira_key="$(git log -1 --pretty=%h 2>/dev/null)"; fi
      # if [[ -z "$jira_title" ]]; then jira_title="$(git log -1 --pretty=%s 2>/dev/null)"; fi
...
function pro_pwd_() {
  ...
  local pkg_name="$(get_pkg_name_ -o "$PWD" "" 1 2>/dev/null)"
...
function proj_handler() {
  ...
  local sub_cmds=("bkp" "clone" "gha" "jira" "prs" "pull" "rel" "rels" "rev" "revs" "run" "setup" "tag" "tags" "exec")
  ...
  if [[ " ${sub_cmds[*]} " != *" $1 "* ]]; then
    if (( proj_handler_is_h )); then
      ...
      print "  ${hi_yellow_cor}${proj_cmd} revs${reset_cor} : same as ${hi_yellow_cor}${proj_cmd} rev -e${reset_cor}"
      print "  --"
...
function commit() {
  ...
  if (( commit_is_h )); then
    ...
    print "  ${hi_yellow_cor}commit -e${reset_cor} : --amend"
    print "  ${hi_yellow_cor}commit -c${reset_cor} : create message using conventional commits standard"
    print "  ${hi_yellow_cor}commit -j${reset_cor} : create message using jira if available"
    ...
    print "  ${hi_yellow_cor}commit -q${reset_cor} : --quiet"
  ...
  if (( commit_is_a )); then
    ...
    if (( ! is_amend )) || [[ -z "$message" ]]; then
      if ! get_branch_status_ -i "$folder" &>/dev/null; then
        if (( ! is_quiet )); then
  ...
  local jira_key=""
  local jira_title="$message"

  if (( ! is_amend || commit_is_j )); then
    if [[ -n "$message" ]]; then
      jira_key="$(extract_jira_key_ "$message")"
      if [[ -n "$jira_key" ]]; then
        jira_title="$(echo "$message" | sed -E "s/^\s*\(?$jira_key\)?[:\-\s]*//" | sed -E 's/^[[:space:]]+|[[:space:]]+$//')"
      fi
    fi
    ...
    if [[ -z "$jira_key" ]]; then
      ...
      if (( commit_is_j )) && [[ -z "$jira_title" ]]; then
    ...
    if (( commit_is_j )) && [[ -z "$jira_key" || -z "$jira_title" ]]; then
      local commit_key="" commit_title=""
      local output=""
      output="$(read_commits_ -t "$target_branch" "$my_branch" "$folder")"
      if (( $? == 0 )); then
  ...
  if [[ -n "$jira_key" ]]; then
    ...
    if [[ -z "$message" ]] && (( commit_is_j )); then
      message="$jira_title"
    fi

    local temp_message="${pump_pr_title_format//\<jira_key\>/$jira_key}"
    ...
    if (( commit_is_f )); then
      if [[ -z "$message" ]]; then
        if (( is_amend )); then
          ...
          message="Update $(get_my_branch_ "$folder" 2>/dev/null)"
        fi
      fi

      message="${temp_message//\<jira_title\>/$message}"
    else
      temp_message="${temp_message//\<jira_title\>/$message}"

      if [[ -n "$type_commit" ]]; then
        message="$(input_type_mandatory_ -k "" "commit message" 255 "${type_commit}: $temp_message")"
      else
        message="$(input_type_mandatory_ -k "" "commit message" 255 "$temp_message")"
      fi
      if (( $? == 130 )); then return 130; fi
    fi
    ...
    if (( ! commit_is_f )); then
      ...
      if [[ -n "$type_commit" ]]; then
        message="$(input_type_mandatory_ -k "" "commit message" 255 "${type_commit}: ")"
      else
        message="$(input_type_mandatory_ -k "" "commit message" 255)"
      fi
      if (( $? == 130 )); then return 130; fi
    fi
  ...
  if (( is_amend )); then
    if [[ -n "$message" ]]; then
      if [[ -n "$details" ]]; then
        git -C "$folder" commit --no-verify -m "$message" -m "$details" "${flags[@]}" "$@"
      else
        git -C "$folder" commit --no-verify -m "$message" "${flags[@]}" "$@"
      fi
    else
      git -C "$folder" commit --no-verify "${flags[@]}" "$@"
    fi
  else
    git -C "$folder" commit --no-verify -m "$message" -m "$details" "${flags[@]}" "$@"
  fi
...
function recommit() {
  ...
  if (( recommit_is_h )); then
    ...
    print "  --"
    print "  ${hi_yellow_cor}recommit -i${reset_cor} : only recommit staged changes"
    print "  ${hi_yellow_cor}recommit -c${reset_cor} : create message using conventional commits standard"
    print "  ${hi_yellow_cor}recommit -j${reset_cor} : create message using jira if available"
    print "  ${hi_yellow_cor}recommit -n${reset_cor} : prompt for a bigger commit message"
    print "  ${hi_yellow_cor}recommit -q${reset_cor} : --quiet"
    return 0;
  fi
  ...
  local arg_count=0
  local output=""
  output="$(parse_args_ "$proj_cmd gha" "message:to,folder:fz" "$@")"
  if (( $? != 0 )); then return 1; fi
  IFS=$TAB read -r message folder arg_count <<< "$output"
  ...
  if [[ -z "$last_message" ]]; then
    print " no previous commit, cannot recommit" >&2
    return 1;
  fi

  if ! commit -eq "${flags[@]}" "$message" "$folder" "$@"; then return 1; fi
...
function help() {
  ...
  print ""
  print "  type ${hi_yellow_cor}-h${reset_cor} to see more usage, example: ${hi_yellow_cor}pro -h${reset_cor}"
  print ""
...
function validate_proj_cmd_strict_() {
  ...
  local invalid_values=("pwd" "quit" "done" "path" "bkp" "clone" "gha" "jira" "prs" "pull" "rel" "rels" "rev" "revs" "run" "setup" "tag" "tags" "exec")
...
function get_folders_() {
  ...
  if (( get_folders_is_p )); then
    ...
    local dir=""
    for dir in "${dirs[@]}"; do
      if [[ -z "$(extract_jira_key_ "$dir")" && " ${exclude_folders[@]} " != *" ${dir:t} "* && " ${exclude_folders[@]} " != *" $dir "* && " ${filtered_folders[@]} " != *" $dir "* && " ${filtered_folders[@]} " != *" ${dir:t} "* ]]; then
        if [[ -z "$(ls -- "$dir")" ]]; then
          continue;
        fi
        # if is_folder_git_ "$dir" &>/dev/null; then
        ...
        if (( get_folders_is_i )); then
          filtered_folders+=("$(get_folders_i_ "$dir" 2>/dev/null)")
        else
          if (( get_folders_is_f )); then
            filtered_folders+=("$dir")
          else
...
function get_folders_i_() {
  ...
  local pump_jira_title="$(read_pump_value_ "JIRA_TITLE" "$dir")"
  ...
  if [[ -n "$pump_jira_title" ]]; then
    ...
    # pump_jira_title="$(echo "$pump_jira_title" | sed -E 's/^-+//; s/^[[:space:]]+//; s/[[:space:]]+$//; s/^(.{80}).*/\1/' 2>/dev/null)"
    pump_jira_title="$(trim_ $pump_jira_title)"
    pump_jira_title="$(truncate_ "$pump_jira_title" 80)"

    local spaces="$( [[ "$(basename -- "$folder")" == "release" ]] && echo "10s" || echo "14s" )"
...
function kill() {
  ...
  local arg_count=0
  local output=""
  output="$(parse_args_ "$0" "port:n" "$@")"
  if (( $? != 0 )); then return 1; fi
  IFS=$TAB read -r port arg_count <<< "$output"
...
function hg() {
  ...
  if (( hg_is_h )); then
    print "  ${hi_yellow_cor}hg [<search>]${reset_cor} : history | grep text"
    return 0;
  fi
