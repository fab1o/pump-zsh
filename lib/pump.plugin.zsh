...
function is_branch_name_valid() {
  local typed_value="$1"
  local proj_folder="$2"

  git -C "$proj_folder" check-ref-format --branch "$typed_value"
}
...
function input_branch_name_() {
  ...
  while true; do
    ...
    if [[ -n "$typed_value" ]] && is_branch_name_valid "$typed_value" "$git_proj_folder" &>/dev/null; then
      echo "$typed_value"
      return 0;
    fi
  done
...
function check_proj_() {
  ...
  if (( check_proj_is_d )); then
    if [[ -z "${PUMP_JIRA_DOMAIN[$i]}" ]]; then
        local typed_domain=""
        typed_domain=$(input_text_ -o "type your project domain url" "" 99 "" 2>/dev/tty)
        local RET=$?

        if (( RET == 130 || RET == 2 )); then return 130; fi
        if [[ -z "$typed_domain" ]]; then return 1; fi

        update_config_ $i "PUMP_JIRA_DOMAIN" "$typed_domain" &>/dev/null
    fi
  fi
}
...
function remove_proj_() {
  ...
  if (( remove_proj_is_u )); then
    ...
    update_config_ $i "PUMP_JIRA_DOMAIN" "" &>/dev/null
...
function set_current_proj_() {
  ...
  CURRENT_PUMP_JIRA_DOMAIN="${PUMP_JIRA_DOMAIN[$i]}"
...
function print_current_proj_() {
  ...
  if (( i > 0 )); then
    ...
    print " [${hi_magenta_cor}PUMP_PUMP_JIRA_DOMAIN_$i=${reset_cor}${hi_gray_cor}${PUMP_JIRA_DOMAIN[$i]}${reset_cor}]"
    ...
    print " [${hi_magenta_cor}PUMP_JIRA_PULL_SUMMARY$i=${reset_cor}${hi_gray_cor}${PUMP_JIRA_PULL_SUMMARY[$i]}${reset_cor}]"
  ...
  print " [${hi_magenta_cor}CURRENT_PUMP_JIRA_DOMAIN=${reset_cor}${hi_gray_cor}${CURRENT_PUMP_JIRA_DOMAIN}${reset_cor}]"
...
function get_remote_origin_() {
  ...
  local ref
  for ref in refs/remotes/origin/{main,master,stage,staging,prod,production,release,dev,develop,trunk,mainline,default,stable}; do
...
function determine_target_branch_() {
  ...
  if ! is_folder_git_ "$git_proj_folder" &>/dev/null; then return 1; fi

  local is_pwd=0;
  
  if [[ "$git_proj_folder" == "$proj_folder" ]]; then
    is_pwd=1
  else
    local proj_pwd=$(find_proj_by_folder_ "$proj_folder" 2>/dev/null)
  ...
  if (( determine_target_branch_is_d )); then
    default_branch=$(get_default_branch_ "$git_proj_folder" 2>/dev/null)
  fi
  
  if (( determine_target_branch_is_b )); then
    base_branch=$(find_base_branch_ "$git_proj_folder" 2>/dev/null)
  fi

  if (( determine_target_branch_is_t )); then
    target_branch=$(get_base_branch_ "$git_proj_folder" 2>/dev/null)
  fi

  if (( determine_target_branch_is_m )); then
    main_branch=$(get_main_branch_ "$git_proj_folder" 2>/dev/null)
  fi
  ...
  if (( is_pwd )); then
    ...
    if (( determine_target_branch_is_y )); then
      my_branch=$(get_my_branch_ "$git_proj_folder" 2>/dev/null)
    fi
  fi

  local branches=("${(@f)$(printf "%s\n" "$default_branch" "$base_branch" "$my_branch" "$target_branch" "$main_branch" | sort -u)}")

  if (( ${#branches[@]} == 0 )); then return 1; fi

  local remote_name=$(get_remote_origin_ "$git_proj_folder")

  branches+=("${(@f)$(git -C "$git_proj_folder" branch --all --list "*release*" -i --no-column --format="%(refname:short)" \
    | sed "s#^$remote_name/##" \
    | grep -v 'detached' \
    | grep -v 'HEAD' \
    | sort -fu
  ...
  if [[ "$selected_branch" == "<enter manually>" ]]; then
    ...
    selected_branch=$(input_branch_name_ "type the target branch" "${default_branch:-main}" "$git_proj_folder")
  fi
...
function get_remote_branch_() {
  ...
  # local ref=$(git -C "$git_folder" rev-parse --abbrev-ref --symbolic-full-name @{u} 2>/dev/null)

  local remote_name=$(get_remote_origin_ "$git_folder")

  local ref=""
  if git -C "$git_folder" show-ref -q --verify refs/remotes/$remote_name/$branch; then
...
function get_main_branch_() {
  ...
  local ref=""
  for ref in refs/remotes/origin/{main,master,trunk,mainline,default,stable}; do
    if git -C "$git_folder" show-ref -q --verify $ref; then
...
function find_base_branch_() {
  ...
  for base in "${candidate_bases[@]}"; do
    ...
    local found=0
    for ref in refs/{heads,remotes}/{origin,upstream}/$base; do
      if git -C "$git_folder" show-ref -q --verify $ref; then
        found=1
        break;
      fi
...
function covc_() {
  ...
  if [[ -n "$branch_arg" ]]; then
    if ! is_branch_name_valid "$branch_arg" "$PWD" &>/dev/null; then
...
function read_commits_() {
  ...
  if (( read_commits_is_t )); then
    ...
    print -r -- "${pr_title_jira_key}\x1f${pr_title_rest}"
    return 0;
  fi
...
function pr() {
  ...
  target_branch=$(get_remote_branch_ -f "$target_branch" "$folder")

  if [[ -z "$target_branch" ]]; then
    print " fatal: could not determine target branch for pull request" >&2
    return 1;
  fi

  local pr_commit_msgs=("${(@f)$(read_commits_ -c "$my_branch" "$target_branch" "$folder")}")
  if [[ -z "$pr_commit_msgs" ]]; then return 1; fi

  local pr_field=(${(s:\x1f:)$(read_commits_ -t "$my_branch" "$target_branch" "$folder")})
...
function run() {
  ...
  if [[ -n "$proj_arg" ]]; then
    if (( ! single_mode )); then
      ...
      folder=$(echo "$folder" | awk -F'\t' '{print $1}')
...
function proj_rev_() {
  ...
  # proj_rev_ -b select branch
    ...
    if [[ -z "$branch" ]]; then
      local pr
      pr=(${(s:\x1f:)$(select_pr_ "$branch_arg" "$proj_repo" "pull request to review")})
      if (( $? == 130 )); then return 130; fi
  ...
  if is_folder_git_ "$full_rev_folder" &>/dev/null; then
    ...
    if (( ! skip_setup )); then
      if [[ -n "$(git -C "$full_rev_folder" status --porcelain 2>/dev/null)" ]]; then
        skip_setup=1
        st -sb "$full_rev_folder"
        
        confirm_ "changes detected in local branch, erase changes and reset branch?" "reset" "do nothing"
        local RET=$?
        if (( RET == 130 || RET == 2 )); then return 130; fi
        if (( RET == 0 )); then
          if reseta -o "$full_rev_folder" --quiet &>/dev/null; then
...
function proj_clone_() {
  ...
  if [[ -n "$branch_arg" && "$branch_arg" == "$target_branch_arg" ]]; then
    print " fatal: branch cannot be the same as target branch: $branch_arg" >&2
  ...
  if (( single_mode )); then
    ...
    if [[ -n "$git_proj_folder" ]]; then
      ...
      if [[ -z "$work_type" ]]; then
        work_type=$(choose_work_type_)
        if (( $? == 130 )); then return 130; fi
      fi

      if [[ -n "$work_type" && "$work_type" != "none" ]]; then
  ...
  if [[ -n "$branch_arg" ]]; then
    ...
    if [[ -n "$jira_key" ]]; then
      branch_arg=$(get_branch_with_monogram_ "$branch_arg" "$work_type")
    fi
  fi
  ...
  if (( ! skip_clone )); then
    if (( is_switch_ok )) && [[ -n "$branch_arg" && "$branch_arg" != "main" && "$branch_arg" != "master" ]]; then
      ...
      if [[ -z "$target_branch" ]]; then
        if command -v gh &>/dev/null && [[ -n "$proj_repo" ]]; then
          target_branch=$(gh pr view $my_branch --json baseRefName --repo "$proj_repo" --jq '.baseRefName' 2>/dev/null)
        fi

        if [[ -z "$target_branch" ]]; then
          if (( proj_clone_is_j )); then
            target_branch=$(determine_target_branch_ -dbe "$branch_arg" "$proj_cmd" "$proj_folder")
          else
            target_branch=$(determine_target_branch_ -dbey "$branch_arg" "$proj_cmd" "$proj_folder")
          fi
          if [[ -z "$target_branch" ]]; then return 1; fi
        fi
      fi
  ...
  local target_branch=$(get_base_branch_ -f "$folder_to_clone")
  if [[ -z "$target_branch" ]]; then
    target_branch=$(get_base_branch_ "$folder_to_clone")
  fi
...
function proj_prs_() {
  ...
  if (( proj_prs_is_r )); then
    proj_prs_r_ "$proj_repo" "$proj_folder" "$proj_cmd" "$@"
...
function proj_prs_r_() {
  ...
  echo "$pr_list" | jq -c '.' | while read -r pr; do
    ...
    if [[ -z "$pr_commits" ]]; then
      print " ${red_cor}pr aborted:${reset_cor} $pr_desc" >&2
      continue;
    fi
    ...
    if [[ "$pr_is_draft" == "true" ]]; then
      ...
      if (( RET == 130 || RET == 2 )); then return 130; fi
      if (( RET == 1 )); then
        print " ${yellow_cor}pr skipped:${reset_cor} $pr_desc" >&2
        continue;
      fi
    fi
    ...
    gum spin --title="cleaning... " -- git -C "$folder" reset --quiet --hard $remote_name/$pr_branch &>/dev/null
    ...
    if (( proj_prs_r_is_x )); then
      ...
      if git -C "$folder" push $remote_name $pr_branch --force --quiet &>/dev/null; then
        if (( is_merge_commit )); then
          print " ${cyan_cor}pr  merged:${reset_cor} $pr_desc" >&2
        else
          print " ${cyan_cor}pr rebased:${reset_cor} $pr_desc" >&2
        fi
      else
        print " ${red_cor}pr aborted:${reset_cor} $pr_desc" >&2
      fi
...
function approve_pr_() {
  ...
  local from_branch=$(gh pr view $my_branch --json headRefName --repo "$proj_repo" --jq '.headRefName' 2>/dev/null)
...
function select_pr_() {
  ...
  print -r -- "${select_pr_choice}\x1f${select_pr_branch}\x1f${select_pr_title}"
}
...
function proj_dbkp_() {
  ...
  local dirs
  dirs=("${(@f)$(get_folders_ -ijp $i "$backups_folder" 2>/dev/null)}")
  if (( $? == 130 )); then return 130; fi
...
function proj_release_() {
  ...
  if [[ -z "$tag" ]]; then
    ...
    if [[ -z "$tag" ]]; then
      ...
      pkg_tag=$(get_from_pkg_json_ "version" "$proj_folder" 2>/dev/null)
...
function proj_jira_find_folder_() {
  ...
  local folders=($(get_maybe_jira_tickets_ $i 0 "$proj_folder" "$jira_key" 2>/dev/null))
  
  folders+=($(get_maybe_jira_tickets_ $i 0 "${proj_folder}/feature" "$jira_key" 2>/dev/null))
  folders+=($(get_maybe_jira_tickets_ $i 0 "${proj_folder}/bugfix" "$jira_key" 2>/dev/null))
  folders+=($(get_maybe_jira_tickets_ $i 0 "${proj_folder}/release" "$jira_key" 2>/dev/null))
  ...
  local dirs=("other...")
  local jira_folder=""
  for jira_folder in "${folders[@]}"; do
    local key=$(extract_jira_key_ "$jira_folder")
    if [[ -n "$key" ]]; then
      dirs+=("$jira_folder")
    fi
  done
...
function proj_jira_() {
  ...
  if (( proj_jira_is_v_v )); then
    ...
    local maybe_jira_keys=($(get_maybe_jira_tickets_ $i "$single_mode" "$proj_folder" 2>/dev/null))
    ...
    for maybe_jira_key in "${maybe_jira_keys[@]}"; do
      ...
      if [[ -n "$key" ]]; then
        ...
        if (( ! single_mode )) && [[ "$maybe_jira_key" == "release" || "$maybe_jira_key" == "feature" || "$maybe_jira_key" == "bugfix" ]]; then
          proj_jira_ -vv "$proj_cmd" "$jira_proj_or_key" "$jira_status" "${proj_folder}/${maybe_jira_key}"
        fi
      fi
    done
  ...
  if (( proj_jira_is_c || proj_jira_is_p || proj_jira_is_r || proj_jira_is_s || proj_jira_is_v )); then
    ...
    if [[ -z "$jira_key" ]] || (( proj_jira_is_c )); then
      if (( single_mode )); then
        local branch_found=""
        branch_found=$(proj_jira_find_branch_ "$jira_key" "$proj_folder" "$proj_jira_is_c")
        if (( $? == 130 || $? == 2 )); then return 130; fi
        ...
        folder_to_jira=$(proj_jira_find_folder_ $i "$jira_key" "$proj_folder" "$proj_jira_is_c")
        if (( $? == 130 || $? == 2 )); then return 130; fi
  ...
  local jira_domain="${PUMP_JIRA_DOMAIN[$i]}"

  if [[ -n "$jira_domain" ]]; then
    local jira_link="https://${jira_domain}.atlassian.net/browse/${jira_key}"
    local jira_key_link=$'\e]8;;'"$jira_link"$'\a'"$jira_key"$'\e]8;;\a'

    print " opening jira ticket... ${pink_cor}$jira_key_link${reset_cor}"
  else
    print " opening jira ticket... ${pink_cor}$jira_key${reset_cor}"
  fi
  ...
  if (( single_mode )); then
    if ! is_folder_git_ "$proj_folder" &>/dev/null; then
      if ! proj_clone_ -j "$proj_cmd" "$jira_key"; then
        return 1;
      else
        proj_folder="${PUMP_FOLDER[$i]}"
      fi
    ...
    if [[ -n "$branch_found" ]]; then
      ...
      local final_branch=$(get_branch_with_monogram_ "$jira_key" "$work_type")
  ...
  local jira_status=$(acli jira workitem view "$jira_key" --fields="status" --json | jq -r '.fields.status.name // empty' 2>/dev/null)
...
function choose_work_type_() {
  local work_type
  work_type=$(choose_one_ "type of work" "bugfix" "feature" "release" "none")
  if (( $? == 130 || $? == 2 )); then return 130; fi
...
function get_jira_status_() {
  ...
  gum spin --title="retrieving jira status... $jira_key" -- acli jira workitem view "$jira_key" --json | jq -r --arg sep "\x1f" '
    [
      .fields.status.name,
...
function update_jira_status_() {
  ...
  local stat=(${(s:\x1f:)$(get_jira_status_ "$jira_key" 2>/dev/null)})
  if [[ -z "$stat" ]] then
    print " fatal: cannot retrieve jira ticket status for: $jira_key" >&2
    print " run ${hi_yellow_cor}acli jira auth login --web${reset_cor} to make sure you are connected" >&2
    return 1;
  fi
  
  local current_status="${stat[1]}"
  local current_jira_assignee="${stat[2]}"
  local assignee="${stat[3]}"
  local summary="${stat[4]}"
  ...
  if [[ -z "$current_user" ]]; then
    print " fatal: cannot retrieve current user" >&2
    print " run ${hi_yellow_cor}acli jira auth login --web${reset_cor} to make sure you are connected" >&2
  ...
  if [[ -n "$current_jira_assignee" && "$current_jira_assignee" != "$current_user" && "$current_status" == "$jira_done" ]]; then
    ...
    print " fatal: cannot transition a closed ticket assigned to $current_jira_assignee" >&2
    return 1;
  fi
  ...
  if echo "$output" | grep -qE "Failure" && ! echo "$output" | grep -qE "Field Story Points is required"; then
    ...
    if [[ -n "$jira_status" ]] && ; then
      ...
      if echo "$output" | grep -qE "Failure"; then
        print " fatal: cannot transition ticket: $jira_key" >&2
        print " $output" | grep -w "$jira_key" >&2
        return 1;
      fi
    fi
...
function get_branch_with_monogram_() {
  ...
  if [[ -n "$work_type" && "$work_type" != "none" ]]; then
    final_branch_name="${work_type}/${final_branch_name}"
  fi
...
function renb() {
  ...
  if ! is_branch_name_valid "$new_name" "$folder" &>/dev/null; then
...
function fetch() {
  ...
  if [[ -n "$branch_arg" ]]; then
    if ! is_branch_name_valid "$branch_arg" "$folder" &>/dev/null; then
      print " fatal: invalid branch argument: $branch_arg" >&2
      print " run ${hi_yellow_cor}fetch -h${reset_cor} to see usage" >&2
      return 1;
    fi
...
function co() {
  ...
  if (( co_is_p && co_is_r )); then
    ...
    local pr
    pr=(${(s:\x1f:)$(select_pr_ "$branch_arg" "$proj_repo" "pull request to detach")})
    if (( $? == 130 )); then return 130; fi
    ...
    if [[ -z "$pr" || -z "$pr_number" ]]; then
      ...
      print " fatal: failed to fetch pull requests on: $repo_name" >&2
      return 0;
    fi
  ...
  if (( co_is_b || co_is_c )); then
    if [[ -n "$base_branch" ]]; then
      if ! is_branch_name_valid "$base_branch" "$folder" &>/dev/null; then
  ...
  # co branch_arg or co (no arguments) look all branches
  if [[ -z "$base_branch" ]] && (( ! co_is_e )); then
    ...
    if [[ -n "$branch_arg" ]]; then
      ...
      branch_choice=$(select_branch_ -aic "$branch_arg" "$short_branch_arg to switch" "$folder")
    else
...
function rebase() {
  ...
  if (( rebase_is_h )); then
    print "  ${hi_yellow_cor}rebase ${yellow_cor}[<base_branch>] [<folder>]${reset_cor} : apply the commits from your branch on top of base branch"
    print "  --"
  ...
  if [[ -n "$base_branch" ]]; then
    ...
    local found_base_branch=$(select_branch_ -ix "$base_branch" "base branch" "$folder" 2>/dev/null)
    if [[ -z "$found_base_branch" ]]; then
      base_branch=$(select_branch_ -i "$base_branch" "base branch" "$folder" 2>/dev/null)
      if [[ -z "$base_branch" ]]; then return 1; fi
    fi
  fi
...
function merge() {
  ...
  if (( merge_is_w )); then
    ...
    for branch in "${selected_branches[@]}"; do
      if git -C "$folder" switch "$branch" --quiet; then
        if (( merge_is_s )); then
          merge -spa "$strategy" "$base_branch" "$folder" "$@"
          RET=$?
          if (( RET == 130 )); then return 130; fi
        else
          merge -pa "$base_branch" "$folder" "$@"
          RET=$?
          if (( RET == 130 )); then return 130; fi
        fi
      else
...
function get_maybe_jira_tickets_() {
  ...
  if (( single_mode )); then
    ...
    maybe_jira_keys=($(git -C "$proj_folder" branch --list --format="%(refname:short)" \
        | grep -v 'detached' \
        | grep -v 'HEAD' \
        | sort -fu
      ))
    ...
    maybe_jira_keys=("${(@f)$(get_folders_ -p $i "$proj_folder" "$jira_key" 2>/dev/null)}")
  fi

  print -r -- "${maybe_jira_keys[@]}"
}
...
function pro_c_act_() {
  ...
  local maybe_jira_keys=($(get_maybe_jira_tickets_ $i "$single_mode" "$proj_folder"))
  ...
  for maybe_jira_key in "${maybe_jira_keys[@]}"; do
    ...
    if [[ -n "$jira_key" ]]; then
      local stat=(${(s:\x1f:)$(get_jira_status_ "$jira_key" 2>/dev/null)})
      if [[ -z "$stat" ]] then return 1; fi

      local jira_status="${stat[1]}"

      if [[ "$jira_status" == "$jira_done" || "$jira_status" == "Canceled" ]]; then
        local folder_to_delete=""

        if (( single_mode )); then
          delb -sei "$maybe_jira_key" "${proj_folder}"
        else
          del -s "${proj_folder}/${maybe_jira_key}"
        fi
      fi
    else
      if (( ! single_mode )) && [[ "$maybe_jira_key" == "release" || "$maybe_jira_key" == "feature" || "$maybe_jira_key" == "bugfix" ]]; then
        pro_c_act_ $i "$jira_done" "$single_mode" "${proj_folder}/${maybe_jira_key}"
      fi
    fi
  done
}
...
function proj_handler() {
  ...
  set +x
  eval "$(parse_flags_exclusive_ "$0" "mefinrudc" "cprvdsmnbjaelx" "$@")"
  ...
  if [[ " ${sub_cmds[*]} " != *" $1 "* ]]; then
    if (( proj_handler_is_h )); then
      ...
      print "  ${hi_yellow_cor}${proj_cmd} jira -vv${reset_cor} : view all ticket statuses"
      print "  --"
      print "  ${hi_yellow_cor}${proj_cmd} prs${reset_cor} : open pull requests in github.com"
      ...
      print "  --"
      print "  ${hi_yellow_cor}${proj_cmd} rel ${yellow_cor}[<version>]${reset_cor} : create release version"
      print "  ${hi_yellow_cor}${proj_cmd} rel -d ${yellow_cor}[<version>]${reset_cor} : delete release versions"
      print "  ${hi_yellow_cor}${proj_cmd} rel -m${reset_cor} : create major release version"
      print "  ${hi_yellow_cor}${proj_cmd} rel -n${reset_cor} : create minor release version"
      print "  ${hi_yellow_cor}${proj_cmd} rel -p${reset_cor} : create patch release version"
      ...
      print "  ${hi_yellow_cor}${proj_cmd} revs${reset_cor} : open existing code review"
      print "  ${hi_yellow_cor}${proj_cmd} revs -l${reset_cor} : display existing code review"
      print "  --"
  ...
  if (( proj_handler_is_f )); then
    ...
    local dirs
    dirs=("${(@f)$(get_folders_ -ijp $i "$proj_folder" "$folder" 2>/dev/null)}")
    if (( $? == 130 )); then return 130; fi
...
function proj_handler_open_() {
  ...
  if (( ! is_proj_folder )); then
    ...
    if [[ -n "$dirs" ]]; then
      local folder;
      # folder has to be declared first so that RET can be captured
      folder=$(choose_one_ -i "folder" "${dirs[@]}")
      local RET=$?
      ...
      if [[ -n "$folder" ]]; then
        ...
        proj_handler_open_ $i "$folder_to_open/$folder"
        return $?;
      fi
    fi
  fi
...
function get_folders_() {
  ...
  if (( get_folders_is_j )) && [[ -z "$jira_pull_summary" ]] && command -v acli &>/dev/null; then
    confirm_ "attempt to pull folder descriptions from jira tickets?"
    local RET=$?
    if (( RET == 130 || RET == 2 )); then return 130; fi
    if (( RET == 0 )); then
  ...
  if (( get_folders_is_p )); then
    local priorities=(dev develop main master production stage staging trunk mainline default stable bugfix feature release)
    ...
    for name in "${folders[@]}"; do
      if [[ ! " ${priorities[@]} " == *" $name "* && ! " ${exclude_folders[@]} " == *" $name "* ]]; then
        if (( get_folders_is_i )); then
          name=$(get_folders_i_ "$jira_pull_summary" "$folder" "$name" 2>/dev/null)
        fi
        filtered_folders+=("$name");
      fi
    done
...
function get_folders_i_() {
  ...
  local full_folder="${folder}/${name}"
  local name_display="$name"
  local proj_desc=""

  if [[ -f "${full_folder}/.jira" ]]; then
    proj_desc=$(<"${full_folder}/.jira")
  fi

  local jira_key=$(extract_jira_key_ "$name")

  if (( jira_pull_summary )) && [[ -z "$proj_desc" ]]; then
    if [[ -n "$jira_key" ]]; then
      if command -v acli &>/dev/null; then
        proj_desc=$(acli jira workitem view "$jira_key" --json | jq -r '.fields.summary' 2>/dev/null)
      fi
      # save in file even if could not get value because we dont want to do this all the time
      echo "$proj_desc" > "${full_folder}/.jira"
    fi
  fi
  
  if [[ -z "$proj_desc" ]]; then
    proj_desc=$(git -C "$full_folder" log -1 --pretty=%s 2>/dev/null)
  fi

  if [[ -n "$proj_desc" ]]; then
    if [[ -n "$jira_key" ]]; then
      proj_desc="${proj_desc//$jira_key/}"
    fi
    proj_desc="$(echo "$proj_desc" | sed -E 's/^-+//; s/^[[:space:]]+//; s/[[:space:]]+$//; s/^(.{99}).*/\1/' 2>/dev/null)"
    name_display="${name}\t${proj_desc}";
  fi
...
typeset -g CURRENT_PUMP_JIRA_DOMAIN=""
