...
function determine_target_branch_() {
  ...
  local branches=("${(@f)$(printf "%s\n" "$default_branch" "$base_branch" "$my_branch" | sort -ru)}")
...
function get_short_name_() {
  ...
  local short_branch="${branch#refs/remotes/origin/}"
  
  short_branch="${branch#refs/remotes/upstream/}"
  short_branch="${branch#refs/remotes/$remote_name/}"
      ...
      if [[ -z "$base_branch" || "$short_my_branch" == "$short_base_branch" ]]; then
        ...
        if [[ -z "$base_branch" || "$short_my_branch" == "$short_base_branch" ]]; then
          ...
          if [[ -z "$base_branch" || "$short_my_branch" == "$short_base_branch" ]]; then
            ...
            if [[ -z "$base_branch" || "$short_my_branch" == "$short_base_branch" ]]; then
              base_branch=$(git -C "$git_folder" symbolic-ref refs/remotes/upstream/HEAD 2>/dev/null)
              short_base_branch=$(get_short_name_ "$base_branch" "$git_folder")
            fi
          fi
        fi
...
function get_default_branch_() {
  ...
  if [[ -z "$default_branch" ]]; then
    ...
    if [[ -z "$default_branch" ]]; then
      default_branch=$(git -C "$git_folder" symbolic-ref refs/remotes/upstream/HEAD 2>/dev/null)
    fi
  ...
  if (( ! skip_clone )); then
    ...
    if [[ -z "$default_branch" ]]; then
      if (( proj_clone_is_j )); then
        default_branch=$(determine_target_branch_ -db "$branch_arg" "$proj_cmd" "$proj_folder")
      else
        default_branch=$(determine_target_branch_ -dbm "$branch_arg" "$proj_cmd" "$proj_folder")
      fi
      if (( $? == 130 )); then return 130; fi
...
function rebase() {
  ...
  local short_my_branch=$(get_short_name_ "$ref" "$folder")
  local short_base_branch=$(get_short_name_ "$ref" "$folder")
...
function validate_proj_cmd_strict_() {
  ...
  local invalid_values=("pwd" "quit" "done" "path" "bkp" "clone" "gha" "jira" "prs" "release" "releases" "rev" "revs" "tag" "tags")
