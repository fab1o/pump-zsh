...
function parse_flags__() {
  ...
  for arg in "$@"; do
    ...
    for (( i=0; i < ${#letters}; i++ )); do
      ...
      #   flags+=("-${number}")
      #   if (( is_double && i == 1 )) && { [[ "$valid_flags_pass_along" == "all" ]] || [[ $valid_flags_pass_along == *${letters:0:1}* && $valid_flags_pass_along == *${letters:1:1}* ]] }; then
...
function parse_arg_() {
  ...
  case "$type" in
    ...
    e)
      # env
      if [[ -z "$value" ]] || [[ "$value" != "dev" && "$value" != "stage" && "$value" != "qa" && "$value" != "prod" ]]; then
        is_valid=0
        if [[ -z "$value" ]]; then
          value="$default_value"
        else
...
function get_pump_jira_title_() {
  ...
  if [[ -z "$folder" ]]; then
    folder="$(find_git_folder_ 2>/dev/null)"
...
function validate_proj_cmd_strict_() {
  ...
  local invalid_values=("pwd" "quit" "done" "path" "bkp" "clone" "del" "gha" "jira" "prs" "pull" "rel" "rels" "rev" "revs" "run" "setup" "tag" "tags" "exec" "version")
...
function te() {
  ...
  if (( te_is_h )); then
    print "  ${hi_yellow_cor}te${reset_cor} : run tests configured by PUMP_TEST"
    print "  ---"
    print "  configure PUMP_TEST in your ${hi_gray_cor}$PUMP_CONFIG_FILE${reset_cor} file"
    return 0;
  fi
...
function run() {
  ...
  if (( run_is_h )); then
    if [[ -n "$CURRENT_PUMP_SHORT_NAME" ]]; then
      if [[ -n "$CURRENT_PUMP_RUN" ]]; then
        print "  ${hi_yellow_cor}run [<folder>]${reset_cor} : run ${CURRENT_PUMP_SHORT_NAME}'s PUMP_RUN"
      else
        print "  ${hi_yellow_cor}run [<folder>]${reset_cor} : run ${CURRENT_PUMP_SHORT_NAME}'s dev or start script"
      fi
    else
      print "  ${hi_yellow_cor}run [<folder>]${reset_cor} : run dev or start script"
    fi
    print "  ---"
    if [[ -n "$CURRENT_PUMP_SHORT_NAME" ]]; then
      if [[ -n "$PUMP_RUN_STAGE" ]]; then
        print "  ${hi_yellow_cor}run stage [<folder>]${reset_cor} : run ${CURRENT_PUMP_SHORT_NAME}'s PUMP_RUN_STAGE"
      else
        print "  ${hi_yellow_cor}run stage [<folder>]${reset_cor} : run ${CURRENT_PUMP_SHORT_NAME}'s stage script"
      fi
      if [[ -n "$PUMP_RUN_QA" ]]; then
        print "  ${hi_yellow_cor}run qa [<folder>]${reset_cor} : run ${CURRENT_PUMP_SHORT_NAME}'s PUMP_RUN_QA"
      else
        print "  ${hi_yellow_cor}run qa [<folder>]${reset_cor} : run ${CURRENT_PUMP_SHORT_NAME}'s qa script"
      fi
      if [[ -n "$PUMP_RUN_PROD" ]]; then
        print "  ${hi_yellow_cor}run prod [<folder>]${reset_cor} : run ${CURRENT_PUMP_SHORT_NAME}'s PUMP_RUN_PROD"
      else
        print "  ${hi_yellow_cor}run prod [<folder>]${reset_cor} : run ${CURRENT_PUMP_SHORT_NAME}'s prod script"
      fi
    else
      print "  ${hi_yellow_cor}run <script>${reset_cor} : run any script in ${CURRENT_PUMP_SHORT_NAME}'s package.json"  
    fi
    print "  ---"
    print "  ${hi_yellow_cor}run <script> ${yellow_cor}[<folder>]${reset_cor} : run any script in ${CURRENT_PUMP_SHORT_NAME}'s package.json"
    print "  ---"
    print "  edit the custom run script PUMP_RUN in your config file: ${hi_gray_cor}$PUMP_CONFIG_FILE${reset_cor}"
    return 0;
  fi
...
function proj_run_() {
  ...
  eval "$(parse_args_ "$proj_cmd run" "env:to,folder:to" "$@")"
  shift $arg_count

  local proj_folder="$CURRENT_PUMP_FOLDER"
  local single_mode="$CURRENT_PUMP_SINGLE_MODE"
  local pkg_manager="$CURRENT_PUMP_PKG_MANAGER"
  
  local pump_run=""
  local script="PUMP_RUN"
  ...
  if (( i )); then
    ...
    if [[ -z "$env" ]]; then
      ...
      if [[ "$env" == "stage" ]]; then
        ...
        script="PUMP_RUN_STAGE"
      elif [[ "$env" == "prod" ]]; then
        pump_run="${PUMP_RUN_PROD[$i]}"
        script="PUMP_RUN_PROD"
      elif [[ "$env" == "qa" ]]; then
  ...
  if (( i )); then
    if (( single_mode )); then
      ...
      if [[ -n "$folder" ]]; then
        folder_to_execute="$(proj_handler_folder_ $i "$folder")"
        if [[ -z "$folder_to_execute" ]]; then return 1; fi
      else
  ...
  if command -v realpath &>/dev/null; then
    folder_to_execute="$(realpath -- "$folder_to_execute" 2>/dev/null)"
  else
    folder_to_execute="${folder_to_execute:A}"
  fi
  ...
  if [[ -z "$pump_run" ]]; then
    local pump_run_script="$(get_from_package_json_ "scripts.${script}" "$folder_to_execute" 2>/dev/null)"
...
function setup() {
  ...
  if (( setup_is_h )); then
    if [[ -n "$CURRENT_PUMP_SHORT_NAME" ]]; then
      if [[ -n "$CURRENT_PUMP_SETUP" ]]; then
        print "  ${hi_yellow_cor}setup [<folder>]${reset_cor} : run ${CURRENT_PUMP_SHORT_NAME}'s PUMP_SETUP"
      else
        print "  ${hi_yellow_cor}setup [<folder>]${reset_cor} : run ${CURRENT_PUMP_SHORT_NAME}'s setup script or package manager install"
      fi
    else
      print "  ${hi_yellow_cor}setup [<folder>]${reset_cor} : run setup script or package manager install"
    fi
    print "  ---"
    print "  edit the custom setup script PUMP_SETUP in your config file: ${hi_gray_cor}$PUMP_CONFIG_FILE${reset_cor}"
    return 0;
  fi
...
function proj_setup_() {
  ...
  local single_mode="$CURRENT_PUMP_SINGLE_MODE"
  local pkg_manager="$CURRENT_PUMP_PKG_MANAGER"
  local pump_setup="$CURRENT_PUMP_SETUP"

  local script="PUMP_SETUP"
  ...
  if [[ -z "$pump_setup" || "$pump_setup" == "0" ]]; then
    if (( i )) && [[ -z "$pump_setup" ]]; then
      ...
      if (( _RET == 0 )); then
        ...
        if [[ -n "$custom_setup" ]]; then
          update_config_ $i "PUMP_SETUP" "$custom_setup"
    ...
    if [[ -z "$pump_setup" || "$pump_setup" == "0" ]]; then
      ...
      print " running $pump_setup on ${cyan_cor}${folder_to_execute}${reset_cor}"
    else
      print " running PUMP_SETUP on ${cyan_cor}${folder_to_execute}${reset_cor}"
    fi
...
function proj_prs_() {
  set +x
  eval "$(parse_flags_ "$0" "alus" "xfrqd" "$@")"
...
function proj_prs_l_() {
  ...
  for ticket in "${tickets[@]}"; do
    ...
    if (( proj_prs_l_is_qd )); then
      pr_list="$(list_prs_ -odm "$key" "in:title,body" "$proj_repo" "prs for $key")"
    else
      pr_list="$(list_prs_ -odm "$key" "in:title" "$proj_repo" "prs for $key")"

      if [[ -z "$pr_list" ]]; then
        print " ${yellow_cor}warning:${reset_cor} no open pull requests found for ticket: $key" >&2
        print " run: ${hi_yellow_cor}$proj_cmd prs${reset_cor} with flag ${hi_yellow_cor}-qd${reset_cor} to also search in pr description" >&2
      fi
    fi
        ...
        for label in "${existing_labels[@]}"; do
          ...
          if [[ "$label" =~ ^[0-9]+\.[0-9]+\.[0-9]+$ ]]; then
            ...
            if (( is_remove_all_labels )); then
              ...
              if [[ " ${to_be_removed_labels[*]} " == *" $label "* ]]; then
                ...
                confirm_ "remove ${label}?" "yes" "no" "no"
                _RET=$?
                if (( _RET == 130 )); then return 130; fi;
                if (( _RET == 0 )); then
                  is_to_remove=1
          ...
          if (( is_to_remove )); then
            ...
            if gh pr edit "$pr_number" --repo "$proj_repo" --remove-label "$label" &>/dev/null; then
              ...
              local prs_with_label=$(gh pr list --repo "$proj_repo" --label "$label" --limit 1 2>/dev/null)
    ...
    if (( proj_prs_l_is_debug )); then
      if ! confirm_ "processed $key - continue?"; then
        return 1;
      fi
    fi
...
function proj_jira_() {
  ...
  if (( proj_jira_is_ss )) && (( proj_jira_is_t || proj_jira_is_r || proj_jira_is_a || proj_jira_is_c || proj_jira_is_d || proj_jira_is_e )); then
...
function workflow_run_() {
  ...
  if (( workflow_run_is_t || workflow_run_is_b || workflow_run_is_v )); then
    local values=()
...
function proj_gha_() {
  ...
  if (( proj_gha_is_d && proj_gha_is_a && proj_gha_is_f )); then
    eval "$(parse_args_ "$proj_cmd gha" "format:m,interval:nz:10,search:a:to" "$@")"

  elif (( proj_gha_is_f && proj_gha_is_a )); then
    eval "$(parse_args_ "$proj_cmd gha" "format:m,interval:nz:10" "$@")"

  elif (( proj_gha_is_d && proj_gha_is_a )); then
    eval "$(parse_args_ "$proj_cmd gha" "interval:nz:10,search:a:to" "$@")"

  elif (( proj_gha_is_d && proj_gha_is_f )); then
    ...
    eval "$(parse_args_ "$proj_cmd gha" "workflow:to,interval:nz:10,search:a:to" "$@")"
...
function glog() {
  ...
  if (( glog_is_h )); then
    print "  ${hi_yellow_cor}glog ${yellow_cor}[<search>|<branch>] [<format>] [<folder>]${reset_cor} : see log commits --pretty=<format>"
  ...
  eval "$(parse_args_ "$0" "search:to,format:to,folder:fz" "$@")"
  ...
  if [[ -n "$search" ]]; then
    if is_local_branch_ "$search" "$folder" || is_remote_branch_ "$search" "$folder" || is_commit_ "$search" "$folder"; then
      branch="$search"
      search=""
    fi
  fi
  ...
  if [[ -n "$branch" ]]; then
    ...
    if [[ "$branch" == "$my_short_branch" || "$branch" == "$my_branch" ]]; then
    ...
    if [[ -n "$format" ]]; then
      if [[ -n "$search" ]]; then
        git -C "$folder" --no-pager log $branch..$my_branch --use-mailmap --no-decorate --pretty="$format" "${flags[@]}" --grep="$search" "$@"
      else
        git -C "$folder" --no-pager log $branch..$my_branch --use-mailmap --no-decorate --pretty="$format" "${flags[@]}" "$@"
      fi
    else
      if [[ -n "$search" ]]; then
        git -C "$folder" --no-pager log $branch..$my_branch --use-mailmap --no-decorate "${flags[@]}" --grep="$search" "$@"
      else
        git -C "$folder" --no-pager log $branch..$my_branch --use-mailmap --no-decorate "${flags[@]}" "$@"
      fi
    fi
  ...
  if [[ -n "$format" ]]; then
    if [[ -n "$search" ]]; then
      git -C "$folder" --no-pager log $branch --use-mailmap --no-decorate --pretty="$format" "${flags[@]}" --grep="$search" "$@"
    else
      git -C "$folder" --no-pager log $branch --use-mailmap --no-decorate --pretty="$format" "${flags[@]}" "$@"
    fi
  elif [[ -n "$search" ]]; then
    git -C "$folder" --no-pager log $branch --use-mailmap --no-decorate "${flags[@]}" --grep="$search" "$@"
  else
    git -C "$folder" --no-pager log $branch --use-mailmap --no-decorate "${flags[@]}" "$@"
  fi
}
...
function proj_handler() {
  ...
  if (( single_mode )); then
    sub_cmds=("bkp" "clone" "gha" "jira" "prs" "pull" "rel" "rels" "rev" "revs" "run" "setup" "tag" "tags" "exec" "version")
  else
    sub_cmds=("bkp" "clone" "del" "gha" "jira" "prs" "pull" "rel" "rels" "rev" "revs" "run" "setup" "tag" "tags" "exec" "version")
  fi
  ...
  if [[ " ${sub_cmds[*]} " != *" $1 "* ]]; then
    if (( proj_handler_is_h )); then
      ...
      print "  ${hi_yellow_cor}  -f${yellow_cor} <format>${reset_cor} : format using printf (combine with one of the display options:"
      print "  ${hi_yellow_cor}  -b${reset_cor} : display branch (can combine with -v and -t)"
      print "  ${hi_yellow_cor}  -v${reset_cor} : display version (can combine with -b and -t)"
      print "  ${hi_yellow_cor}  -t${reset_cor} : display tag (can combine with -b and -v)"
      print "  ---"
      ...
      print "  ${hi_yellow_cor}  -ql${reset_cor} : include labels when searching for prs"
      print "  ---"
      if (( single_mode )); then
      ...
      if [[ -n "${PUMP_RUN[$i]}" ]]; then
        if (( single_mode )); then
          print "  ${hi_yellow_cor}${proj_cmd} run${reset_cor} : run ${proj_cmd}'s PUMP_RUN"
        else
          print "  ${hi_yellow_cor}${proj_cmd} run [<folder>]${reset_cor} : run ${proj_cmd}'s PUMP_RUN"
        fi
      else
      ...
      if [[ -n "${PUMP_RUN_STAGE[$i]}" ]]; then
        (( single_mode )) && print "  ${hi_yellow_cor}${proj_cmd} run stage${reset_cor} : run ${proj_cmd}'s PUMP_RUN_STAGE"
        (( ! single_mode )) && print "  ${hi_yellow_cor}${proj_cmd} run stage [<folder>]${reset_cor} : run ${proj_cmd}'s PUMP_RUN_STAGE"
      else
        (( single_mode )) && print "  ${hi_yellow_cor}${proj_cmd} run stage${reset_cor} : run ${proj_cmd}'s stage script"
        (( ! single_mode )) && print "  ${hi_yellow_cor}${proj_cmd} run stage [<folder>]${reset_cor} : run a ${proj_cmd}'s stage script"
      fi
      if [[ -n "${PUMP_RUN_QA[$i]}" ]]; then
        (( single_mode )) && print "  ${hi_yellow_cor}${proj_cmd} run qa${reset_cor} : run ${proj_cmd}'s PUMP_RUN_QA"
        (( ! single_mode )) && print "  ${hi_yellow_cor}${proj_cmd} run qa [<folder>]${reset_cor} : run ${proj_cmd}'s PUMP_RUN_QA"
      else
        (( single_mode )) && print "  ${hi_yellow_cor}${proj_cmd} run qa${reset_cor} : run ${proj_cmd}'s qa script"
        (( ! single_mode )) && print "  ${hi_yellow_cor}${proj_cmd} run qa [<folder>]${reset_cor} : run a ${proj_cmd}'s qa script"
      fi
      if [[ -n "${PUMP_RUN_PROD[$i]}" ]]; then
        (( single_mode )) && print "  ${hi_yellow_cor}${proj_cmd} run prod${reset_cor} : run ${proj_cmd}'s PUMP_RUN_PROD"
        (( ! single_mode )) && print "  ${hi_yellow_cor}${proj_cmd} run prod [<folder>]${reset_cor} : run ${proj_cmd}'s PUMP_RUN_PROD"
      else
        (( single_mode )) && print "  ${hi_yellow_cor}${proj_cmd} run prod${reset_cor} : run ${proj_cmd}'s prod script"
        (( ! single_mode )) && print "  ${hi_yellow_cor}${proj_cmd} run prod [<folder>]${reset_cor} : run a ${proj_cmd}'s prod script"
      fi
      (( single_mode )) && print "  ${hi_yellow_cor}${proj_cmd} run <script>${reset_cor} : run any script in ${proj_cmd}'s package.json"
      (( ! single_mode )) && print "  ${hi_yellow_cor}${proj_cmd} run <script> [<folder>]${reset_cor} : run any script in ${proj_cmd}'s package.json"
      print "  ---"

      if [[ -n "${PUMP_SETUP[$i]}" ]]; then
        (( single_mode )) && print "  ${hi_yellow_cor}${proj_cmd} setup${reset_cor} : run ${proj_cmd}'s PUMP_SETUP"
        (( ! single_mode )) && print "  ${hi_yellow_cor}${proj_cmd} setup [<folder>]${reset_cor} : run ${proj_cmd}'s PUMP_SETUP"
      else
        (( single_mode )) && print "  ${hi_yellow_cor}${proj_cmd} setup${reset_cor} : run ${proj_cmd}'s setup script or package manager install"
        (( ! single_mode )) && print "  ${hi_yellow_cor}${proj_cmd} setup [<folder>]${reset_cor} : run ${proj_cmd}'s setup script or package manager install"
      fi
      print "  ---"
