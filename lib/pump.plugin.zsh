...
function select_branch_() {
  set +x
  eval "$(parse_flags_ "$0" "" "alrixscjoq" "$@")"
...
function delplus() {
  ...
  if (( delplus_is_h )); then
    print "  ${hi_yellow_cor}delplus <folder> <glob>${reset_cor} : delete all files/folders in folder"
...
function pr() {
  ...
  # pr -l
  if (( pr_is_l )); then
    label="$(gh label list --repo "$proj_repo" --limit 2000 --json=name | jq -r '.[].name' 2>/dev/null)"
    all_labels=("${(@f)label}")
  fi
...
function proj_rev_() {
  ...
  if is_folder_git_ "$full_rev_folder" &>/dev/null; then
    ...
    if ! clone_repo_ -bo "$proj_repo" "$full_rev_folder" "cloning... ${green_cor}$branch_user${reset_cor}"; then
...
function proj_prs_l_() {
  ...
  if (( ! proj_prs_l_is_f )); then
    ...
    if (( _RET == 0 )); then
      local all_labels_out="$(gh label list --repo "$proj_repo" --limit 2000 --json=name 2>/dev/null | jq -r '.[].name' | sort -rf)"
  ...
  if (( ! proj_prs_l_is_f )); then
    if (( is_version_label )); then
      ...
      if (( _RET == 0 )); then
        ...
        confirm_ "remove other version labels from prs?" "yes" "no" "no"
        local _RET=$?
  ...
  if [[ -n "${choose_labels[@]}" ]]; then
    ...
    if (( ! is_remove_all_labels && ! is_remove_version_labels )); then return 1; fi
  fi
  ...
  for ticket in "${tickets[@]}"; do
    ...
    # also search for bump version PRs
    ...
    for line in "${(@f)pr_list}"; do
      ...
      if (( is_remove_all_labels || is_remove_version_labels )); then
        local is_removed=0
        ...
        for label in "${existing_labels[@]}"; do
          ...
          if [[ "$label" =~ ^[0-9]+\.[0-9]+\.[0-9]+$ ]] && (( is_remove_version_labels )); then
            is_to_remove=1
          else
            if [[ " ${to_be_removed_labels[*]} " == *" $label "* ]] || (( is_remove_all_labels )); then
              is_to_remove=1
            elif confirm_ "remove ${label}?" "yes" "no" "no"; then
              is_to_remove=1
            elif confirm_ "ignore future removals of ${label}?"; then
    ...
    done # looping for prs

  done # looping for tickets

  if (( ${#prs_labels_ignore[@]} )); then
...
function query_prs_() {
  ...
  if [[ -n "$search_text" ]]; then
    if (( query_prs_is_g )); then
      ...
      if (( query_prs_is_b )); then
        search_text="bump AND \"$search_text\""
      fi
    fi
  fi
