...
function update_config_() {
  ...
  if (( display_disclaimer )); then
    print " ${hi_gray_cor}reset config by running ${PUMP_SHORT_NAME[$i]} -u${reset_cor}"
  fi
...
function get_proj_special_folder_() {
  ...
  if [[ -z "$proj_cmd" ]]; then
    print " ${red_cor}internal error: get_proj_special_folder_ requires proj_cmd${reset_cor}" >&2
    return 1;
  fi
...
function get_proj_index_() {
  ...
  if [[ -z "$proj_cmd" ]]; then
    print " ${red_cor}internal error: get_proj_index_ requires proj_cmd${reset_cor}" >&2
    return 1;
  fi
...
function covc_() {
  ...
  local branch_arg="$1"
  local remote_branch_arg=$(get_remote_branch_ -r "$branch_arg" "$PWD")
  ...
  if (( branch_behind || branch_ahead )); then
    print " ${low_yellow_cor}warning: your branch is behind $branch_arg by $branch_behind commits and ahead by $branch_ahead commits${reset_cor}" >&2
...
function pr() {
  ...
  if [[ -n "$pr_link" ]]; then
    print " fatal: pull request already exists: $pr_link" >&2
    return 1;
  fi
...
function run() {
  ...
  if [[ -n "$proj_arg" ]]; then
    ...
    if [[ "$env_mode" == "stage" ]]; then
      pump_run="${CURRENT_PUMP_RUN_STAGE[$i]}"
    elif [[ "$env_mode" == "prod" ]]; then
      pump_run="${CURRENT_PUMP_RUN_PROD[$i]}"
    fi
  fi
...
function proj_revs_() {
  ...
  if [[ -z "$proj_cmd" ]]; then
    print " ${red_cor}internal error: proj_revs_ requires proj_cmd${reset_cor}" >&2
    return 1;
  fi
...
function proj_rev_() {
  ...
  if [[ -z "$proj_cmd" ]]; then
    print " ${red_cor}internal error: proj_rev_ requires proj_cmd${reset_cor}" >&2
    return 1;
  fi
...
function proj_clone_() {
  ...
  if [[ -z "$proj_cmd" ]]; then
    print " ${red_cor}internal error: proj_clone_ requires proj_cmd${reset_cor}" >&2
    return 1;
  fi
  ...
  print " default branch is: ${green_cor}$(git -C "$folder_to_clone" config --get init.defaultBranch)${reset_cor}"
...
function proj_pr_() {
  ...
  if (( proj_pr_is_h )); then
    eval "$proj_cmd -h | grep --color=never -E '\pr\b'"
    return 0;
  fi

  if ! command -v gh &>/dev/null; then
    print " fatal: $proj_cmd pr requires gh" >&2
    print " install gh: ${blue_cor}https://github.com/cli/cli/${reset_cor}" >&2
    return 1;
  fi

  if [[ -z "$proj_cmd" ]]; then
    print " ${red_cor}internal error: proj_pr_ requires proj_cmd${reset_cor}" >&2
    return 1;
  fi
  ...
  if (( proj_pr_is_a )); then
    if [[ -z "$2" ]]; then
      print " fatal: not a valid argument" >&2
      print " run ${yellow_cor}$proj_cmd pr -h${reset_cor} to see usage" >&2
      return 1;
    fi

    local pr_number_or_branch="$2"

    gh pr review "$pr_number_or_branch" --approve --repo "$proj_repo"
    return $?;
  fi

  if (( proj_pr_is_s )); then
    if ! command -v jq &>/dev/null; then
      print " fatal: $proj_cmd pr -s requires jq" >&2
    ...
    echo $prs | jq -c '.' | while read -r pr; do
      ...
      if [[ -z "$assignees" ]]; then
        if gh pr edit "$pr_number" --add-assignee "$author" --repo "$proj_repo" &>/dev/null; then
          print " ${green_cor}PR #$pr_number${reset_cor} is assigned to ${cyan_cor}$author${reset_cor}"
        else
          print " ${red_cor}PR #$pr_number is not assigned${reset_cor}"
        fi
      else
        print " ${green_cor}PR #$pr_number${reset_cor} is assigned to ${cyan_cor}$assignees${reset_cor}"
      fi
    done
...
function proj_bkp_() {
  ...
  if [[ -z "$proj_cmd" ]]; then
    print " ${red_cor}internal error: proj_bkp_ requires proj_cmd${reset_cor}" >&2
    return 1;
  fi
...
function proj_dbkp_() {
  ...
  if [[ -z "$proj_cmd" ]]; then
    print " ${red_cor}internal error: proj_dbkp_ requires proj_cmd${reset_cor}" >&2
    return 1;
  fi
...
function proj_dtag_() {
  ...
  if [[ -z "$proj_cmd" ]]; then
    print " ${red_cor}internal error: proj_dtag_ requires proj_cmd${reset_cor}" >&2
    return 1;
  fi
...
function proj_tag_() {
  set +x
  eval "$(parse_flags_ "$0" "sdl" "" "$@")"
  (( proj_tag_is_debug )) && set -x
  
  local proj_cmd="$1"

  if (( proj_tag_is_h )); then
    eval "$proj_cmd -h | grep --color=never -E '\btag\b'"
    return 0;
  fi
  ...
  if (( proj_tag_is_l )); then
    proj_tags_ $@
    return $?;
  fi

  if [[ -z "$proj_cmd" ]]; then
    print " ${red_cor}internal error: proj_tag_ requires proj_cmd${reset_cor}" >&2
    return 1;
  fi
...
function proj_tags_() {
  ...
  if [[ -z "$proj_cmd" ]]; then
    print " ${red_cor}internal error: proj_tags_ requires proj_cmd${reset_cor}" >&2
    return 1;
  fi
  ...
  if [[ -n "$2" ]]; then
    if [[ $2 == <-> ]]; then
      ...
      print " fatal: not a valid argument: $2" >&2
      print " run ${yellow_cor}$proj_cmd tag -h${reset_cor} to see usage" >&2
      return 1;
    fi
  fi
...
function proj_drelease_() {
  ...
  if [[ -z "$proj_cmd" ]]; then
    print " ${red_cor}internal error: proj_drelease_ requires proj_cmd${reset_cor}" >&2
    return 1;
  fi
...
function proj_release_() {
  ...
  if (( proj_release_is_l )); then
    proj_releases_ $@
    return $?;
  fi

  if [[ -z "$proj_cmd" ]]; then
    print " ${red_cor}internal error: proj_release_ requires proj_cmd${reset_cor}" >&2
    return 1;
  fi
...
function proj_releases_() {
  ...
  if [[ -z "$proj_cmd" ]]; then
    print " ${red_cor}internal error: proj_releases_ requires proj_cmd${reset_cor}" >&2
    return 1;
  fi
  ...
  if [[ -n "$2" ]]; then
    if [[ $2 == <-> ]]; then
      ...
      print " fatal: not a valid argument: $2" >&2
      print " run ${yellow_cor}$proj_cmd release -h${reset_cor} to see usage" >&2
      return 1;
    fi
  fi
...
function proj_jira_() {
  ...
  if ! command -v gum &>/dev/null; then
    print " fatal: $proj_cmd jira requires gum" >&2
    print " install gum: ${blue_cor}https://github.com/charmbracelet/gum/${reset_cor}" >&2
    return 1;
  fi

  if [[ -z "$proj_cmd" ]]; then
    print " ${red_cor}internal error: proj_jira_ requires proj_cmd${reset_cor}" >&2
    return 1;
  fi
  ...
  if (( proj_jira_is_c )); then
    ...
    if (( proj_jira_is_c )); then
      if (( single_mode )); then
        ...
        delb -e "$branch_arg" "$resolved_folder"
      else
        del "$resolved_folder"
      fi
      if (( $? == 130 )); then return 130; fi
  ...
  if (( proj_jira_is_p )); then
    update_jira_status_ -p $i "$current_user" "$current_jira_assignee" "$proj_cmd" "$jira_key"
    return $?;
  elif (( proj_jira_is_r )); then
    update_jira_status_ -r $i "$current_user" "$current_jira_assignee" "$proj_cmd" "$jira_key"
    return $?;
  elif (( proj_jira_is_c )); then
    update_jira_status_ -c $i "$current_user" "$current_jira_assignee" "$proj_cmd" "$jira_key"
    return $?;
  elif (( proj_jira_is_v )); then
    update_jira_status_ -v $i "$current_user" "$current_jira_assignee" "$proj_cmd" "$jira_key"
    return $?;
  elif (( proj_jira_is_s )); then
    update_jira_status_ -s $i "$current_user" "$current_jira_assignee" "$proj_cmd" "$jira_key" "$jira_status"
    return $?;
  fi
  ...
  if (( single_mode )); then
    ...
    if ! is_folder_git_ "$git_proj_folder" &>/dev/null; then
...
function update_jira_status_() {
  set +x
  eval "$(parse_flags_ "$0" "csrp" "" "$@")"
  (( update_jira_status_is_debug )) && set -x

  local i="$1"
  local current_user="$2"
  local current_jira_assignee="$3"
  local proj_cmd="$4"
  local jira_key="$5"
  local jira_status="$6"
  ...
  local current_jira_status=$(gum spin --title="retrieving current jira status..." -- \
      acli jira workitem view "$jira_key" --fields="status" --json | jq -r '.fields.status.name')
  ...
  if (( update_jira_status_is_p )); then
    ...
    if [[ -z "$jira_status" ]]; then
      ...
      if [[ -z "$jira_status" ]]; then
        jira_status=$(input_from_ "enter jira status (e.g. In Progress, In Review, Done)" "$current_jira_status" 20)
      fi
  ...
  if echo "$output" | grep -qE "Failure"; then
    ...
    if [[ -n "$jira_status" ]] && ; then
      gum spin --title="transitioning jira ticket..." -- \
          acli jira workitem transition --key="$jira_key" --status="$jira_status" --yes
      RET=$?
      if (( update_jira_status_is_p )); then
...
function select_jira_key_() {
  ...
  if command -v gum &>/dev/null; then
    ...
    print " retrieving jira projects..." >&2
...
function proj_gha_() {
  ...
  if [[ -z "$proj_cmd" ]]; then
    print " ${red_cor}internal error: proj_gha_ requires proj_cmd${reset_cor}" >&2
    return 1;
  fi
...
function get_pkg_name_() {
  ...
  local folder=$(get_proj_for_pkg_ "$proj_folder" 2>/dev/null)
  if [[ -n "$folder" ]]; then
...
function proj_handler() {
  ...
  set +x
  eval "$(parse_flags_exclusive_ "$0" "miue" "cprvdsmnbjael" "$@")"
  ...
  local sub_cmds=("bkp" "clone" "gha" "jira" "pr" "release" "rev" "revs" "tag")
  ...
  if [[ " ${sub_cmds[*]} " != *" $1 "* ]]; then
    if (( proj_handler_is_h )); then
      ...
      print "  ${yellow_cor}${proj_cmd} jira -s ${low_yellow_cor}[<jira_key>] [<status>]${reset_cor} : move a ticket status"
      print "  ${yellow_cor}${proj_cmd} jira -v ${low_yellow_cor}[<jira_key>]${reset_cor} : view a ticket status"
      print "  --"
      print "  ${yellow_cor}${proj_cmd} pr${reset_cor} : list pull requests in github"
      print "  ${yellow_cor}${proj_cmd} pr -a <branch_or_pr>${reset_cor} : approve a pull request"
      print "  ${yellow_cor}${proj_cmd} pr -s${reset_cor} : set the assignee for all pull requests"
      print "  --"
      print "  ${yellow_cor}${proj_cmd} release ${low_yellow_cor}[<version>]${reset_cor} : create a release"
      print "  ${yellow_cor}${proj_cmd} release -d ${low_yellow_cor}[<version>]${reset_cor} : delete a release"
      print "  ${yellow_cor}${proj_cmd} release -l${reset_cor} : list releases"
      ...
      print "  ${yellow_cor}${proj_cmd} release -s${reset_cor} : create a release and skip confirmation"
      print "  --"
      print "  ${yellow_cor}${proj_cmd} rev ${low_yellow_cor}[<branch>]${reset_cor} : create a code review"
      ...
      print "  --"
      print "  ${yellow_cor}${proj_cmd} tag ${low_yellow_cor}[<name>]${reset_cor} : create a tag"
      print "  ${yellow_cor}${proj_cmd} tag -d ${low_yellow_cor}[<name>]${reset_cor} : delete a tag"
      print "  ${yellow_cor}${proj_cmd} tag -l ${low_yellow_cor}[<n>]${reset_cor} : list n number of tags"
      print "  ${yellow_cor}${proj_cmd} tag -s${reset_cor} : create a tag and skip confirmation"
...
function proj_handler_open_() {
  ...
  if [[ -z "$proj_cmd" ]]; then
    print " ${red_cor}internal error: proj_handler_open_ requires proj_cmd${reset_cor}" >&2
    return 1;
  fi
