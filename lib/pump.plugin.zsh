...
function update_setting_() {
  ...
  if (( $? != 0 )); then
    ...
    print " ${hi_gray_cor}updated setting ${gray_cor}${key}_$i=${value}${reset_cor}"
  fi
  if (( display_disclaimer )); then
    print " ${gray_cor}reset settings by running ${PUMP_SHORT_NAME[$i]} -u${reset_cor}"
  fi
...
function check_proj_() {
  ...
  if (( check_proj_is_c )); then
    if ! check_proj_cmd_ -x $i "${PUMP_SHORT_NAME[$i]}" ${@:2}; then return 1; fi
  fi

  if (( check_proj_is_r )); then
    if (( check_proj_is_v )); then
      if ! check_proj_repo_ -xsev $i "${PUMP_REPO[$i]}" "${PUMP_FOLDER[$i]}" "${PUMP_SHORT_NAME[$i]}" ${@:2}; then return 1; fi
    else
      if ! check_proj_repo_ -xse $i "${PUMP_REPO[$i]}" "${PUMP_FOLDER[$i]}" "${PUMP_SHORT_NAME[$i]}" ${@:2}; then return 1; fi
    fi
  ...
  if (( check_proj_is_f )); then
    if ! check_proj_folder_ -xs $i "${PUMP_FOLDER[$i]}" "${PUMP_SHORT_NAME[$i]}" "${PUMP_REPO[$i]}" ${@:2}; then return 1; fi
  ...
  if (( check_proj_is_p )); then
    if ! check_proj_pkg_manager_ -xq $i "${PUMP_PKG_MANAGER[$i]}" "${PUMP_FOLDER[$i]}" "${PUMP_REPO[$i]}" ${@:2}; then return 1; fi
...
function get_proj_special_folder_() {
  ...
  if (( single_mode )); then
    ...
    echo "$parent_folder/$category/$folder_name"
  else
    echo "$proj_folder/$category"
  fi
}
...
function save_proj_folder_() {
  ...
  if (( save_proj_folder_is_a || save_proj_folder_is_r )); then
    ...
    if (( save_proj_folder_is_a )); then
      if find_proj_by_folder_ "$(pwd)" &>/dev/null; then
        # ask to use pwd
  ...
  if [[ -z "$TEMP_PUMP_FOLDER" ]]; then
    ...
    if (( save_proj_folder_is_x )); then
      print "  ${hi_gray_cor}project folder: ${TEMP_PUMP_FOLDER}${reset_cor}" >&1
    else
...
function save_proj_repo_() {
  ...
  if [[ -z "$TEMP_PUMP_REPO" ]]; then
    ...
    clear_last_line_1_
    if (( save_proj_repo_is_x )); then
      print "  ${hi_gray_cor}project repository: ${TEMP_PUMP_REPO}${reset_cor}" >&1
    else
...
function save_pkg_manager_() {
  ...
  if (( save_proj_cmd_is_x )); then
    print "  ${hi_gray_cor}project manager: ${pkg_manager}${reset_cor}" >&1
  else
...
function save_proj_() {
  ...
  if (( save_proj_is_e )); then
    ...
    if ! save_proj_mode_ -e $i "${PUMP_FOLDER[$i]}" "${PUMP_SINGLE_MODE[$i]}"; then return 1; fi
  else
    ...
    if ! save_proj_mode_ -a $i "${PUMP_FOLDER[$i]}" "${PUMP_SINGLE_MODE[$i]}"; then return 1; fi
  fi
  ...
  if [[ -n "$old_single_mode" ]] && (( old_single_mode != ${PUMP_SINGLE_MODE[$i]} )); then
    ...
    if [[ -n "$git_proj_folder" || -n "$pkg_proj_folder" ]]; then
      if create_backup_ -s "${PUMP_FOLDER[$i]}" "${PUMP_SINGLE_MODE[$i]}"; then
...
function remove_proj_() {
  ...
  if (( remove_proj_is_r )); then
    ...
    local revs_folder="$(get_proj_special_folder_ -r "$proj_folder" "$single_mode")"
    local cov_folder="$(get_proj_special_folder_ -c "$proj_folder" "$single_mode")"
...
function get_node_versions_() {
  ...
  local installed_versions=($(nvm ls --no-colors | grep -E '^[-> ]+\s+v[0-9]+\.[0-9]+\.[0-9]+' | sed 's/^[-> ]*//' | sed 's/^v//' | sed 's/ *\*$//' | sort -V))
...
function find_proj_by_folder_() {
  ...
  local parent_folder_name="$(basename -- "$(dirname -- "$folder")")"
  if [[ "$parent_folder_name" == ".backups" ]]; then
    return 1;
  fi
  ...
  if (( ! find_proj_by_folder_is_k )); then
    ...
    folder_name="${folder_name%-cov}"

    local name=$(find_proj_by_folder_ -k "${parent_folder}/${folder_name}")
...
function covc_() {
  ...
  if [[ -z "$proj_repo" ]]; then
    print " PUMP_REPO_$i is missing for $CURRENT_PUMP_SHORT_NAME" >&2
    return 1;
  fi

  if [[ -z "$CURRENT_PUMP_COV" ]]; then
    print " PUMP_COV_$i is missing for $CURRENT_PUMP_SHORT_NAME" >&2
  ...
  local coverage_folder_single_mode="${parent_folder}/.${folder_name}-cov"
  local coverage_folder_multiple_mode="${CURRENT_PUMP_FOLDER}/.cov"
...
function reset1() {
  ...
  git -C "$folder" --no-pager log --oneline --pretty=format:'%s' -1
...
function reset2() {
  ...
  git -C "$folder" --no-pager log --oneline --pretty=format:'%s' -2
...
function reset3() {
  ...
  git -C "$folder" --no-pager log --oneline --pretty=format:'%s' -3
...
function reset4() {
  ...
  git -C "$folder" --no-pager log --oneline --pretty=format:'%s' -4
...
function reset5() {
  ...
  git -C "$folder" --no-pager log --oneline --pretty=format:'%s' -5
...
function pr() {
  ...
  if [[ -n "$test_script" ]]; then
    if (( ! pr_is_t && ! pr_is_s )) && [[ -z "$CURRENT_PUMP_PR_RUN_TEST" ]]; then
      ...
      if (( RET == 130 || RET == 2 )); then return 130; fi
      if (( RET == 0 )); then
        update_setting_ $i "PUMP_PR_RUN_TEST" 1 $updated_settings
      else
        update_setting_ $i "PUMP_PR_RUN_TEST" 0 $updated_settings
      fi
    fi
...
function run() {
  ...
  if (( run_is_h )); then
    ...
    print "  --"
    if [[ -n "$CURRENT_PUMP_SHORT_NAME" ]]; then
      print "  ${yellow_cor}run <folder>${reset_cor} : run a ${CURRENT_PUMP_SHORT_NAME}'s folder on dev environment"
      print "  ${yellow_cor}run <folder> ${low_yellow_cor}[<env>]${reset_cor} : run a ${CURRENT_PUMP_SHORT_NAME}'s folder on given environment"
      print "  --"
    fi
    print "  ${yellow_cor}run <pro> <folder>${reset_cor} : run a project's folder on dev environment"
    print "  ${yellow_cor}run <pro> <folder> ${low_yellow_cor}[<env>]${reset_cor} : run a project's folder on a given environment"
    return 0;
  fi
  ...
  if [[ -n "$folder_arg" ]]; then
    ...
    if [[ "$proj_arg" != "$CURRENT_PUMP_SHORT_NAME" ]] || ! is_folder_pkg_ "$PWD" &>/dev/null; then
      if (( single_mode )); then
        ...
        local dirs=("${(@f)$(get_folders_ -p "$proj_folder")}")
        if [[ -n "$dirs" ]]; then
...
function setup() {
  ...
  if (( setup_is_h )); then
      print "  ${yellow_cor}setup${reset_cor} : setup current folder"
      if [[ -n "$CURRENT_PUMP_SHORT_NAME" ]]; then
        print "  ${yellow_cor}setup <folder>${reset_cor} : setup a folder for $CURRENT_PUMP_SHORT_NAME"
      fi
      print "  --"
    print "  ${yellow_cor}setup <pro>${reset_cor} : setup a project"
    print "  ${yellow_cor}setup <pro> ${low_yellow_cor}[<folder>]${reset_cor} : setup a project's folder"
    return 0;
  fi
  ...
  print "  • ${yellow_cor}${proj_arg} -h${reset_cor} to see more options"
  print "  • ${yellow_cor}help${reset_cor} for more help"
}

function proj_revs_() {
  set +x
  eval "$(parse_flags_ "$0" "ra" "" "$@")"
  ...
  # proj_revs_ -r proj_revs_ -a
  if (( proj_revs_is_r && proj_revs_is_a )); then
...
function proj_rev_() {
  ...
  if (( ! skip_setup )); then
    setup "$proj_cmd" "$full_rev_folder"
...
function proj_clone_() {
  ...
  if (( single_mode )); then
    ...
    if [[ -n "$(ls -A -- "$proj_folder" 2>/dev/null)" ]]; then
      if (( ! proj_clone_is_r )); then
        ...
        if (( RET == 130 || RET == 2 )); then return 130; fi
        if (( RET == 1 )); then
          print " cannot clone $proj_cmd because is set to ${purple_cor}single mode${reset_cor}" >&2
  ...
  if (( ! skip_clone )); then
    if (( create_backup )); then
      if create_backup_ -s "$proj_folder" "$single_mode"; then
        print " re-cloning project: ${green_cor}${proj_cmd}${reset_cor}"
      else
        return 1;
      fi
    fi
  ...
  if [[ "$default_branch" != "$branch_arg" ]]; then
    ...
    print " ${magenta_cor}git config branch.${branch_arg}.gh-merge-base${reset_cor}"
  ...
  print ""
  print " successfully cloned branch: ${green_cor}${branch_arg}${reset_cor}"
...
function get_branch_with_monogram_() {
  ...
  if [[ -z "$pump_no_monogram" ]]; then
    ...
    update_setting_ $i "PUMP_NO_MONOGRAM" $RET >&2
...
function fetch() {
  set +x
  eval "$(parse_flags_ "$0" "to" "pqn" "$@")"
  (( fetch_is_debug )) && set -x

  if (( fetch_is_h )); then
    print "  ${yellow_cor}fetch ${low_yellow_cor}[folder]${reset_cor} : fetch and prune from origin all branches and tags"
    print "  ${yellow_cor}fetch -t${reset_cor} : force fetch"
    print "  ${yellow_cor}fetch -to${reset_cor} : fetch all tags only"
    return 0;
  fi
...
function push() {
  set +x
  eval "$(parse_flags_ "$0" "tfuv" "nqs" "$@")"
  (( push_is_debug )) && set -x

  if (( push_is_h )); then
    print "  ${yellow_cor}push ${low_yellow_cor}[<branch>] [<folder>]${reset_cor} : push --no-verify"
  ...
  if (( RET == 0 )); then
    ...
    git -C "$folder" push $remote_name $branch_arg ${flags[@]} --set-upstream $@
  fi
  RET=$?
  ...
  if (( RET != 0 && is_quiet == 0 )); then
    print ""
    if (( push_is_f )); then
      if confirm_ "push failed, try push --force?"; then
        pushf -f "$branch_arg" "$folder" ${flags[@]} $@
        return $?;
      fi
    else
...
function pushf() {
  set +x
  eval "$(parse_flags_ "$0" "tfv" "qns" "$@")"
  (( pushf_is_debug )) && set -x

  if (( pushf_is_h )); then
    print "  ${yellow_cor}pushf ${low_yellow_cor}[<branch>] [<folder>]${reset_cor} : push --no-verify --force-with-lease"
...
function dtag() {
  ...
  if (( dtag_is_h )); then
    print "  ${yellow_cor}dtag ${low_yellow_cor}[<pro>] [<name>]${reset_cor} : delete a project's tag"
...
function proj_bkp_() {
  ...
  if (( single_mode )); then
    create_backup_ "$proj_folder" "$single_mode"
  else
      ...
      create_backup_ "${proj_folder}/${folder}" "$single_mode"
  fi
}
...
function proj_dbkp_() {
  ...
  if (( proj_dbkp_is_h )); then
    eval "$proj_arg -h | grep --color=never -E '\bbkp -d\b'"
    return 0;
  fi
  ...
  local i=$(get_proj_index_ "$proj_arg")
  ...
  local backups_folder="$(get_proj_special_folder_ -b "$proj_folder" "$single_mode")"
  ...
  if [[ -z "$(get_folders_ "$backups_folder")" ]]; then
    rm -rf -- "$backups_folder"
  fi

  local parent_folder="$(dirname -- "$backups_folder")"
  if [[ -z "$(get_folders_ "$parent_folder")" ]]; then
    rm -rf -- "$backups_folder"
  fi
...
function create_backup_() {
  set +x
  eval "$(parse_flags_ "$0" "dsg" "" "$@")"
  (( create_backup_is_debug )) && set -x

  local proj_folder="$1"
  local single_mode="${2:-0}"

  if [[ ! -d "$proj_folder" ]]; then
    print " fatal: not a valid folder argument for backup: $proj_folder" >&2
    return 1;
  fi
  ...
  if [[ -d "$proj_folder/.git" ]]; then
    mkdir -p -- "$proj_backup_folder/.git_backup"
  fi
  ...
  if (( create_backup_is_s )); then
    local realfolder="${proj_folder:A}"
    if [[ "$PWD" == "$realfolder" ]]; then
      cd "$parent_folder"
    fi
  fi
  ...
  if command -v gum &>/dev/null; then
    gum spin --title="cleaning ${folder_name}..." -- \
      find "$proj_folder" -type d -name "node_modules" -prune -exec rm -rf '{}' +

    if (( create_backup_is_s && create_backup_is_g )); then
      gum spin --title="cleaning .git folder in ${folder_name}..." -- \
        find "$proj_folder" -type d -name ".git" -prune -exec rm -rf '{}' +
    
    elif (( create_backup_is_g )) && [[ -d "$proj_folder/.git" ]]; then
      gum spin --title="creating backup for .git..." -- \
        rsync -a --remove-source-files -- "$proj_folder/.git/" "$proj_backup_folder/.git_backup/"
      rm -rf -- "$proj_folder/.git"
    fi

    if (( create_backup_is_s )); then
      gum spin --title="creating backup and deleting folder ${folder_name}..." -- \
        rsync -a --remove-source-files -- "$proj_folder/" "$proj_backup_folder/"
    else
    ...
    if (( create_backup_is_s && create_backup_is_g )); then
      print " cleaning .git folder in ${folder_name}..."
      find "$proj_folder" -type d -name ".git" -prune -exec rm -rf '{}' +
      clear_last_line_1_
    elif (( create_backup_is_g )) && [[ -d "$proj_folder/.git" ]]; then
      print "creating backup for .git..."
      rsync -a --remove-source-files -- "$proj_folder/.git/" "$proj_backup_folder/.git_backup/"
      rm -rf -- "$proj_folder/.git"
      clear_last_line_1_
    fi
...
function proj_release_() {
  ...
  if [[ -z "$my_branch" ]]; then
    ...
    print " make sure you are on a branch before creating a release" >&2
    return 1;
  fi
  ...
  if [[ -z "$tag" ]]; then
    ...
    if [[ -z "$tag" ]]; then
      ...
      print " make sure version has format: <major>.<minor>.<patch>"
      return 1;
    fi
  else
...
function tag() {
  ...
  if (( tag_is_h )); then
    print "  ${yellow_cor}tag ${low_yellow_cor}[<pro>] [<name>]${reset_cor} : create a new tag for a project"
...
function tags() {
  ...
  if (( tags_is_h )); then
    print "  ${yellow_cor}tags ${low_yellow_cor}[<pro>] [<n>]${reset_cor} : list tags of a project"
...
function delb() {
  ...
  if (( delb_is_h )); then
    print "  ${yellow_cor}delb${reset_cor} : delete a branch locally in current folder"
    print "  ${yellow_cor}delb ${low_yellow_cor}[<branch>] [<folder>]${reset_cor} : delete a branch locally in folder"
...
function get_pkg_name_() {
  ...
  if [[ -z "$proj_repo" ]]; then
    ...
    if [[ -n "$folder" ]]; then
      proj_repo=$(get_repo_ "$folder" 2>/dev/null)
    fi
  fi
...
function pro() {
  ...
  if (( pro_is_h )); then
    print "  ${yellow_cor}pro [<project>]${reset_cor} : set a project"
    print "  ${yellow_cor}pro -l${reset_cor} : list all projects"
    print "  ${yellow_cor}pro -a ${low_yellow_cor}[<project>]${reset_cor} : add a new project"
    print "  ${yellow_cor}pro -e [<project>]${reset_cor} : edit a project"
    print "  ${yellow_cor}pro -r [<project>]${reset_cor} : remove a project"
    print "  --"
    print "  ${yellow_cor}pro -i ${low_yellow_cor}[<project>]${reset_cor} : display project settings"
    print "  ${yellow_cor}pro -n ${low_yellow_cor}[<project>]${reset_cor} : set the project node version using nvm"
    print "  ${yellow_cor}pro -u ${low_yellow_cor}[<project>] [<setting>]${reset_cor} : reset project settings"
    return 0;
  fi
  ...
  if (( pro_is_n )); then
    ...
    if (( skip_lookup == 0 )); then
      ...
      setopt NO_NOTIFY
      {
        gum spin --title="detecting node..." -- bash -c 'sleep 3'
      } 2>/dev/tty

      # gum spin --title="detecting node engine..." -- sleep 2 2>/dev/tty &!
      ...
      if [[ -n "$nvm_use_v" ]]; then
        ...
        update_setting_ $i "PUMP_NVM_USE_V" "$nvm_use_v" 0
      ...
      if [[ -n "$nvm_use_v" ]] && (( ! pro_is_x )); then
        ...
        print " ${yellow_cor}warning: could not find \"engines.node\" to detect node version in package.json${reset_cor}"
...
function proj_handler() {
  ...
  set +x
  eval "$(parse_flags_exclusive_ "$0" "meiu" "cprvdsmnbja" "$@")"
  (( proj_handler_is_debug )) && set -x

  if ! check_proj_ -cmq $i; then return 1; fi
  ...
  if [[ " ${sub_cmds[*]} " != *" $1 "* ]]; then
    if (( proj_handler_is_h )); then
      ...
      print "  ${yellow_cor}${proj_cmd} revs -r${reset_cor} : remove code reviews"
      print "  ${yellow_cor}${proj_cmd} revs -ra${reset_cor} : remove all code reviews"
    ...
    if (( proj_handler_is_m )); then
      ...
      if (( ! single_mode )); then
        folder=$(get_default_folder_ "$proj_folder" 2>/dev/null)
      fi
  ...
  if (( single_mode )); then
    ...
    proj_handler_open_ "$proj_cmd" "$proj_folder" "$branch"
    return $?;
  fi
  ...
  local dirs=("${(@f)$(get_folders_ -p "$proj_folder" "${folder:-$1}")}")
  if [[ -n "$dirs" ]]; then
...
function help() {
  ...
  if [[ -n "$CURRENT_PUMP_SHORT_NAME" ]]; then
    ...
    print "  ${red_cor}no project is set${reset_cor}" >&2
  fi
  print ""
...
function get_folders_() {
  set +x
  eval "$(parse_flags_ "$0" "poOntsf" "" "$@")"
  (( get_folders_is_debug )) && set -x

  # if (( get_folders_is_h )); then
  #   print "  ${yellow_cor}get_folders_ ${low_yellow_cor}[<folder>]${reset_cor} : list folders"
  #   print "  ${yellow_cor}get_folders_ -p${reset_cor} : order by priority: special folders first"
  #   print "  ${yellow_cor}get_folders_ -on${reset_cor} : order by name"
  #   print "  ${yellow_cor}get_folders_ -On${reset_cor} : order by name (reverse order)"
  #   print "  ${yellow_cor}get_folders_ -ot${reset_cor} : order by time"
  #   print "  ${yellow_cor}get_folders_ -Ot${reset_cor} : order by time (reverse order)"
  #   return 0;
  # fi
