...
function refix() {
  ...
  if (( refix_is_h )); then
    print "  ${yellow_cor}refix ${low_yellow_cor}[<folder>]${reset_cor} : run fix lint and format on last commit"
    print "  --" >&1
  ...
  if is_folder_git_ "$full_rev_folder" &>/dev/null; then
    ...
    if [[ -n "$pump_clone" ]]; then
      ...
      if ! eval "$pump_clone"; then
        ...
        print " edit config: $PUMP_CONFIG_FILE then run ${yellow_cor}refresh${reset_cor}"
      fi
    fi
  fi
...
function glog() {
  ...
  if (( glog_is_h )); then
    ...
    print "  ${yellow_cor}glog -t${reset_cor} : display comment details such as timestamp and author"
...
function co() {
  ...
  if (( co_is_h )); then
    print "  ${yellow_cor}co ${low_yellow_cor}[<branch>] [<folder>]${reset_cor} : switch to branch"
    print "  ${yellow_cor}co <branch> <base_branch>${reset_cor} : create new branch off of base branch"
    print "  --" >&1
    print "  ${yellow_cor}co -l ${low_yellow_cor}[<branch>]${reset_cor} : switch to local branch"
    print "  ${yellow_cor}co -pr ${low_yellow_cor}[<pr>]${reset_cor} : switch to pull request (detached branch)"
    print "  --" >&1
    print "  ${yellow_cor}co -e <branch_or_commit>${reset_cor} : switch to an exact branch or commit, no lookup"
    print "  ${yellow_cor}co -c <branch> ${low_yellow_cor}[<base_branch>]${reset_cor} : create new branch off of base branch, no lookup with base_branch"
  ...
  if (( co_is_p && co_is_r )); then
    if [[ -n "$2" && $2 != -* ]]; then
      ...
      print " run ${yellow_cor}co -h${reset_cor} to see usage"
      return 1;
    fi
  ...
  if (( co_is_e )); then
    ...
    if [[ -n "$2" && $2 != -* ]]; then
      print " fatal: co -e does not accept a second argument" >&2
      print " run ${yellow_cor}co -h${reset_cor} to see usage"
      return 1;
    fi
...
function proj_handler() {
  ...
  if [[ " ${sub_cmds[*]} " != *" $1 "* ]]; then
    if (( proj_handler_is_h )); then
      ...
      print "  ${yellow_cor}${proj_cmd} prs -aa ${low_yellow_cor}[<search_term>]${reset_cor} : approve pull requests every $_interval min"
      ...
      print "  ${yellow_cor}${proj_cmd} rev -e ${low_yellow_cor}[<rev_folder>]${reset_cor} : open existing code review"
      print "  ${yellow_cor}${proj_cmd} rev -d${reset_cor} : delete code reviews"
      print "  ${yellow_cor}${proj_cmd} rev -dd${reset_cor} : delete all code reviews"
      print "  ${yellow_cor}${proj_cmd} revs${reset_cor} : display code reviews"
      print "  --" >&1
