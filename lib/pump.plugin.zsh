...
function update_proj_repo_() {
  ...
  if (( single_mode )); then
    ...
    dirs=("${(@f)$(get_folders_ "$proj_folder")}")
  fi
...
function save_proj_repo_() {
  ...
  if (( save_proj_repo_is_q )); then
    if update_config_ $i "PUMP_REPO" "$proj_repo" &>/dev/null; then
      update_proj_repo_ "$proj_repo" "$proj_folder" "${PUMP_SINGLE_MODE[$i]}"
    fi
...
function save_proj_f_() {
  ...
  if update_config_ $i "PUMP_REPO" "$TEMP_PUMP_REPO" &>/dev/null; then
    update_proj_repo_ "$TEMP_PUMP_REPO" "$PWD" 1 &>/dev/null
  fi
...
function save_proj_() {
  ...
  if update_config_ $i "PUMP_REPO" "$TEMP_PUMP_REPO" &>/dev/null; then
    update_proj_repo_ "$TEMP_PUMP_REPO" "$TEMP_PUMP_FOLDER" "$TEMP_SINGLE_MODE" &>/dev/null
  fi
  update_config_ $i "PUMP_PKG_MANAGER" "$TEMP_PUMP_PKG_MANAGER" &>/dev/null
  update_config_ $i "PUMP_SHORT_NAME" "$TEMP_PUMP_SHORT_NAME" &>/dev/null
...
function pr() {
  ...
  local pr_field=(${(s:|:)$(read_commits_ -t "$my_branch" "$target_branch" "$folder")})
...
function proj_rev_() {
  ...
  if is_folder_git_ "$full_rev_folder" &>/dev/null; then
    ...
    if command -v gum &>/dev/null; then
      gum spin --title="cloning... $repo_name" -- rm -rf -- "$full_rev_folder"
...
function proj_prs_() {
  ...
  if (( proj_prs_is_a )); then
    ...
    local prs=("${(@f)$(select_prs_ "$search_term" "$proj_repo" "prs to approve")}")
    if (( $? == 130 )); then return 130; fi
    ...
    for pr in "${prs[@]}"; do
      ...
      approve_pr_ -s "$pr_number" "$pr_title" "$proj_repo" "$pr_approval_min" "$current_user" "$pr_approval_min"
      local RET=$?
...
function proj_prs_r_() {
  ...
  echo $prs | jq -c '.' | while read -r pr; do
    ...
    if (( is_merge_commit )); then
      ...
      if merge -pa "$pr_base_branch" "$folder" --quiet &>/dev/null; then
        _merged+=("$pr_desc")
      else
      ...
      if rebase -pa "$pr_base_branch" "$folder" --quiet &>/dev/null; then
        _rebased+=("$pr_desc")
      else
...
function proj_prs_aa_() {
  ...
  local pr_list=()
  if command -v gum &>/dev/null; then
    pr_list=("${(@f)$(gum spin --title="fetching prs... $repo_name" -- gh pr list --repo "$proj_repo" --json number,title --jq '.[] | "\(.number)\t\(.title)"' | grep -i "$search_term"  2>/dev/null)}")
  else
  ...
  # Declare an associative array to track authorized PRs
  typeset -A authorized_prs
  ...
  for pr in "${pr_list[@]}"; do
    ...
    # skip if this PR has already been authorized or unauthorized
    if [[ -n "${authorized_prs[$pr_number]}" ]]; then
      continue;
    fi
    ...
    # save the authorized status for this PR
    authorized_prs[$pr_number]=$RET
  done
}
  ...
  if (( approval_count < pr_approval_min )); then
    if (( user_has_approved )); then
      ...
      if [[ "$pr_author" != "$current_user" ]]; then
        if (( ! approve_pr_is_s )) && [[ "$from_branch" != "main" ]]; then
      ...
      if (( is_authorized || approve_pr_is_s )) && [[ "$pr_author" != "$current_user" ]]; then
...
function proj_prs_s_() {
  ...
  local prs=""
  if command -v gum &>/dev/null; then
    prs=$(gum spin --title="fetching prs... $repo_name" -- gh pr list --repo "$proj_repo" --limit 100 -s open --json number,author,assignees --jq '.[] | {number, author: .author.login, assignees} // empty')
  else
    print " fetching prs... $repo_name"
    prs=$(gh pr list --repo "$proj_repo" --limit 100 -s open --json number,author,assignees --jq '.[] | {number, author: .author.login, assignees} // empty')
  fi
...
function select_pr_() {
  ...
  local pr_list
  # local pr_list=$(gh pr list --repo "$proj_repo" --limit 100 | grep -i "$search_text" | awk -F'\t' '{print $1 "\t" $2 "\t" $3}' 2>/dev/null)
  if command -v gum &>/dev/null; then
    pr_list=$(gum spin --title="fetching prs... $repo_name" -- gh pr list --repo "$proj_repo" --limit 100 --json number,title,headRefName --jq '.[] | "\(.number)\t\(.title)\t\(.headRefName)"' | grep -i "$search_text" 2>/dev/null)
  else
    print " fetching prs... $repo_name"
    pr_list=$(gh pr list --repo "$proj_repo" --limit 100 --json number,title,headRefName --jq '.[] | "\(.number)\t\(.title)\t\(.headRefName)"' | grep -i "$search_text" 2>/dev/null)
  fi
...
function select_prs_() {
  ...
  local pr_list
  if command -v gum &>/dev/null; then
    pr_list=$(gum spin --title="fetching pull requests..." -- gh pr list --repo "$proj_repo" --limit 100 --json number,title,headRefName --jq '.[] | "\(.number)\t\(.title)\t\(.headRefName)"' | grep -i "$search_text" 2>/dev/null)
  else
    pr_list=$(gh pr list --repo "$proj_repo" --limit 100 --json number,title,headRefName --jq '.[] | "\(.number)\t\(.title)\t\(.headRefName)"' | grep -i "$search_text" 2>/dev/null)
  fi
...
function update_jira_status_() {
  ...
  if [[ -z "$current_status" ]]; then
    print " cannot retrieve status of ticket: $jira_key"
    return 1;
  fi
...
function co() {
  ...
  if (( co_is_p && co_is_r )); then
    ...
    local pr
    pr=(${(s:|:)$(select_pr_ "$branch_arg" "$proj_repo" "pull request to detach")})
    if (( $? == 130 )); then return 130; fi
  ...
  if (( co_is_b || co_is_c )); then
    ...
    if [[ -z "$base_branch" ]]; then
      ...
      # local base_branch=$(get_base_branch_ "$folder" 2>/dev/null)
...
function rebase() {
  ...
  if (( rebase_is_h )); then
    ...
    print "  ${hi_yellow_cor}rebase -pa${reset_cor} : push after rebase succeeds or abort if conflicts"
  ...
  if [[ -n "$target_branch_arg" ]]; then
    ...
    base_branch=$(select_branch_ -ai "$target_branch_arg" "base branch" "$folder")
  ...
  if (( RET == 0 )); then
    if [[ -n "$my_branch" ]] && (( rebase_is_p )); then
    ...
    if (( rebase_is_a )); then
      if ! git -C "$folder" rebase --abort &>/dev/null; then
        print " warning: failed to abort rebase, please check the repository state" >&2
      fi
    fi
  fi
...
function merge() {
  ...
  if (( merge_is_h )); then
    ...
    print "  ${hi_yellow_cor}merge -pa${reset_cor} : push after merge succeeds or abort if conflicts"
  ...
  if (( RET == 0 )); then
    if [[ -n "$my_branch" ]] && (( merge_is_p )); then
    ...
    if (( merge_is_a )); then
      if ! git -C "$folder" merge --abort &>/dev/null; then
        print " warning: failed to abort merge, please check the repository state" >&2
      fi
    fi
  fi
...
function pro() {
  set +x
  eval "$(parse_flags_ "$0" "aeruflnxdis" "" "$@")"
  (( pro_is_debug )) && set -x
...
function proj_handler() {
  ...
  set +x
  eval "$(parse_flags_exclusive_ "$0" "mefinrud" "cprvdsmnbjael" "$@")"
...
function proj_handler_open_() {
  ...
  if [[ -z "$(find "$folder_to_open" -maxdepth 1 -type f)" && -n "$(get_folders_ "$folder_to_open")" ]]; then
    local dirs=("${(@f)$(get_folders_ -p "$folder_to_open")}")
