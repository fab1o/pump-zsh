...
function select_jira_status_() {
  ...
  local chosen_status=""
  chosen_status="$(choose_one_ "status $label" "${(@f)jira_statuses}")"
  if (( $? == 130 )); then return 130; fi
...
function select_multiple_jira_status_() {
  ...
  chosen_statuses=($(choose_multiple_ "statuses $label" "${(@f)jira_statuses}"))
  if (( $? == 130 )); then return 130; fi
...
function get_local_branch_() {
  ...
  local ref=""
  for ref in refs/heads/$short_branch; do
...
function get_remote_branch_() {
  ...
  local ref=""
  for ref in refs/remotes/$remote_name/$short_branch; do
    if git -C "$folder" show-ref --verify --quiet "$ref"; then
...
function load_config_index_() {
  ...
  if [[ -z "$i" ]]; then
    print " fatal: load_config_index_ missing index" >&2
    return 1;
  fi
  ...
  local key=""
  for key in "${keys[@]}"; do
    value="$(load_config_entry_ $i "$key")"
...
function proj_prs_l_() {
  ...
  for ticket in "${tickets[@]}"; do
    ...
    local output="$(find_pr_by_jira_key_ -omc "$key" "$proj_repo" 2>/dev/null)"
    local prs=("${(@f)output}")
...
function proj_jira_() {
  ...
  if ! check_jira_ -ipw $i; then return 1; fi
...
function check_jira_() {
  ...
  if (( check_jira_is_s )); then
    ...
    if (( check_jira_is_d || check_jira_is_s_s )); then
      ...
      if [[ -z "$jira_todo" ]]; then
        jira_todo="$(select_jira_status_ $i "to mark issue ${cor}\"To Do\"${reset_cor}")"
        if (( $? == 130 )); then return 130; fi
      fi
      if [[ -z "$jira_todo" ]]; then
    ...
    if (( check_jira_is_o || check_jira_is_s_s )); then
      ...
      if [[ -z "$jira_in_progress" ]]; then
        jira_in_progress="$(select_jira_status_ $i "to mark issue ${cor}\"In Progress\"${reset_cor}")"
        if (( $? == 130 )); then return 130; fi
      fi
      if [[ -z "$jira_in_progress" ]]; then
    ...
    if (( check_jira_is_r || check_jira_is_s_s )); then
      ...
      if [[ -z "$jira_in_review" ]]; then
        jira_in_review="$(select_jira_status_ $i "to mark issue ${cor}\"In Review\"${reset_cor}")"
        if (( $? == 130 )); then return 130; fi
      fi
      if [[ -z "$jira_in_review" ]]; then
        jira_in_review="$(input_type_ "jira status to mark issue ${cor}\"In Review\"${reset_cor}" "In Review" 40)"
        if (( $? == 130 || $? == 2 )); then return 130; fi
      fi
    ...
    if (( check_jira_is_t || check_jira_is_s_s )); then
      ...
      if [[ -z "$jira_in_test" ]]; then
        jira_in_test="$(select_jira_status_ $i "to mark issue ${cor}\"In Test\"${reset_cor}")"
        if (( $? == 130 )); then return 130; fi
      fi
      if [[ -z "$jira_in_test" ]]; then
        jira_in_test="$(input_type_ "jira status to mark issue ${cor}\"n Test\"${reset_cor}" "In Test" 40)"
        if (( $? == 130 || $? == 2 )); then return 130; fi
      fi
    ...
    if (( check_jira_is_a || check_jira_is_s_s )); then
      ...
      if [[ -z "$jira_almost_done" ]]; then
        jira_almost_done="$(select_jira_status_ $i "to mark issue ${cor}\"Ready For Production\"${reset_cor}")"
        if (( $? == 130 )); then return 130; fi
      fi
      if [[ -z "$jira_almost_done" ]]; then
    ...
    if (( check_jira_is_e || check_jira_is_s_s )); then
      ...
      if [[ -z "$jira_done" ]]; then
        jira_done="$(select_jira_status_ $i "to mark issue ${cor}\"Done\"${reset_cor}")"
        if (( $? == 130 )); then return 130; fi
      fi
      if [[ -z "$jira_done" ]]; then
    ...
    if (( check_jira_is_c || check_jira_is_s_s )); then
      ...
      if [[ -z "$jira_canceled" ]]; then
        jira_canceled="$(select_jira_status_ $i "to mark issue ${cor}\"Canceled\"${reset_cor}")"
        if (( $? == 130 )); then return 130; fi
      fi
      if [[ -z "$jira_canceled" ]]; then
...
function find_pr_by_jira_key_() {
  ...
  if (( find_pr_by_jira_key_is_o )); then states+=('"OPEN"'); fi
  if (( find_pr_by_jira_key_is_m )); then states+=('"MERGED"'); fi
  if (( find_pr_by_jira_key_is_c )); then states+=('"CLOSED"'); fi
...
function select_pr_by_jira_key_() {
  ...
  local output="$(find_pr_by_jira_key_ "$jira_key" "$proj_repo" $@ 2>/dev/null)"
  local prs=("${(@f)output}")
...
function update_status_() {
  ...
  if (( update_status_is_v )); then
    ...
    local output_lines="$(find_pr_by_jira_key_ -omc "$jira_key" "$proj_repo" 2>/dev/null)"
...
function select_jira_key_() {
  ...
  local ticket=""
  if (( $(echo "$tickets" | wc -l) > 22 )); then
    ticket="$(filter_one_ "work item $label" "${(@f)$(printf "%s\n" "$tickets")}")"
  elif [[ -n "$tickets" ]]; then
...
function pro() {
  ...
  if (( pro_is_h )); then
    ...
    print "  --"
    print "  ${hi_yellow_cor}pro -n ${yellow_cor}[<version>]${reset_cor} : set the node version using nvm"
  ...
  if (( pro_is_u )); then
    ...
    local config_settings=(
      ...
      "PUMP_JIRA_TODO          =${PUMP_JIRA_TODO[$i]}"
      ...
      "PUMP_JIRA_CANCELED      =${PUMP_JIRA_CANCELED[$i]}"
