...
function parse_args_() {
    ...
    if [[ -n "$type_okz" ]]; then
      if [[ "$type_char" == "f" ]]; then
        ...
        if [[ "$type_char" != "${last_type[1]}" ]]; then
          echo "print ${(q)red_cor}fatal: parse_args_ array type must be the last argument in the spec${(q)reset_cor} >&2"
          echo "return 1"
          return 1;
        fi

        if [[ -z "$sub_type_char" ]]; then
          echo "print ${(q)red_cor}fatal: parse_args_ array type must have a subtype specified after a second colon (e.g. a:t or a:b)${(q)reset_cor} >&2"
          echo "return 1"
          return 1;
        fi
  ...
  if (( arg_count > max_arg )); then
    ...
    echo "print run: ${(q)hi_yellow_cor}$proj_cmd -h${(q)reset_cor} to see usage >&2"
    echo "return 1"
    return 1;
  fi
  ...
  while (( spec_idx <= max_arg )); do
    ...
    if (( arg_idx > $# )) || [[ "${value}" == -* ]]; then
      ...
      if (( is_default )); then
        case "$type" in
          b)
            value="$(get_my_branch_ 2>/dev/null)"
            ;;
          f)
            value="$(pwd)"
            ;;
  ...
  for name in "${(@k)name_values}"; do
    ...
    local _word_count="$(echo "${name_values[$name]}" | wc -w | tr -d ' ')"
    if [[ "$_word_count" -le 1 ]]; then
      echo "${name}=\"${name_values[$name]}\""
    else
      echo "${name}=(${(j: :)${(s: :)name_values[$name]}})"
    fi
  done
...
function proj_rev_() {
  ...
  if is_folder_git_ "$full_rev_folder" &>/dev/null; then
    ...
    if ! clone_repo_ -o "$proj_repo" "$full_rev_folder" "cloning... ${pink_cor}$branch_user${reset_cor}"; then
  ...
  update_pump_file_ "PR_BRANCH" "$branch_user" "$full_rev_folder"
...
function proj_clone_() {
  ...
  if (( single_mode )); then
    ...
    if (( is_first_branch )); then
      ...
      if [[ -n "$jira_key" && "${branch:t:u}" == "${jira_key:u}" ]]; then
  ...
  if ! is_folder_git_ "$folder_to_clone" &>/dev/null; then
    if ! clone_repo_ -o "$proj_repo" "$folder_to_clone" "cloning... ${pink_cor}$branch${reset_cor}"; then
      print " ${red_cor}fatal: failed to clone repository: $proj_repo${reset_cor}" >&2
      return 1;
    fi
  fi
...
function update_jira_status_() {
  ...
  if [[ "$current_assignee_email" == "$current_jira_user_email" ]]; then
    ...
    # if update to in progress, or in review status, assign to self if not already assigned to self
...
function renb() {
  ...
  eval "$(parse_args_ "$0" "new_branch:b,folder:fz" "$@")"
...
function revert() {
  ...
  eval "$(parse_args_ "$0" "hash:ho,parent_number:no,folder:fz" "$@")"
...
function workflow_run_() {
  ...
  if [[ -z "$result" ]]; then
    ...
    gum style --border=rounded --padding="0 1" --margin="0 1" --width="60" --align=left "$error" --border-foreground "$(ansi_to_gum ${hi_gray_cor})"
  ...
  gum style --border=rounded --padding="0 1" --margin="0 1" --width="60" --align=left "$box_content" --border-foreground "$(ansi_to_gum ${cor})"
}
...
function co() {
  ...
  if (( co_is_e && ! co_is_x )); then
    ...
    if ! git -C "$folder" switch "$branch" "$@" && is_branch_status_clean_ "$folder"; then
...
function pro() {
  ...
  if [[ "$proj_arg" == "$CURRENT_PUMP_SHORT_NAME" ]]; then
    if [[ "$PWD/" != "$OLDPWD/"* && "$OLDPWD/" != "$PWD/"* && "$PWD/" == "$CURRENT_PUMP_FOLDER/"* ]]; then
      ...
      if [[ -n "$jira_key" && -z "$jira_title" ]] && command -v acli &>/dev/null; then
        jira_title="$(acli jira workitem view "$jira_key" --fields=summary --json 2>/dev/null | jq -r '.fields.summary')"
        update_pump_file_ "JIRA_TITLE" "$jira_title"
      fi
...
function proj_handler_open_() {
  ...
  if (( ! is_proj_folder )); then
    ...
    if [[ -n "$dirs" ]]; then
      ...
      if (( RET == 130 )); then
        if [[ "$folder_to_open" != "$proj_folder" ]]; then
...
function recommit() {
  ...
  eval "$(parse_args_ "$proj_cmd gha" "message:to,folder:fz" "$@")"
  ...
  if (( recommit_is_i )); then
    if ! commit -eq "${flags[@]}" "$message" "$folder" "$@"; then
      return 1;
    fi
  else
    if ! commit -eaq "${flags[@]}" "$message" "$folder" "$@"; then
      return 1;
    fi
  fi
...
function get_folders_i_() {
  ...
  if [[ -z "$pump_jira_title" ]]; then
    if [[ -n "$pump_jira_key" ]]; then
      if command -v acli &>/dev/null; then
        pump_jira_title="$(acli jira workitem view "$pump_jira_key" --fields=summary --json 2>/dev/null | jq -r '.fields.summary')"
      fi
