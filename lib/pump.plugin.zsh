...
function workflow_runs_() {
  ...
  if [[ -z "$@" ]]; then
    ...
    if (( workflow_runs_is_s )); then
      workflow_run_ -s "$proj_repo" "$workflow" "" "$cor"
    else
  ...
  for arg in "$@"; do
    ...
    if (( workflow_runs_is_s )); then
      workflow_run_ -s "$proj_repo" "$workflow" "$arg"
    else
...
function proj_handler() {
  ...
  if (( proj_handler_is_o && proj_handler_is_f )); then
    ...
    folder="$(proj_handler_folder_ -of $i "$@")"
    RET=$?
