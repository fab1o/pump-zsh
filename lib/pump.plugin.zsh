...
typeset -g bold_gray_cor=$'\e[1;37m'
...
typeset -g colors=("${bold_yellow_cor}" "${bold_blue_cor}" "${bold_magenta_cor}" "${bold_green_cor}" "${bold_dark_green_cor}" "${bold_orange_cor}" "${bold_red_cor}" "${bold_cyan_cor}")
...
function is_flag_() {
  if [[ "$1" =~ '^-[a-zA-Z0-9]+$' || "$1" =~ '^--[a-zA-Z0-9]+(-+[a-zA-Z0-9]+)*$' ]]; then
    return 0;
  fi
...
function is_optional_arg_() {
  if [[ -z "$1" ]] || ! is_flag_ "$1"; then
    return 0;
  fi
...
function parse_flags__() {
  ...
  # if the function name prefix_arg is private (has _ at the end), then it doesn't need to validate flags,
  # any flag is valid
  ...
  for arg in "$@"; do
    ...
    if is_double_dash_flag_ "$arg"; then
      flags_double_dash+=("${arg}")

      continue;
    fi

    if [[ "$arg" == -<-> ]]; then # number flag (e.g. -1, -2, -3)
      local numbers="${arg#-}"
      local is_invalid=0
      local is_pass_along=0

      local i=0
      for (( i=0; i < ${#numbers}; i++ )); do
        local opt="${numbers:$i:1}"
      ...
      if (( is_invalid )); then
        ...
        if [[ ! " ${invalid_opts[@]} " =~ " -${numbers} " ]]; then
          invalid_opts+=("-${numbers}")

          if (( ! internal_func || ! is_invalid )); then
            if [[ "$@" == *"--debug"* ]] || (( is_debug )); then
              print " ${red_cor}${prefix_arg} fatal: invalid option: -${numbers}${reset_cor}" >&2
            else
              print " ${red_cor} fatal: invalid option: -${numbers}${reset_cor}" >&2
            fi
            print "  ---" >&2
          fi
        fi
    ...
    local all_letters="${arg#-}"
    local y=0

    for (( y=0; y < ${#all_letters}; y++ )); do
      # get the next two letters to check if it's a double flag (e.g. -ab, -aa, -bc)
      local letters="${all_letters:$y:2}"      
      local is_double=0

      # check if arg is double flag (e.g. -ab, -aa, -bc)
      if [[ ${#letters} -eq 2 ]]; then
        is_double=1
      fi
      ...
      for (( i=0; i < ${#letters}; i++ )); do
        ...
        if [[ $valid_flags != *$opt* && "$valid_flags_pass_along" != "all" ]] && [[ -z "$hidden_flags" || $hidden_flags != *$opt* || -z "$evaluated_flags" ]]; then
          if [[ ! " ${invalid_opts[@]} " =~ " -${opt} " ]]; then
            ...
            if (( ! internal_func || ! is_invalid )); then
              # if [[ "$@" == *"--debug"* ]] || (( is_debug )); then
                # print " ${red_cor}${prefix_arg} fatal: invalid option: -${opt}${reset_cor}" >&2
              # else
          ...
          # if (( is_double )); then
          #   if (( i == 1 )) && { [[ "$valid_flags_pass_along" == "all" ]] || [[ $valid_flags_pass_along == *${letters:0:1}* && $valid_flags_pass_along == *${letters:1:1}* ]] }; then
          #     double_flags+=("-${letters}");
          #   fi
          # else
          #   if ! [[ " ${flags[@]} " =~ " -${opt} " ]]; then
          #     flags+=("-${opt}")
        ...
        if ! [[ " ${invalid_opts[@]} " =~ " -${opt} " ]]; then
          ...
          if (( is_double )); then
            echo "${prefix}_is_${letters}=1"
          fi

          if [[ $valid_flags_pass_along == *$opt* || "$valid_flags_pass_along" == "all" ]]; then
            if (( is_double )); then
              if (( i == 1 )) && { [[ "$valid_flags_pass_along" == "all" ]] || [[ $valid_flags_pass_along == *${letters:0:1}* && $valid_flags_pass_along == *${letters:1:1}* ]] }; then
                double_flags+=("-${letters}");
              fi
  ...
  echo "${prefix}_is_flags=()"

  flag=""
  for flag in "${flags[@]}"; do
    echo "${prefix}_is_flags+=(\"$flag\")"
  done

  flag=""
  for flag in "${double_flags[@]}"; do
    echo "${prefix}_is_flags+=(\"$flag\")"
  done

  if [[ ${#non_flags} -gt 0 ]]; then
    print -r -- set -- "${(q)non_flags[@]}" "${(q)flags[@]}" "${(q)double_flags[@]}" "${(q)flags_double_dash[@]}"
  else
    print -r -- set -- "${(q)flags[@]}" "${(q)double_flags[@]}" "${(q)flags_double_dash[@]}"
  fi
}
...
function parse_simple_flags_() {
  ...
  print -r -- set -- "${(q)flags_double_dash[@]}" "${(q)non_flags[@]}" 
}
...
function parse_args_() {
  ...
  #   e  = env (either "dev" "qa" "stage" or "prod")
  ...
  #   m  = markup/formatted text (any non-flag string, can contain spaces and start with dash)
  ...
  #
  # output: TAB-separated values followed by arg_count
  #   value1<TAB>value2<TAB>...<TAB>arg_count
  ...
  local ignore_max_restriction=0
  ...
  for s in "${specs[@]}"; do
    ...
    if [[ "$name" == "*" ]]; then
      ignore_max_restriction=1
      break;
    fi

    local type="${s#*:}" # mz:%s
    local type_char="${type[1]}" # m
    local type_okz="${type[2]}" # z

    local sub_type="${type#*:}" # %s
      ...
      if [[ "$type_okz" == "o" ]]; then
        ...
        case "$type_char" in
          ...
          n)
            ...
            if [[ -z "$default_value" && "$name" == "interval" ]]; then
  ...
  if (( ignore_max_restriction )); then
    arg_count="${#names[@]}"
  fi
  ...
  if (( arg_count > max_arg )); then
    ...
    echo "print \" run: ${hi_yellow_cor}$cmd -h${reset_cor} to see usage\" >&2"
    echo "return 1"
    return 1;
  fi
  ...
  while (( spec_idx <= max_arg )); do
    ...
    if (( arg_idx > $# )) || is_flag_ "${value}"; then
      if (( ! optional )); then
    ...
    local output=""
    output="$(parse_arg_ "$value" "$type" "$sub_type" "$default_value" "$has_array" "$has_folder" "$arg_count" "$max_arg")"
  ...
  case "$type" in
    a)
      ...
      if [[ -z "$value" ]]; then
        ...
        local output=""
        output="$(parse_arg_ "$value" "$sub_type" "" "$default_value" "$has_array" "$has_folder" "$arg_count" "$max_arg")"
        IFS=$NUL read -r is_valid is_usable value <<< "$output"
      fi
      ;;
    b)
    ...
    e)
      # env
      local env="${value##*:}"
      if [[ -z "$env" ]] || [[ "$env" != "dev" && "$env" != "stage" && "$env" != "qa" && "$env" != "prod" && "$env" != "test" && "$env" != "pre" ]]; then
        is_valid=0
        if [[ -z "$value" ]]; then
          value="$default_value"
        else
    ...
    t)
      ...
      ;;
    m)
      # markup/formatted text (must have %)
      if [[ -z "$value" || "$value" != *%* ]]; then
        is_valid=0
        if [[ -z "$default_value" || "$default_value" != *%* ]]; then
          is_usable=0
        else
          value="$default_value"
        fi
  ...
  if [[ -n "$latest_version" && "$PUMP_VERSION" != "$latest_version" ]]; then
    ...
    if command -v gum &>/dev/null; then
      gum spin --title="updating pump-zsh..." -- \
        zsh -c "$(curl -H "Cache-Control: no-cache" -H 'Pragma: no-cache' -fsSL https://raw.githubusercontent.com/fab1o/pump-zsh/refs/heads/main/scripts/update.zsh)"
    else
...
function input_type_() {
  ...
  if [[ -n "$header" ]]; then
    print " ${purple_cor}${cor}$header:${reset_cor}" >&2
  fi

  if command -v gum &>/dev/null; then
    _input="$(gum input --placeholder="$placeholder" --char-limit="$max" --value="$value" "${flags[@]}")"
    local RET=$?
...
function filter_one_() {
  ...
  print " ${purple_cor}${cor}choose $header: ${reset_cor}" >&2
...
function choose_one_() {
  ...
  if command -v gum &>/dev/null; then
    local choice=""
    choice="$(gum choose --height="25" --limit=1 --header="${cor} choose $header:${reset_cor}" "${flags[@]}" -- "${@:2}" 2>/dev/tty)"
    local RET=$?
    if (( RET != 0 )); then return $RET; fi
...
function choose_multiple_() {
  ...
  if command -v gum &>/dev/null; then
    ...
    local choices_out=""
    choices_out="$(gum choose --height="25" --no-limit --header="${cor} choose multiple $header ${purple_cor}(use spacebar to select)${purple_cor}:${reset_cor}" "${flags[@]}" -- "${@:2}")"
    local RET=$?
    if (( RET != 0 )); then return $RET; fi

    local choice=""
    for choice in "${(@f)choices_out}"; do
...
function get_pump_jira_title_() {
  set +x
  eval "$(parse_flags_ "$0" "ns" "" "$@")"
  # (( get_pump_jira_title_is_debug )) && set -x
  ...
  if [[ -z "$folder" ]]; then
    folder="$(find_git_folder_ "$PWD" 2>/dev/null)"
  ...
  if [[ -z "$jira_title" ]] || [[ -n "$pump_jira_key" && "$pump_jira_key" != "$jira_key" ]]; then
    if command -v acli &>/dev/null; then
      ...
      if (( ! get_pump_jira_title_is_n )); then
...
function read_pump_value_() {
  ...
  if [[ -z "$key" ]]; then
    print " ${red_cor}error: key is required to read pump file${reset_cor}" >&2
    return 1;
  fi

  if [[ -z "$folder" ]]; then
    print " ${red_cor}error: folder is required to read pump file${reset_cor}" >&2
    return 1;
  fi
  ...
  local file="${folder}/.pump/${branch_folder}.pump"
...
function update_pump_file_() {
  ...
  if [[ -z "$key" ]]; then
    print " ${red_cor}error: key is required to update pump file${reset_cor}" >&2
    return 1;
  fi

  if [[ -z "$folder" ]]; then
    print " ${red_cor}error: folder is required to update pump file${reset_cor}" >&2
    return 1;
  fi
...
function setup_git_merge_tool_() {
  if [[ -z "$PUMP_MERGE_TOOL" ]]; then
    if confirm_ "do you want to define a merge tool?" "yes" "no"; then
      ...
      update_setting_ -fd "PUMP_MERGE_TOOL" "-"
    fi
  fi

  if [[ -n "$PUMP_MERGE_TOOL" ]] && command -v "$PUMP_MERGE_TOOL" &>/dev/null; then
    ...
    local cmd="$(basename "$PUMP_MERGE_TOOL")"

    git config --global mergetool.$cmd.cmd "$PUMP_MERGE_TOOL --new-window --wait --merge \"\$LOCAL\" \"\$REMOTE\" \"\$BASE\" \"\$MERGED\""
...
function pro_c_() {
  ...
  if (( pro_c_is_f )); then
    remove_proj_folders_ -a "$proj_cmd" "$proj_folder"
  else
  ...
  for branch_or_folder in "${branch_or_folders[@]}"; do
    ...
    if [[ -n "$key" ]]; then
      ...
      if [[ " ${chosen_statuses[*]:u} " == *" ${jira_status:u} "* ]]; then
        if (( single_mode )); then
          if (( pro_c_is_f )); then
        ...
        print " ${magenta_cor}${branch_or_folder}${reset_cor}: ${cyan_cor}${jira_status}${reset_cor}"
      fi
    fi
  done
}
...
function pro_i_() {
  ...
  if [[ -n "$proj_arg" ]]; then
    x="$(find_proj_index_ -oe "$proj_arg" "project to show info")"
    (( x )) || return 1;
  fi
  ...
  for i in {1..9}; do
    ...
    if [[ -n "${PUMP_FOLDER[$i]}" && -n "${PUMP_SHORT_NAME[$i]}" ]]; then
      ...
      local name_v="$(truncate_ "${PUMP_SHORT_NAME[$i]}" 60)"
      local folder_v="$(truncate_ "${PUMP_FOLDER[$i]}" 60)"
      local manager_v="$(truncate_ "${PUMP_PKG_MANAGER[$i]}" 60)"
      local node_v="$(truncate_ "${PUMP_NVM_USE_V[$i]}" 60)"
      local repo_v="$(truncate_ "${PUMP_REPO[$i]}" 60)"

      local box_content=""
      box_content+="${gray_cor}command: ${blue_cor}${name_v}${reset_cor}"$'\n'
      box_content+="${gray_cor}folder: ${hi_gray_cor}${folder_v}${reset_cor}"$'\n'
      box_content+="${gray_cor}mode: ${mode_cor}${new_mode}${reset_cor}"$'\n'
      box_content+="${gray_cor}node v.: ${hi_cyan_cor}${node_v}${reset_cor} (${hi_magenta_cor}${manager_v}${reset_cor})"$'\n'
      box_content+="${gray_cor}repository: ${hi_gray_cor}${repo_v}${reset_cor}"

      if command -v gum &>/dev/null; then
        gum style --border=rounded --padding="0 1" --margin="0 1" --width="76" --align=left "$box_content" --border-foreground 33
      else
        print -r -- "$box_content"
      fi
    fi
...
function pro_u_() {
  ...
  local all_project_settings=(
    ...
    "PUMP_ENV_LIST            =$(truncate_ "${PUMP_ENV_LIST[$i]}" 70)"
    ...
    "PUMP_JIRA_BASE_URL       =$(truncate_ "${PUMP_JIRA_BASE_URL[$i]}" 70)"
    ...
    "PUMP_PR_LABELS_IGNORE    =$(truncate_ "${PUMP_PR_LABELS_IGNORE[$i]}" 70)"
...
function pro_r_() {
  ...
  for arg in "${proj_args[@]}"; do
    ...
    remove_proj_folders_ -a "${PUMP_SHORT_NAME[$i]}" "${PUMP_FOLDER[$i]}"
...
function pro_pwd_() {
  ...
  if (( foundI )); then
    if confirm_ "update project ${bold_pink_cor}${pkg_name}${reset_cor} to new folder: ${hi_cyan_cor}$PWD${reset_cor} ?" "update" "ignore"; then
...
function help_most_popular_() {
  ...
  if [[ -n "$pkg_manager" ]]; then
    ...
    if (( ${#CURRENT_PUMP_TEST} > max )); then
      printf "  ${hi_magenta_cor}%-$spaces${reset_cor} = %s \n" "te" "run CURRENT_PUMP_TEST"
    else
      printf "  ${hi_magenta_cor}%-$spaces${reset_cor} = %s \n" "te" "$CURRENT_PUMP_TEST"
    fi
...
function help_pkg_manager_() {
  ...
  if [[ -n "$CURRENT_PUMP_RUN_DEV" ]]; then
    printf "  ${hi_magenta_cor}%-$spaces${reset_cor} = %s \n" "run" "run PUMP_RUN_DEV"
  else
  ...
  if (( ${#CURRENT_PUMP_TEST_WATCH} > max )); then
    printf "  ${hi_magenta_cor}%-$spaces${reset_cor} = %s \n" "tew" "run CURRENT_PUMP_TEST_WATCH"
  else
    printf "  ${hi_magenta_cor}%-$spaces${reset_cor} = %s \n" "tew" "$CURRENT_PUMP_TEST_WATCH"
  fi
  if [[ "$CURRENT_PUMP_TEST" != "$pkg_manager test" ]]; then
    printf "  ${magenta_cor}%-$spaces${reset_cor} = %s \n" "${pkg_manager:0:1}te" "$pkg_manager test"
  fi
  ...
  if [[ "$CURRENT_PUMP_TEST_WATCH" != "$pkg_manager run test:watch" ]]; then
    printf "  ${magenta_cor}%-$spaces${reset_cor} = %s \n" "${pkg_manager:0:1}tew" "$pkg_manager run test:watch"
  fi
}
...
function validate_proj_cmd_strict_() {
  ...
  local invalid_values=("pwd" "quit" "done" "path" "bkp" "clone" "del" "deploy" "gha" "jira" "prs" "pull" "rel" "rels" "rev" "revs" "run" "setup" "tag" "tags" "exec" "version")
...
function validate_proj_cmd_() {
  ...
  elif ! [[ "$proj_cmd" =~ ^[a-z0-9][a-z0-9-]*$ ]] || is_flag_ "$proj_cmd"; then
...
function get_folders_() {
  ...
  local is_work_types_added=0
  local is_dash_added=0
  local filtered_folders=()

  if (( get_folders_is_p )); then
    # work types: bug, chore, story...
    local dir=""
    for dir in "${dirs[@]}"; do
      ...
      is_work_types_added=1
    done

    local is_non_work_types_added=0

    # non work items: folders that are not work types and not jira work items
    local dir=""
    for dir in "${dirs[@]}"; do
      if [[ -n "$(extract_jira_key_ "$dir")" || " ${filtered_folders[@]} " == *" $dir "* ]]; then
        continue;
      fi
      if [[ -z "$(ls -- "$dir")" ]] || is_folder_git_ -ro "$dir" &>/dev/null; then
        continue;
      fi

      filtered_folders+=("$dir")

      if (( is_work_types_added && is_dash_added == 0 )); then
        display_folders+=("--")
        is_dash_added=1
      fi
      ...
      is_non_work_types_added=1
    done

    local is_non_work_items_added=0
    is_dash_added=0
    ...
    for dir in "${dirs[@]}"; do
      ...
      if (( is_non_work_types_added && is_dash_added == 0 )); then
        display_folders+=("--")
        is_dash_added=1
      fi
    ...
    for dir in "${dirs[@]}"; do
      ...
      if (( is_non_work_items_added && is_dash_added == 0 )); then
        display_folders+=("--")
        is_dash_added=1
      fi
...
function trim_() {
  ...
  if [[ -n "$val" ]]; then
    # val="$(print -r -- "$val" | xargs 2>/dev/null)" # this is causing \n to be removed from the string
    val="${val#"${val%%[!$' \t\r\n']*}"}"
    val="${val%"${val##*[!$' \t\r\n']}"}"
...
function update_setting_() {
  ...
  if (( update_setting_is_d )); then
    print " ${gray_cor}run: ${hi_gray_cor}pro -u${gray_cor} to reset this setting${reset_cor}" >&2
  fi
}

function update_config_() {
  set +x
  eval "$(parse_flags_ "$0" "ds" "" "$@")"
  ...
  if (( update_config_is_d )) && [[ -n "${PUMP_SHORT_NAME[$i]}" ]]; then
...
function normalize_branch_name_() {
  ...
  if [[ -z "$branch" ]]; then
    print " ${red_cor}fatal: branch name cannot be empty${reset_cor}" >&2
    return 1;
  fi
  ...
  if ! git check-ref-format --branch "$branch" &>/dev/null; then
    print " ${red_cor}fatal: invalid branch name: $(truncate_ $branch)${reset_cor}" >&2
    return 1;
  fi
...
function input_branch_name_() {
  ...
  while true; do
    ...
    if [[ -n "$typed_value" ]]; then
      typed_value="$(normalize_branch_name_ "$typed_value")"
...
function input_type_mandatory_() {
  ...
  while true; do
    ...
    local typed_value=""
    if (( input_type_mandatory_is_a )); then
      typed_value="$(input_type_ -a "$header" "$placeholder" "$max" "$value")"
      if (( $? == 124 )); then
        typed_value="$value"
      fi
    else
    ...
    if [[ -n "$typed_value" ]]; then
      if (( input_type_mandatory_is_n )) && [[ "$typed_value" != <-> ]]; then
        print " ${red_cor}value must be a number${reset_cor}" >&2
        continue;
      fi
      ...
      if (( input_type_mandatory_is_m )) && [[ ! "$typed_value" =~ ^[a-zA-Z]{1,2}$ ]]; then
        print " ${red_cor}value must be 1-2 characters and contain letters only${reset_cor}" >&2
        continue;
      fi
...
function input_number_() {
  ...
  local typed_value=""
  typed_value="$(input_type_mandatory_ -n "$@")"
  local RET=$?
...
function check_proj_() {
  set +x
  eval "$(parse_flags_ "$0" "fmeprgx" "qv" "$@")"
  ...
  if [[ -z "$i" ]]; then
    print " fatal: check_proj_ project index is required" >&2
    return 1;
  fi
  ...
  if (( i )); then
    ...
    if [[ -z "$folder" ]]; then
      print " fatal: check_proj_ project index 0 must have a folder argument" >&2
      return 1;
    fi
  fi
  ...
  local is_updated_config=0
  local is_updated_settings=0

  if (( check_proj_is_r || check_proj_is_e )); then
    if [[ -z "$PUMP_INTERVAL" ]]; then
      local internval=""
      while true; do
        interval="$(input_number_ -o "type the interval in minutes" "" 2 "20")"
        if (( $? == 130 )); then return 130; fi

        if (( interval < 1 )); then
          print " ${red_cor}value must be more than 0" >&2
          continue;
        fi

        break;
      done
      is_updated_settings=1
      update_setting_ -f "PUMP_INTERVAL" "$interval"
      PUMP_INTERVAL=$interval
    fi
  fi
  ...
  if (( i )); then
    ...
    if (( check_proj_is_e )); then
      if [[ -z "${PUMP_ENV_LIST[$i]}" ]]; then
        confirm_ "would you like to setup environment types for ${PUMP_SHORT_NAME[$i]}?"
        local RET=$?
        if (( RET == 130 )); then return 130; fi
        if (( RET == 0 )); then
          local env_list=""
          local env=""
          env="$(input_type_ "write an environment type" "" 20 "dev")"
          ...
          if [[ -n "$env" ]]; then
            ...
            while true; do
              env=""
              env="$(input_type_ "write another environment type or leave empty to finish" "" 20)"
              if (( $? == 130 )) then
                env_list=""
              fi

              if [[ -z "$env" ]]; then break; fi

              env_list="${env_list}$TAB${env}"
            done

            if [[ -n "$env_list" ]]; then
              is_updated_config=1
              update_config_ $i "PUMP_ENV_LIST" "$env_list" &>/dev/null
              PUMP_ENV_LIST[$i]="$env_list"

              print " ${gray_cor}updated: PUMP_ENV_LIST_${i}=${env_list//$TAB/,}${reset_cor}" >&2
            fi
          fi
        else
          is_updated_config=1
          update_config_ $i "PUMP_ENV_LIST" "0"
          PUMP_ENV_LIST[$i]="0"
        fi
      fi
    fi
    ...
    if (( check_proj_is_f )); then
      if [[ ! -d "$folder" ]]; then
        print " ${red_cor}not a valid folder: $folder${reset_cor}" >&2
        return 1;
      fi
    fi

    if (( check_proj_is_m )); then
      if [[ -z "$CURRENT_PUMP_SINGLE_MODE" ]]; then
        CURRENT_PUMP_SINGLE_MODE=1
      fi
    fi

    if (( check_proj_is_r )); then
      if ! check_proj_repo_ -sq $i "$CURRENT_PUMP_REPO" "$folder" "" "$CURRENT_PUMP_SINGLE_MODE" "$@"; then return 1; fi

      if (( ! check_proj_is_q )) && [[ -z "$CURRENT_PUMP_REPO" ]]; then
        print " ${red_cor}missing repository uri${reset_cor}" >&2
        return 1;
      fi
    fi

    if (( check_proj_is_p )); then
      if [[ -z "$CURRENT_PUMP_REPO" ]]; then
        CURRENT_PUMP_REPO="$(get_repo_ "$folder" 2>/dev/null)"
      fi

      if ! check_proj_pkg_manager_ -sq $i "$CURRENT_PUMP_PKG_MANAGER" "$folder" "$CURRENT_PUMP_REPO" "$@"; then return 1; fi

      if [[ -z "$CURRENT_PUMP_PKG_MANAGER" ]]; then
        CURRENT_PUMP_PKG_MANAGER="npm"
      fi
    fi

  fi

  if (( is_updated_config )); then
    print " ${gray_cor}run: ${hi_gray_cor}${PUMP_SHORT_NAME[$i]} -u${gray_cor} to reset config${reset_cor}" >&2
  fi
}
...
function get_jira_board_() {
  ...
  local jira_boards="$(gum spin --title="fetching boards..." -- curl -s \
...
function select_jira_proj_() {
  ...
  local output="$(gum spin --title="fetching projects..." -- acli jira project list --recent --json | jq -r '.[].key')"
...
function check_work_types_() {
  ...
  if [[ -n "$work_types" && "${PUMP_JIRA_PROJECT[$i]}" == "$jira_proj" ]]; then
    ...
    local jira_work_types="$(gum spin --title="fetching work types..." -- acli jira workitem search --jql "project=\"$jira_proj\"" --fields=issuetype --json | jq -r '.[].fields.issuetype.name' | sort -u)"
...
function select_jira_release_() {
  ...
  select_jira_releases_ -un "$@"
}

function select_jira_releases_() {
  set +x
  eval "$(parse_flags_all_ "$0" "un" "$@")"
  ...
  local jira_releases="$(gum spin --title="fetching releases..." -- curl -s \
    -u "$current_jira_user_email:$jira_api_token" \
    -H "Accept: application/json" \
  ...
  if (( select_jira_releases_is_un )); then
    local release=""
  ...
  if (( $? == 130 )); then return 130; fi
  if [[ -z "$releases" ]]; then return 1; fi
  ...
  if [[ -z "$error_msg" ]]; then
    ...
    for j in {1..10}; do
      if [[ $j -ne $i && -n "$PUMP_FOLDER[$j]" && -n "${PUMP_SHORT_NAME[$j]}" ]]; then
        ...
        local real_pump_folder=""

        if command -v realpath &>/dev/null; then
          real_pump_folder="$(realpath -- "${PUMP_FOLDER[$j]}" 2>/dev/null)"
        else
          real_pump_folder="${PUMP_FOLDER[$j]:A}"
        fi
...
function check_proj_script_folder_() {
  ...
  if [[ -z "$proj_script_folder" ]]; then
    ...
    if command -v realpath &>/dev/null; then
      ...
      real_proj_folder="${proj_script_folder:A}"
    fi
...
function save_proj_folder_() {
  ...
  while true; do
    ...
    if (( save_proj_folder_is_e )) && [[ -n "$proj_folder" ]]; then
      confirm_ "keep using project folder: ${hi_cyan_cor}$proj_folder${reset_cor}?"
      RET=$?
      if (( RET == 130 )); then return 130; fi
      if (( RET == 1 )); then
        proj_folder=""
...
function proj_handler_folder_() {
  ...
  if [[ -n "$folder_arg" && -z "${folder_arg//\/}" ]]; then
    if [[ ! -d "$proj_folder" || -z "$(ls -- "$proj_folder")" ]]; then
  ...
  # check if folder_arg contains a slash, if so, treat it as a path relative to the project folder
  if [[ -n "$folder_arg" && "$folder_arg" == *"/"* ]]; then
    local folder_path="${proj_folder}/${folder_arg}"
  ...
  if (( single_mode )); then
    ...
    if [[ -n "$folder_arg" ]] || (( is_of )); then
      ...
      if [[ -z "$choosen_folder" ]]; then
        local dirs=""
        dirs="$(get_folders_ -ap $i "$proj_folder" "$folder_arg" 2>/dev/null)"
        if (( $? == 130 )); then return 130; fi

        if [[ -z "$dirs" && -n "$folder_arg" ]]; then
          print " not a valid folder: ${hi_cyan_cor}$folder_arg${reset_cor}" >&2

          dirs="$(get_folders_ -ap $i "$proj_folder" 2>/dev/null)"
          if (( $? == 130 )); then return 130; fi
        fi
        ...
        RET=$?
        if (( RET == 130 )); then return 130; fi
        if [[ -z "$choosen_folder" ]]; then return 1; fi
      fi
    fi
    ...
    if [[ -n "$folder_arg" ]] && (( is_of )); then
      ...
      if [[ -d "${proj_folder}/${folder_arg}" ]]; then
        print -r -- "${proj_folder}/${folder_arg}"
    ...
    local dirs=""
    dirs="$(get_folders_ -aijp $i "$proj_folder" "$folder_arg" 2>/dev/null)"
    if (( $? == 130 )); then return 130; fi

    if [[ -z "$dirs" && -n "$folder_arg" ]]; then
      if [[ -d "${proj_folder}/${folder_arg}" ]]; then
        folder_to_open="${proj_folder}/${folder_arg}"

        if [[ -z "$(ls -- "$folder_to_open")" ]]; then
          print " run: ${hi_yellow_cor}${proj_cmd} clone${reset_cor}" >&2
        fi

        print -r -- "$folder_to_open"
        return 0;
      fi

      print " not a valid path within $proj_cmd: ${red_cor}$folder_arg${reset_cor}" >&2

      dirs="$(get_folders_ -aip $i "$proj_folder" 2>/dev/null)"
      if (( $? == 130 )); then return 130; fi
    fi
  ...
  print -r -- "$(proj_handler_open_ -- $i "$folder_to_open")"
}
...
function proj_handler_open_() {
  ...
  if (( ! is_proj_folder )); then
    local dirs=""
    dirs="$(get_folders_ -aijp $i "$folder_to_open" 2>/dev/null)"
    if (( $? == 130 )); then return 130; fi
    ...
    if [[ -n "$dirs" ]]; then
      ...
      if (( RET == 130 )); then
        ...
        if (( go_back )); then
          print -r -- "$(proj_handler_open_ -- $i "$proj_folder")"
        ...
        if [[ -z "$go_back" ]]; then
          ...
          local _RET=$?
          if (( _RET == 0 )); then
            update_config_ $i "PUMP_GO_BACK" "0" &>/dev/null
            cd "$folder_to_open"
            return $?;
          fi
          if (( _RET == 1 )); then
            update_config_ $i "PUMP_GO_BACK" "1" &>/dev/null
      ...
      if [[ -n "$choosen_folder" ]]; then
        ...
        print -r -- "$(proj_handler_open_ -- $i "${folder_to_open}/${choosen_folder}")"
...
function remove_proj_() {
  ...
  if (( remove_proj_is_u )); then
    ...
    update_config_ $i "PUMP_RUN_DEV" "" &>/dev/null
    ...
    update_config_ $i "PUMP_PR_LABELS_IGNORE" "" &>/dev/null
    update_config_ $i "PUMP_GO_BACK" "" &>/dev/null
    update_config_ $i "PUMP_ENV_LIST" "" &>/dev/null
...
function remove_proj_folders_() {
  ...
  if (( remove_proj_folders_is_r )) || (( ! remove_proj_folders_is_c && ! remove_proj_folders_is_t && ! remove_proj_folders_is_b )); then
    local revs_folder="$(get_proj_special_folder_ -rd "$proj_cmd" "$folder")"

    if [[ -d "$revs_folder" ]]; then
      confirm_ "remove ${bold_cor}revs${reset_cor} folder: ${bold_yellow_cor}$revs_folder${reset_cor} ?" "$@"
      RET=$?
      if (( RET == 130 )); then return 130; fi
      if (( RET == 0 )); then
        folders_to_delete+=("$revs_folder")
      fi
    fi
  fi

  if (( remove_proj_folders_is_c )) || (( ! remove_proj_folders_is_r && ! remove_proj_folders_is_t && ! remove_proj_folders_is_b )); then
    cov_folder="$(get_proj_special_folder_ -cd "$proj_cmd" "$folder")"

    if [[ -d "$cov_folder" ]]; then
      confirm_ "remove ${bold_cor}cov${reset_cor} folder: ${bold_yellow_cor}$cov_folder${reset_cor} ?" "$@"
      RET=$?
      if (( RET == 130 )); then return 130; fi
      if (( RET == 0 )); then
        folders_to_delete+=("$cov_folder")
      fi
    fi
  fi

  if (( remove_proj_folders_is_t )) || (( ! remove_proj_folders_is_r && ! remove_proj_folders_is_c && ! remove_proj_folders_is_b )); then
    temp_folder="$(get_proj_special_folder_ -td "$proj_cmd" "$folder")"

    if [[ -d "$temp_folder" ]]; then
      confirm_ "remove ${bold_cor}temp${reset_cor} folder: ${bold_yellow_cor}$temp_folder${reset_cor} ?" "$@"
      RET=$?
      if (( RET == 130 )); then return 130; fi
      if (( RET == 0 )); then
        folders_to_delete+=("$temp_folder")
      fi
    fi
  fi

  if (( remove_proj_folders_is_b )) || (( ! remove_proj_folders_is_r && ! remove_proj_folders_is_c && ! remove_proj_folders_is_t )); then
    temp_folder="$(get_proj_special_folder_ -bd "$proj_cmd" "$folder")"

    if [[ -d "$temp_folder" ]]; then
      confirm_ "remove ${bold_cor}backups${reset_cor} folder: ${bold_yellow_cor}$temp_folder${reset_cor} ?" "$@"
      RET=$?
      if (( RET == 130 )); then return 130; fi
      if (( RET == 0 )); then
        folders_to_delete+=("$temp_folder")
  ...
  if [[ -n "${folders_to_delete[*]}" ]]; then
    if command -v gum &>/dev/null; then
      gum spin --title="removing project folders..." -- rm -rf -- "${folders_to_delete[@]}"
    else
...
function set_current_proj_() {
  ...
  CURRENT_PUMP_RUN_DEV="${PUMP_RUN_DEV[$i]}"
  ...
  CURRENT_PUMP_PR_LABELS_IGNORE="${PUMP_PR_LABELS_IGNORE[$i]}"
  CURRENT_PUMP_GO_BACK="${PUMP_GO_BACK[$i]}"
  CURRENT_PUMP_ENV_LIST="${PUMP_ENV_LIST[$i]}"
...
function print_current_proj_() {
  ...
  if (( i > 0 )); then
    ...
    print " [${hi_magenta_cor}PUMP_ENV_LIST_${i}=${hi_gray_cor}${PUMP_ENV_LIST[$i]}${reset_cor}]"
    ...
    print " [${hi_magenta_cor}PUMP_JIRA_BASE_URL_${i}=${hi_gray_cor}${PUMP_JIRA_BASE_URL[$i]}${reset_cor}]"
    ...
    print " [${hi_magenta_cor}PUMP_PR_LABELS_IGNORE_${i}=${hi_gray_cor}${PUMP_PR_LABELS_IGNORE[$i]}${reset_cor}]"
    ...
    print " [${hi_magenta_cor}PUMP_RUN_${i}=${hi_gray_cor}${PUMP_RUN_DEV[$i]}${reset_cor}]"
  ...
  print " [${hi_magenta_cor}CURRENT_PUMP_ENV_LIST=${hi_gray_cor}${CURRENT_PUMP_ENV_LIST}${reset_cor}]"
  ...
  print " [${hi_magenta_cor}CURRENT_PUMP_JIRA_BASE_URL=${hi_gray_cor}${CURRENT_PUMP_JIRA_BASE_URL}${reset_cor}]"
  ...
  print " [${hi_magenta_cor}CURRENT_PUMP_PR_LABELS_IGNORE=${hi_gray_cor}${CURRENT_PUMP_PR_LABELS_IGNORE}${reset_cor}]"
  ...
  print " [${hi_magenta_cor}CURRENT_PUMP_RUN_DEV=${hi_gray_cor}${CURRENT_PUMP_RUN_DEV}${reset_cor}]"
...
function is_folder_pkg_() {
  ...
  if [[ -z "$folder" || ! -d "$folder" ]]; then
    print " fatal: not a folder: $folder" >&2
    return 1;
  fi
  ...
  print " fatal: not a valid project folder, missing ${files[*]}: $folder" >&2
  return 1;
}
...
function find_pkg_folder_() {
  ...
  if command -v realpath &>/dev/null; then
    folder="$(realpath -- "$folder_arg" 2>/dev/null)"
  else
    folder="${folder_arg:A}"
  fi
...
function find_git_folder_() {
  ...
  if [[ -f "$folder" ]]; then
    # find the git repository root folder that contains this file
    local found_git="$(find_up_ ".git" "$folder")"
    folder="$found_git"
  fi
...
# r - make sure it's the root folder of the git repository
# ro - only check for the ".git" folder, not the git work tree
function is_folder_git_() {
  set +x
  eval "$(parse_flags_ "$0" "ro" "" "$@")"
  ...
  if [[ ! -d "$folder" ]]; then
    print " fatal: not a git repository (or any of the parent directories): $folder" >&2 
    return 1;
  fi
...
function determine_target_branch_() {
  set +x
  eval "$(parse_flags_ "$0" "adbmf" "" "$@")"
  (( determine_target_branch_is_debug )) && set -x

  local branch="$1"
  local extra_branch="$2"
  local folder="$(find_git_folder_ "$3" 2>/dev/null)"
  ...
  if (( determine_target_branch_is_a || determine_target_branch_is_d )); then
    default_branch="$(get_default_branch_ "$folder" 2>/dev/null)"
  fi

  if (( determine_target_branch_is_a || determine_target_branch_is_b )); then
    base_branch="$(get_base_branch_ "$my_branch" "$folder" 2>/dev/null)"
  fi

  if (( determine_target_branch_is_a || determine_target_branch_is_m )); then
    main_branch="$(get_main_branch_ "$folder" 2>/dev/null)"
  fi

  local output="$(printf "%s\n" "$pump_merge" "$gh_merge_base" "$vscode_merge_base" "$gk_merge_target" "$default_branch" "$base_branch" "$main_branch" "$extra_branch" 2>/dev/null | grep -v '^$' | sort -u)"
  local branches_sorted=("${(@f)output}")

  local branches=()

  if [[ -n "$branch" ]]; then
    # check that none of the branches is the same as $branch
    local b=""
    for b in "${branches_sorted[@]}"; do
      if [[ "$b" != "$branch" ]]; then
        branches+=("$b")
      fi
    done
  ...
  if [[ -z "$branches" ]]; then
    local list_branches="$(list_branches_ -ac "" "$folder" 2>/dev/null)"

    local b=""
    for b in "${(@f)list_branches}"; do
      if [[ "$b" != "$branch" ]]; then
        branches+=("$b")
      fi
    done
  ...
  if (( determine_target_branch_is_f )); then
    if [[ -z "$branches" ]]; then
      print " fatal: no branches found to select as target" >&2
      return 1;
    fi
  else
  ...
  while true; do
    ...
    if (( determine_target_branch_is_f )); then
      selected_branch="$(choose_one_ -ia "$label" "${branches[@]}")"
    else
    ...
    local remote_branch="$(get_remote_branch_ "$selected_branch" "$folder")"
    ...
    local truncated_branch="$(truncate_ "$selected_branch")"

    print " ${red_cor}target branch not found remotely: $truncated_branch${reset_cor}" >&2
...
function get_local_branch_() {
  ...
  local short_branch="$(get_short_name_ "$branch" "$folder" "$remote_name")"
...
function get_main_branch_() {
  ...
  folder="$(find_git_folder_ "$folder")"
  if [[ -z "$folder" ]]; then return 1; fi
...
function is_commit_() {
  ...
  if git -C "$folder" cat-file -e "$commit" &>/dev/null; then
    # echo "1" # 1 means true when put into a variable
    return 0;
  fi
}
...
function get_long_name_() {
  ...
  local short_base_branch="$(get_short_name_ "$branch" "$folder" "$remote_name")"

  echo "${remote_name}/${short_base_branch}"
}
...
function is_valid_base_branch_() {
  local branch_1="$1"
  local branch_2="$2"
  local remote_name="$3"
  ...
  local short_branch_1="$(get_short_name_ "$branch_1" "" "$remote_name")"
  local short_branch_2="$(get_short_name_ "$branch_2" "" "$remote_name")"

  if [[ "$short_branch_1" == "$short_branch_2" ]]; then
    return 1;
  fi

  if ! get_local_branch_ "$branch_1" "" "$remote_name" &>/dev/null && ! get_remote_branch_ "$branch_1" "" "$remote_name" &>/dev/null; then
    return 1;
  fi
...
function get_base_branch_() {
  ...
  if [[ -z "$base_branch" && -n "$branch" ]]; then
    git -C "$folder" fetch --all --prune --quiet &>/dev/null || true

    local base_branch="$(git -C "$folder" config --get branch.$branch.pump-merge)"

    if ! is_valid_base_branch_ "$base_branch" "$branch" "$remote_name"; then
      ...
      if ! is_valid_base_branch_ "$base_branch" "$branch" "$remote_name"; then
        ...
        if ! is_valid_base_branch_ "$base_branch" "$branch" "$remote_name"; then
            base_branch="$(git -C "$folder" config --get branch.$branch.merge)"
          fi
        fi
      fi
    fi
...
function select_branch_() {
  set +x
  eval "$(parse_flags_ "$0" "" "alrixscjoqey" "$@")"
  ...
  local filtered_branches="$(list_branches_ "$search_arg" "$folder" "${@:4}")"
  ...
  if (( select_branch_is_e )) && [[ "$search_arg" == "${(@f)filtered_branches}" ]]; then
    branch_choice="$(choose_one_ -i "$header" "${(@f)filtered_branches}")"
  elif (( select_branch_is_i )) && [[ -n "$search_arg" ]]; then
    branch_choice="$(choose_one_ -i "$header" "${(@f)filtered_branches}")"
  else
    branch_choice="$(choose_one_ "$header" "${(@f)filtered_branches}")"
  fi
...
function select_branches_() {
  ...
  if (( select_branches_is_e )) && [[ "$search_arg" == "${(@f)filtered_branches}" ]]; then
    branch_choices="$(choose_multiple_ -i "$header" "${(@f)filtered_branches}")"
  elif (( select_branches_is_i )) && [[ -n "$search_arg" ]]; then
    branch_choices="$(choose_multiple_ -i "$header" "${(@f)filtered_branches}")"
  else
    branch_choices="$(choose_multiple_ "$header" "${(@f)filtered_branches}")"
  fi
...
function list_branches_() {
  set +x
  eval "$(parse_flags_ "$0" "alrixscjoqey" "" "$@")"
  (( list_branches_is_debug )) && set -x
  ...
  if (( list_branches_is_c )); then # co command
  ...
  if (( list_branches_is_s )); then
    local b=""
    for b in "${BRANCHES[@]}"; do
      branches_excluded+=("$b")
      branches_excluded+=("${remote_name}/${b}")
    done
  fi

  if (( list_branches_is_y )); then
    local my_b="$(get_my_branch_ -l "$folder" 2>/dev/null)"
    local my_b_short="$(get_my_branch_ "$folder" 2>/dev/null)"

    branches_excluded+=("$my_b")
    branches_excluded+=("$my_b_short")
  fi

  local filtered_branches=()

  if (( list_branches_is_j )) || [[ -n "$branches_excluded" ]]; then
...
function get_from_package_json_() {
  ...
  if command -v realpath &>/dev/null; then
    real_file="$(realpath -- "${folder}" 2>/dev/null)"
  else
    real_file="${folder:A}"
  fi
  ...
  real_file="${real_file}/package.json"
  ...
  for key in "${keys[@]}"; do
    ...
    case "$key" in
      ...
      PUMP_GHA_DEPLOY)
        ...
        ;;
      PUMP_PR_LABELS_IGNORE)
        PUMP_PR_LABELS_IGNORE[$i]="$value"
        ;;
      PUMP_GO_BACK)
        PUMP_GO_BACK[$i]="$value"
        ;;
      PUMP_ENV_LIST)
        PUMP_ENV_LIST[$i]="$value"
        ;;
      PUMP_VERSION_WEB)
...
function load_project_config_() {
  ...
  if [[ -n "$error_msgs" ]]; then
    ...
    print " ${gray_cor}edit file: $PUMP_CONFIG_FILE then run: ${hi_yellow_cor}refresh${reset_cor}" 2>/dev/tty
  fi
}
...
function del_file_() {
  ...
  if (( count <= 3 )) && [[ "${file:t}" != ".DS_Store" ]] && (( ! del_file_is_f )); then;
    if (( ! del_file_is_x )) || [[ "$type" == "folder" && -n "$(ls -A -- "$file" 2>/dev/null)" ]]; then
...
function del_files_() {
  ...
  for file in "${files[@]}"; do
    ...
    if [[ -n "$file" || -n "$a_file" ]]; then
      if (( del_files_is_x )); then
        del_file_ -fx -- "$file"
      elif (( delete_all || del_files_is_f )); then
        del_file_ -f -- "$file"
      else
...
function del() {
  ...
  if [[ -z "$1" ]]; then
    ...
    if (( ${#files[@]} > 1 )); then
      local files_chosen=""
      files_chosen="$(choose_multiple_ -- "files to delete" "${files[@]}")"
      if (( $? == 130 )); then return 130; fi
      files=("${(@f)files_chosen}")
    fi
  else
...
function format_() {
  ...
  if [[ -z "$CURRENT_PUMP_PKG_MANAGER" ]]; then
    print " fatal: missing package manager" >&2
    return 1;
  fi

  local pkg_manager_run="${CURRENT_PUMP_PKG_MANAGER}$([[ $CURRENT_PUMP_PKG_MANAGER == "yarn" ]] && echo "" || echo " run")"
  ...
  if [[ -n "$_prettierfix" ]]; then
    script="$pkg_manager_run prettier:fix"
  elif [[ -n "$_formatfix" ]]; then
    script="$pkg_manager_run format:fix"
  elif [[ -n "$_prettier" ]]; then
    script="$pkg_manager_run prettier"
  elif [[ -n "$_format" ]]; then
    script="$pkg_manager_run format"
  else
...
function lint_() {
  ...
  if [[ -n "$_lintfix" ]]; then
    script="$pkg_manager_run lint:fix"
  elif [[ -n "$_eslintfix" ]]; then
    script="$pkg_manager_run eslint:fix"
  elif [[ -n "$_lint" ]]; then
    script="$pkg_manager_run lint"
  elif [[ -n "$_eslint" ]]; then
    script="$pkg_manager_run eslint"
  else
...
function covc_() {
  ...
  if [[ -z "$proj_repo" ]]; then
    ...
    print " ${gray_cor}edit file: $PUMP_CONFIG_FILE then run: ${hi_yellow_cor}refresh${reset_cor}" >&2
    return 1;
  fi
...
function te() {
  ...
  if (( te_is_h )); then
    print "  ${hi_yellow_cor}te ${yellow_cor}[<folder>]${reset_cor} : run tests configured by PUMP_TEST"
    print "  ---"
    print "  ${gray_cor}edit file: $PUMP_CONFIG_FILE to customize PUMP_TEST${reset_cor}"
    return 0;
  fi
  ...
  if [[ -n "$CURRENT_PUMP_TEST" ]]; then
    test_script="$CURRENT_PUMP_TEST"
  else
    local pkg_manager_run="${CURRENT_PUMP_PKG_MANAGER}$([[ $CURRENT_PUMP_PKG_MANAGER == "yarn" ]] && echo "" || echo " run")"

    test_script="$pkg_manager_run $(get_from_package_json_ "scripts.test" "$PWD" 2>/dev/null)"
  fi

  print " ${script_cor}$test_script${reset_cor} on ${cyan_cor}$folder${reset_cor}"

  ( cd "$folder" && eval "$test_script" "$@")
  local RET=$?
...
function tew() {
  ...
  if (( tew_is_h )); then
    print "  ${hi_yellow_cor}tew${reset_cor} : run PUMP_TEST_WATCH"
    return 0;
  fi
...
function reset1() {
  ...
  git -C "$folder" --no-pager log --use-mailmap --oneline --no-decorate -1 || true
...
function reset2() {
  ...
  git -C "$folder" --no-pager log --use-mailmap --oneline --no-decorate -2 || true
...
function reset3() {
  ...
  git -C "$folder" --no-pager log --use-mailmap --oneline --no-decorate -3 || true
...
function reset4() {
  ...
  git -C "$folder" --no-pager log --use-mailmap --oneline --no-decorate -4 || true
...
function reset5() {
  ...
  git -C "$folder" --no-pager log --use-mailmap --oneline --no-decorate -5 || true
...
function read_commits_() {
  ...
  git -C "$folder" log --no-graph --oneline --no-merges -100 --reverse --pretty=format:'%H%x1F%s%x00' \
  ...
  if (( read_commits_is_t )); then
    ...
    echo "${pr_title_jira_key}${TAB}${pr_title_rest}${TAB}${count}"
  fi
}
...
function extract_jira_key_() {
  set +x
  eval "$(parse_flags_ "$0" "" "" "$@")"
  # (( extract_jira_key_is_debug )) && set -x
...
function pr() {
  set +x
  eval "$(parse_flags_ "$0" "tlsbdecrxu" "f" "$@")"
  (( pr_is_debug )) && set -x

  if (( pr_is_h )); then
    print "  ${hi_yellow_cor}pr ${yellow_cor}[<title>] [<folder>]${reset_cor} : create pull request"
    print "  ${hi_yellow_cor}  -t${reset_cor} : run tests before creating pull request (branch must be clean)"
    print "  ${hi_yellow_cor}  -x${reset_cor} : skip jira status transition"
    print "  ${hi_yellow_cor}  -u${reset_cor} : update pull request if it already exists"
    print "  ${hi_yellow_cor}  -f${reset_cor} : skip confirmation"
    print "  ---"
    print "  ${hi_yellow_cor}pr -l ${yellow_cor}[<title>] [<folder>] [<label...>]${reset_cor} : create pull request with labels"    print "  ${hi_yellow_cor}  -lb${reset_cor} : set label type: bug or bugfix"
  ...
  if (( prs_is_l )); then
    eval "$(parse_args_ "$0" "title:to,folder:fz,labels:a:to" "$@")"
  else
  ...
  local repo_name="$(get_repo_name_ "$folder")"
  ...
  if [[ -z "$my_remote_branch" ]]; then
    ...
    if ! push "$folder"; then return 1; fi
  fi
  ...
  local pr_number="" pr_title="" pr_url="" pr_link="" pr_link_full=""

  output=""
  output="$(get_pr_ -o "$my_branch" "$proj_repo")"
  if (( $? == 0 )); then
    IFS=$TAB read -r pr_number pr_title pr_url _ <<< "$output"
  fi

  if (( ! pr_is_u )) && [[ -n "$pr_number" ]]; then
    pr_link=$'\e]8;;'"$pr_url"$'\a'"$pr_number"$'\e]8;;\a'
    pr_link_full="${blue_cor}$pr_link${reset_cor} ${gray_cor}$pr_title${reset_cor}"

    gh pr view  --repo "$proj_repo" --web $pr_url &>/dev/null
    print " pull request is up: ${blue_cor}$pr_link_full${reset_cor}" >&2
    print " run: ${hi_yellow_cor}pr -u${reset_cor} to update the pull request" >&2
    return 0;
  fi
  ...
  if [[ -z "$target_branch" ]]; then
    target_branch="$(determine_target_branch_ -dbm "$my_branch" "" "$folder" "$@")"
    if (( $? == 130 )); then return 130; fi
    if [[ -z "$target_branch" ]]; then
      print " run: ${hi_yellow_cor}pr -h${reset_cor} to see usage" >&2
      return 1;
  ...
  local jira_key="$(extract_jira_key_ "$my_branch" "$folder")"
  local jira_title=""
  ...
  if [[ "$jira_key" == "$(read_pump_value_ "JIRA_KEY" "$folder")" ]]; then
    jira_title="$(read_pump_value_ "JIRA_TITLE" "$folder")"
  fi

  local commit_key="" commit_title="" count=""
  output=""
  output="$(read_commits_ -t "$target_branch" "$my_branch" "$folder")"
  if (( $? == 0 )); then
    IFS=$TAB read -r commit_key commit_title count <<< "$output"
  fi
  ...
  if (( is_read_commits )) && [[ -n "$commit_title" ]]; then
    if [[ -z "$jira_title" ]] || (( count == 1 && uncommitted == 1 )); then
  ...
  if (( ! pr_is_f )); then
    if [[ -n "$pr_number" && -n "$pr_title" ]]; then
      pr_title="$(input_type_mandatory_ -a "pull request title" "" 255 "$pr_title")"
    else
      pr_title="$(input_type_mandatory_ "pull request title" "" 255 "$pr_title")"
    fi
    if (( $? == 130 || $? == 2 )); then return 130; fi
  fi
  ...
  if [[ -n "$pr_title" ]]; then
    ...
    print " ${purple_cor}pull request title:${reset_cor} (auto fill)" >&2
  fi
  ...
  # pr -l
  if (( pr_is_l )); then
    label="$(gh api repos/$repo_name/labels --paginate --jq '.[].name' 2>/dev/null | sort -rf)"
    all_labels=("${(@f)label}")
  fi
  ...
  if [[ -n "$pr_labels" ]]; then
    print " ${purple_cor}pr labels:${reset_cor} $pr_labels"
  fi

  local pr_body=""

  local copilot_request=$'
    Generate a GitHub pull request description.

    Output ONLY the PR body in GitHub Markdown.

    DO:
    - use the pull request template if it exists
    - include the jira key number in the description where indicated in the template if it exists

    Do NOT:
    - include the title in the output
    - include links to files in the repository
    - explain what you are doing
    - say "Here is the PR description"
    - include any preamble or commentary
    - wrap the output in code fences

    Begin immediately
  '

  if command -v gum &>/dev/null && command -v copilot &>/dev/null; then
    pr_body="$(gum spin --title="writing descripton using copilot..." -- copilot -p "$copilot_request" --allow-all --silent)"
  elif command -v copilot &>/dev/null; then
    pr_body="$(copilot -p "$copilot_request" --allow-all --silent 2>/dev/null)"
  fi

  if [[ -z "$pr_body" ]] && [[ -n "$pump_pr_template_file" && -f "$pump_pr_template_file" ]]; then

    if [[ -z "$pr_body" ]]; then
      if (( is_read_commits )); then
        local commit_messages="$(read_commits_ "$target_branch" "$my_branch" "$folder")"
    ...
    if (( ! pr_is_f )) && [[ -z "$pump_pr_replace" ]]; then
      ...
      if (( i )) && [[ -n "$pr_replace" ]]; then
        ...
        update_config_ -d $i "PUMP_PR_REPLACE" "$pr_replace"
  ...
  if [[ -n "$pr_body" ]]; then
    print " ${purple_cor}pull request body:${reset_cor} $pr_body" >&2
  else
    print " ${purple_cor}pull request body:${reset_cor} (auto fill)" >&2
  fi
  ...
  if [[ -z "$pr_number" ]]; then
    if gh pr create --repo "$proj_repo" --assignee="@me" --title="$pr_title" --body="$pr_body" --head="$my_branch" --base="$short_base_branch" --label="$pr_labels" ${pr_flags[@]}; then
      if (( ! pr_is_x )); then
        if (( pr_is_f )); then
    ...
    if confirm_ "update existing pull request $pr_url ?"; then
      if gh pr edit $pr_number --repo "$proj_repo" --title="$pr_title" --body="$pr_body" --base="$short_base_branch"; then
        if (( ! pr_is_x )); then
          if (( pr_is_f )); then
            update_jira_status_ -rf $i "$jira_key" "$jira_in_review"
          else
...
function run() {
  ...
  if (( run_is_h )); then
    if [[ -n "$CURRENT_PUMP_SHORT_NAME" ]]; then
      if [[ -n "$CURRENT_PUMP_RUN_DEV" ]]; then
        print "  ${hi_yellow_cor}run [<folder>]${reset_cor} : run PUMP_RUN_DEV script from ${CURRENT_PUMP_SHORT_NAME}'s config file"
      else
        print "  ${hi_yellow_cor}run [<folder>]${reset_cor} : run dev or start script from ${CURRENT_PUMP_SHORT_NAME}'s package.json"
      fi
      print "  ${hi_yellow_cor}run <script> ${yellow_cor}[<folder>]${reset_cor} : run any script from ${CURRENT_PUMP_SHORT_NAME}'s package.json"
    else
      print "  ${hi_yellow_cor}run [<folder>]${reset_cor} : run dev or start script from package.json"
      print "  ${hi_yellow_cor}run <script> ${yellow_cor}[<folder>]${reset_cor} : run any script from package.json"
    fi
    if [[ -n "$CURRENT_PUMP_RUN_STAGE" ]]; then
      print "  ${hi_yellow_cor}run stage [<folder>]${reset_cor} : run PUMP_RUN_STAGE script from ${CURRENT_PUMP_SHORT_NAME}'s config file"
    fi
    if [[ -n "$CURRENT_PUMP_RUN_QA" ]]; then
      print "  ${hi_yellow_cor}run qa [<folder>]${reset_cor} : run PUMP_RUN_QA script from ${CURRENT_PUMP_SHORT_NAME}'s config file"
    fi
    if [[ -n "$CURRENT_PUMP_RUN_PROD" ]]; then
      print "  ${hi_yellow_cor}run prod [<folder>]${reset_cor} : run PUMP_RUN_PROD script from ${CURRENT_PUMP_SHORT_NAME}'s config file"
    fi
    print "  ---"
    print "  ${gray_cor}edit file: $PUMP_CONFIG_FILE for more customization${reset_cor}"
    return 0;
  fi
  ...
  eval "$(parse_args_ "run" "script:to,folder:to" "$@")"
  shift $arg_count

  local full_path_to_execute=""

  if [[ -n "$folder" ]]; then
    if [[ -d "${PWD}/${folder}" ]]; then
      full_path_to_execute="${PWD}/${folder}"
    else
    ...
    full_path_to_execute="${PWD}"
  fi

  proj_run_execute_ 0 "$script" "$full_path_to_execute"
}
...
function proj_run_() {
  ...
  eval "$(parse_args_ "$proj_cmd run" "script:to,folder:to" "$@")"
  shift $arg_count
  ...
  if (( single_mode )); then
    full_path_to_execute="${proj_folder}/${folder}"
  else
    full_path_to_execute="$(proj_handler_folder_ $i "$folder")"
    if [[ -z "$full_path_to_execute" ]]; then return 1; fi
  fi

  proj_run_execute_ $i "$script" "$full_path_to_execute"  
}

function proj_run_execute_() {
  set +x
  eval "$(parse_flags_ "$0" "x" "" "$@")"
  (( proj_run_execute_is_debug )) && set -x

  local i="$1"
  local script="$2"
  local full_path_to_execute="$3"

  if command -v realpath &>/dev/null; then
    full_path_to_execute="$(realpath -- "$full_path_to_execute" 2>/dev/null)"
  else
    full_path_to_execute="${full_path_to_execute:A}"
  fi

  if ! is_folder_pkg_ "$full_path_to_execute"; then
    if (( i )); then
      print " run: ${hi_yellow_cor}$proj_cmd run -h${reset_cor} to see usage" >&2
    else
      print " run: ${hi_yellow_cor}run -h${reset_cor} to see usage" >&2
    fi
    return 1;
  ...
  if (( i )); then
    if ! check_proj_ -mp $i; then return 1; fi

    proj_cmd="${PUMP_SHORT_NAME[$i]}"
    single_mode="${SINGLE_MODE[$i]}"
    pkg_manager="${PUMP_PKG_MANAGER[$i]}"

    if [[ "$script" == "dev" ]]; then
      pump_run="${PUMP_RUN_DEV[$i]}"
      pump_script="PUMP_RUN_DEV"
    elif [[ "$script" == "stage" ]]; then
      pump_run="${PUMP_RUN_STAGE[$i]}"
      pump_script="PUMP_RUN_STAGE"
    elif [[ "$script" == "prod" ]]; then
      pump_run="${PUMP_RUN_PROD[$i]}"
      pump_script="PUMP_RUN_PROD"
    elif [[ "$script" == "qa" ]]; then
      pump_run="${PUMP_RUN_QA[$i]}"
      pump_script="PUMP_RUN_QA"
    fi

  else
    if ! check_proj_ -mp 0 "$full_path_to_execute"; then return 1; fi
    ...
    if [[ "$script" == "dev" ]]; then
      pump_run="$CURRENT_PUMP_RUN_DEV"
      pump_script="PUMP_RUN_DEV"
    elif [[ "$script" == "stage" ]]; then
      pump_run="$CURRENT_PUMP_RUN_STAGE"
      pump_script="PUMP_RUN_STAGE"
    elif [[ "$script" == "prod" ]]; then
      pump_run="$CURRENT_PUMP_RUN_PROD"
      pump_script="PUMP_RUN_PROD"
    elif [[ "$script" == "qa" ]]; then
      pump_run="$CURRENT_PUMP_RUN_QA"
      pump_script="PUMP_RUN_QA"
    fi
  fi

  if [[ -n "$pump_script" && -z "$pump_run" ]]; then
    confirm_ "customize $pump_script script?"
    local _RET=$?
    
    if (( _RET == 130 || _RET == 2 )); then return 130; fi
    if (( _RET == 0 )); then
      local custom_setup=""
      custom_setup="$(write_from_ "type your custom script")"
      if (( $? == 130 )); then return 130; fi

      if [[ -n "$custom_setup" ]]; then
        update_config_ -d $i "$pump_script" "$custom_setup"
        pump_run="$custom_setup"
      fi
    else
      update_config_ -d $i "$pump_script" "0"
      pump_run="0"
    fi
  fi
  ...
  if [[ -n "$pump_run" && "$pump_run" != "0" ]]; then
    run_script="$pump_run"

  else
    local pump_run_script="$(get_from_package_json_ "scripts.$script" "$full_path_to_execute" 2>/dev/null)"

    if [[ -z "$pkg_manager" ]]; then
      print " ${red_cor}fatal: missing package manager in project: $full_path_to_execute${reset_cor}" >&2
      return 1;
    fi

    if [[ -n "$pump_run_script" ]]; then
      run_script="$pkg_manager run $script"
    else
      run_script="$pkg_manager start"
    fi
  fi

  print " ${script_cor}$run_script${reset_cor} on ${cyan_cor}$full_path_to_execute${reset_cor}"

  ( cd "$full_path_to_execute" && eval "$run_script" )
  
  if (( $? != 0 )); then
    if (( i )) && [[ "$run_script" == "$pump_run" ]]; then
...
function setup() {
  ...
  if (( setup_is_h )); then
    if [[ -n "$CURRENT_PUMP_SHORT_NAME" ]]; then
      if [[ -n "$CURRENT_PUMP_SETUP" && "$CURRENT_PUMP_SETUP" != "0" ]]; then
        print "  ${hi_yellow_cor}setup [<folder>]${reset_cor} : run PUMP_SETUP script from ${CURRENT_PUMP_SHORT_NAME}'s config file"
      else
        print "  ${hi_yellow_cor}setup [<folder>]${reset_cor} : run setup or install script from ${CURRENT_PUMP_SHORT_NAME}'s package.json"
      fi
    else
      print "  ${hi_yellow_cor}setup [<folder>]${reset_cor} : run setup or install script from package.json"
    fi
    print "  ---"
    print "  ${gray_cor}edit file: $PUMP_CONFIG_FILE to customize PUMP_SETUP${reset_cor}"
    return 0;
  fi

  local folder=""

  eval "$(parse_args_ "setup" "folder:to" "$@")"
  shift $arg_count
  ...
  proj_setup_execute_ 0 "$full_path_to_execute"
}
...
function proj_setup_() {
  ...
  eval "$(parse_args_ "$proj_cmd setup" "folder:to" "$@")"
  shift $arg_count
  ...
  proj_setup_execute_ $i "$full_path_to_execute"
}

function proj_setup_execute_() {
  set +x
  eval "$(parse_flags_ "$0" "" "" "$@")"
  (( proj_setup_execute_is_debug )) && set -x

  local i="$1"
  local full_path_to_execute="$2"
  ...
  if ! is_folder_pkg_ "$full_path_to_execute"; then
    if (( i )); then
      print " run: ${hi_yellow_cor}$proj_cmd setup -h${reset_cor} to see usage" >&2
    else
      print " run: ${hi_yellow_cor}setup -h${reset_cor} to see usage" >&2
    fi
    return 1;
  ...
  if [[ -z "$pump_setup" ]]; then
    confirm_ "customize setup script?"
    local _RET=$?
    ...
    if (( _RET == 0 )); then
      ...
      if [[ -n "$custom_setup" ]]; then
        update_config_ -d $i "PUMP_SETUP" "$custom_setup"
        pump_setup="$custom_setup"
      fi
    else
      update_config_ -d $i "PUMP_SETUP" "0"
      pump_setup="0"
    fi
  fi
  ...
  if [[ -n "$pump_setup" && "$pump_setup" != "0" ]]; then
    setup_script="$pump_setup"

  else
    setup_script="$(get_from_package_json_ "scripts.setup" "$full_path_to_execute" 2>/dev/null)"
    ...
    if [[ -n "$setup_script" ]]; then
      setup_script="$pkg_manager run setup"
    else
      setup_script="$pkg_manager install"
    fi
  fi

  print " ${script_cor}$setup_script${reset_cor} on ${cyan_cor}$full_path_to_execute${reset_cor}"

  ( cd "$full_path_to_execute" && eval "$setup_script" )
  
  if (( $? != 0 )); then
    if (( i )) && [[ "$setup_script" == "${PUMP_SETUP[$i]}" ]]; then
  ...
  local run_dev="$(get_from_package_json_ "scripts.dev" "$full_path_to_execute" 2>/dev/null)"
  local run_start="$(get_from_package_json_ "scripts.start" "$full_path_to_execute" 2>/dev/null)"
  ...
  if [[ -f $pkg_json && -n "$pkg_manager" ]]; then
    ...
    for entry in "${(f)scripts}"; do
      ...
      if [[ "$name" == "test" && -n "$cmd" ]]; then print "  • ${hi_yellow_cor}te${reset_cor} (alias for \"$pkg_manager test\")"; fi
...
function proj_revs_() {
  ...
  local rev_folder=""
  for rev_folder in "${rev_options[@]}"; do
    local pr_number="$(read_pump_value_ "PR_NUMBER" "$rev_folder")"
    local pr_title="$(read_pump_value_ "PR_TITLE" "$rev_folder")"
    local pr_branch="$(read_pump_value_ "PR_BRANCH" "$rev_folder")"

    local rev_name="${rev_folder##*/}"
    rev_name="${rev_name#rev.}"

    if (( proj_revs_is_b )); then
      if [[ -n "$pr_branch" ]]; then
        rev_map+=("0${TAB}${rev_name}${TAB}${pr_branch}")
        rev_choices+=("$rev_name")

      elif [[ -n "$pr_number" && -n "$pr_title" && -n "$pr_branch" ]]; then
  ...
  if [[ "$select_pr_number" == "0" ]]; then
    proj_rev_ -xp "$proj_cmd" "$select_pr_branch_or_folder"
  else
...
function attempt_switch_branch_() {
  ...
  if [[ "$branch" != "$local_branch" ]]; then
    print " current branch is not correct: ${yellow_cor}$local_branch${reset_cor}" >&2
...
function clone_repo_() {
  ...
  if [[ -z "$repo" || -z "$folder" ]]; then return 1; fi
  ...
  if (( ! clone_repo_is_o )); then # overwrite
    if ! is_proj_folder_empty_ "$folder"; then return 1; fi
  fi
  ...
  if [[ "$folder" == *"/.temp/"* ]]; then
    header="cloning... ${repo_name:-$repo}"
  else
    header="cloning... ${repo_name:-$repo} into ${cyan_cor}$folder${reset_cor}"
  fi
  ...
  if ! is_folder_git_ "$folder" &>/dev/null; then
    print " ${red_cor}fatal: failed to clone repository: $repo${reset_cor}" >&2
    return 1;
  fi
}

function print_pr_rev_status_() {
  local pr_state="$1"
  local pr_url="$2"
  ...
  if [[ -n "$pr_state" && -n "$pr_url" ]]; then
    if [[ "$pr_state" == "OPEN" ]]; then
      print " ${cyan_cor}pull request is open${reset_cor}: ${blue_cor}$pr_url${reset_cor}" >&2
    else
      print " ${orange_cor}pull request is ${pr_state:l}${reset_cor}: ${blue_cor}$pr_url${reset_cor}" >&2
    fi
  else
    print " ${red_cor}pull request is not available${reset_cor}" >&2
  fi
}

function proj_rev_() {
  set +x
  eval "$(parse_flags_ "$0" "ebjdxpr" "" "$@")"
  ...
  if (( proj_rev_is_x )); then
    ...
    if (( proj_rev_is_p )); then
      full_rev_folder="${revs_folder}/rev.${branch}"
      branch_user="$(get_my_branch_ "$full_rev_folder")"
    else
      branch_user="$branch"
    fi
  ...
  # proj_rev_ -b select branch_user
    ...
    if [[ -z "$branch_user" ]]; then
      if command -v gh &>/dev/null; then
        local output="" pr_branch=""
        output="$(select_pr_ -od "$branch" "$proj_repo" "pull request to review")"
        if (( $? == 130 )); then return 130; fi
        if (( $? == 0 )); then
          IFS=$TAB read -r pr_number pr_title pr_url pr_branch pr_state _ <<< "$output"
        fi
  ...
  if [[ -z "$pr_number" || -z "$pr_title" || -z "$pr_url" ]]; then
    ...
    if [[ -z "$pr_number" ]]; then
      pr_number="$(read_pump_value_ "PR_NUMBER" "$full_rev_folder" "$branch_user")"
      pr_title="$(read_pump_value_ "PR_TITLE" "$full_rev_folder" "$branch_user")"
  ...
  if [[ -n "$pr_link" ]]; then
    local pr_link_full="${blue_cor}$pr_link${reset_cor} ${gray_cor}$pr_title${reset_cor}"

    print " opening pull request... $pr_link_full"
  else
  ...
  if is_folder_git_ "$full_rev_folder" &>/dev/null; then
    ...
    if (( do_nothing )); then
      print_pr_rev_status_ "$pr_state" "$pr_url"
      cd "$full_rev_folder"
      return 1;
    fi
    ...
    if ! clone_repo_ -bo "$proj_repo" "$full_rev_folder"; then
      return 1;
    fi
    ...
    if command -v gh &>/dev/null; then
      local pr_target_branch="$(gh pr view "$branch_user" --repo "$proj_repo" --json state,baseRefName --jq 'select(.state == "OPEN") | .baseRefName' 2>/dev/null)"
      if [[ -n "$pr_target_branch" ]]; then
  ...
  proj_setup_execute_ $i "$full_rev_folder"
  ...
  if [[ -z "$PUMP_CODE_EDITOR" ]]; then
    ...
    if [[ -n "$PUMP_CODE_EDITOR" ]]; then
      ...
      update_setting_ -fd "PUMP_CODE_EDITOR" "$PUMP_CODE_EDITOR"
    fi
  fi
...
function get_pr_url_() {
  ...
  if command -v gh &>/dev/null; then
    if (( get_pr_url_is_o )); then
      pr_url="$(gh pr view "$branch" --repo "$proj_repo" --json url,state --jq 'select(.state == "OPEN") | .url' 2>/dev/null)"
    fi

    if [[ -z "$pr_url" ]]; then
      if (( get_pr_url_is_m )); then
        pr_url="$(gh pr view "$branch" --repo "$proj_repo" --json url,state --jq 'select(.state == "MERGED") | .url' 2>/dev/null)"
      fi
      if [[ -z "$pr_url" ]] && (( get_pr_url_is_c )); then
        pr_url="$(gh pr view "$branch" --repo "$proj_repo" --json url,state --jq 'select(.state == "CLOSED") | .url' 2>/dev/null)"
      fi
    fi
  fi
...
function proj_clone_() {
  ...
  if (( single_mode )); then
    ...
    if [[ -z "$git_folder" ]]; then
      ...
      if ! clone_repo_ -bo "$proj_repo" "$temp_folder"; then        
        return 1;
      fi
      ...
      if [[ -n "$branch" ]]; then
        ...
        if [[ "$branch" != "$first_branch" ]]; then
          clone_repo_ -o "$proj_repo" "${proj_folder}/${first_branch}"
        fi
  ...
  if is_folder_git_ "$folder_to_clone" &>/dev/null; then
    if [[ -n "$branch" ]]; then
      ...
      if [[ -n "$base_branch" ]]; then
        print " ${purple_cor}target branch: ${hi_cyan_cor}$base_branch${reset_cor}"
      fi
      
      local remote_name="$(get_remote_name_ "$folder_to_clone")"

      if is_branch_existing_remote_ "$branch" "$folder_to_clone"; then
        print " ${purple_cor}opened branch: ${hi_green_cor}$branch${reset_cor} from $remote_name"
      else
        print " ${purple_cor}opened branch: ${pink_cor}$branch${reset_cor} but not in $remote_name"
      fi
    fi

    print_clone_help_ $i "$folder_to_clone"

    if ! cd "$folder_to_clone"; then
      print " ${red_cor}fatal: failed to cd into folder: ${hi_cyan_cor}$folder_to_clone${reset_cor}" >&2
      return 1;
    fi
  ...
  if ! clone_repo_ -o "$proj_repo" "$folder_to_clone"; then
    return 1;
  fi
  ...
  if [[ -n "$branch" && "$first_branch" != "$target_branch_user" && "$branch" != "$first_branch" && "$branch" != "main" && "$branch" != "master" ]]; then
    ...
    if [[ -z "$target_branch_user" ]]; then
      ...
      if [[ -z "$target_branch_user" ]] && ! is_branch_existing_remote_ "$branch" "$folder_to_clone"; then
        target_branch_user="$(determine_target_branch_ -dbm "$branch" "$first_branch" "$folder_to_clone" "$@")"
  ...
  if [[ -n "$target_branch_user" ]]; then
    if is_branch_existing_remote_ "$target_branch_user" "$folder_to_clone"; then
      if ! git -C "$folder_to_clone" switch "$target_branch_user" &>/dev/null; then
  ...
  if [[ -n "$branch" ]]; then
    ...
    if [[ -n "$target_branch_user" && "$target_branch_user" != "$branch" ]]; then
      ...
      if [[ -z "$existing_target_branch" ]] && [[ "$target_branch_user" != "$target_branch" ]]; then
        print " ${purple_cor}target branch: ${yellow_cor}$target_branch_user${reset_cor} but not in $remote_name"
      else
        print " ${purple_cor}target branch: ${hi_cyan_cor}$target_branch_user${reset_cor}"
      fi
    fi

    if is_branch_existing_remote_ "$branch" "$folder_to_clone"; then
      print " ${purple_cor}pulled branch: ${hi_green_cor}$branch${reset_cor} from $remote_name"
    else
      print " ${purple_cor}forked branch: ${pink_cor}$branch${reset_cor} but not in $remote_name"
    fi
...
function print_clone_help_() {
  set +x
  eval "$(parse_simple_flags_ "$0" "" "" "$@")"
  (( print_clone_help_is_debug )) && set -x
  ...
  if [[ ! -e "${folder}/node_modules" ]]; then
    if [[ -n "${PUMP_SETUP[$i]}" ]]; then
      print "  • ${hi_yellow_cor}setup${reset_cor} (runs PUMP_SETUP_${i})"
    else
      local pkg_manager="${PUMP_PKG_MANAGER[$i]}"
      local setup_script="$(get_from_package_json_ "scripts.setup" "$folder" 2>/dev/null)"
  ...
  local b_branch="$(get_base_branch_ "" "$folder" 2>/dev/null)"

  if [[ -n "$b_branch" ]]; then
    print "  • ${hi_yellow_cor}base${reset_cor} (alias for \"git switch $b_branch\")"
    print "  • ${hi_yellow_cor}back${reset_cor} (alias for \"git switch -\")"
  fi
...
function proj_del_() {
  set +x
  eval "$(parse_simple_flags_ "$0" "af" "" "$@")"
  (( proj_del_is_debug )) && set -x

  local i="$1"
  local proj_cmd="${PUMP_SHORT_NAME[$i]}"

  if (( proj_del_is_h )); then
    proj_print_help_ "$proj_cmd del"
    return 0;
  fi
  ...
  if (( single_mode )); then
    print " fatal: cannot delete files in ${purple_cor}single mode${reset_cor}" >&2
    print " run: ${hi_yellow_cor}$proj_cmd -h${reset_cor} to see usage" >&2
    return 1;
  fi

  if [[ -z "$1" ]]; then
    print " fatal: glob argument is required" >&2
    print " run: ${hi_yellow_cor}$proj_cmd del -h${reset_cor} to see usage" >&2
    return 1;
  fi
  ...
  if (( proj_del_is_a )); then
    files=($(find "$proj_folder" -maxdepth 20 ! -type l -not -path '*/.*' 2>/dev/null))
  else
    files=($(find "$proj_folder" -maxdepth 2 ! -type l -not -path '*/.*' 2>/dev/null))
  fi

  if (( ${#files[@]} <= 1 )); then
    print " no files found in: $proj_folder" >&2
    return 1;
  fi
  ...
  for file in "${files[@]}"; do
    ...
    if (( ! proj_del_is_a )) && ! is_folder_git_ -ro "$dir" &>/dev/null; then
      continue;
    fi
    ...
    for f in "${files[@]}"; do
      ...
      if (( proj_del_is_f )); then
        del_file_ -f -- "$target"
      else
        del_file_ -- "$target" 0 "${#files[@]}"
      fi
      RET=$?
      if (( RET == 130 )); then return 130; fi
    done
  ...
  if (( dir_count == 0 )); then
    print " no git folders found in: $proj_folder" >&2
    print " run: ${hi_yellow_cor}$proj_cmd del -a${reset_cor} to include any folder" >&2
    return 1;
  fi

  if (( del_attempt == 0 )); then
    print " no files found in: $proj_folder" >&2
    return 1;
  fi
    ...
    if (( ${#folders_to_delete[@]} )); then
      ...
      if (( _RET == 0 )); then
        for folder in "${folders_to_delete[@]}"; do
          print " deleting folder: ${hi_cyan_cor}$folder${reset_cor} because its local branch no longer exists in remote" >&2
          del -fx -- "$folder"
        done
      fi
    fi
...
function proj_prs_() {
  set +x
  eval "$(parse_flags_ "$0" "aus" "xfrqdl" "$@")"
  ...
  if [[ -z "$current_user" ]]; then
    ...
    print " run: ${hi_yellow_cor}gh auth login${reset_cor} to authenticate with github" >&2
    return 1;
  fi

  if (( proj_prs_is_l + proj_prs_is_u + proj_prs_is_s + proj_prs_is_a + proj_prs_is_r > 1 )); then
    print "fatal: cannot use those flags together" >&2
  ...
  if (( proj_prs_is_a )); then
    ...
    while true; do
      local pr_list=""
      if (( proj_prs_is_qd )); then
        pr_list="$(list_prs_ -o "$search" "in:title,body" "$proj_repo")"
      else
        pr_list="$(list_prs_ -o "$search" "in:title" "$proj_repo")"
      fi
      ...
      if [[ -n "$pr_list" ]]; then
        ...
        for line in "${(@f)pr_list}"; do
          ...
          if (( approved_prs[$pr_number] )); then continue; fi

          if (( proj_prs_is_aa )); then
          ...
          approved_prs[$pr_number]=1
        done
      fi

      if (( ! proj_prs_is_aa )); then break; fi
  ...
  if (( proj_prs_is_u )); then
    if ! check_jira_ -ipss $i; then return 1; fi

    proj_prs_l_ -u $i "" "$search" "" "$@"
  ...
  if [[ -n "$search" ]]; then
    if (( proj_prs_is_qd )); then
      pr_list="$(list_prs_ -omc "$search" "in:title,body" "$proj_repo")"
    else
      pr_list="$(list_prs_ -omc "$search" "in:title" "$proj_repo")"
    fi
  else
    if (( proj_prs_is_qd )); then
      pr_list="$(list_prs_ -o "" "in:title,body" "$proj_repo")"
    else
      pr_list="$(list_prs_ -o "" "in:title" "$proj_repo")"
    fi

    if [[ -n "$pr_list" ]]; then
      print " listing all open pull requests..." >&2
    else
...
function proj_prs_l_() {
  ...
  if [[ "$jira_release" =~ ([0-9]+)(\.[0-9]+)?(\.[0-9]+)? ]]; then
    local release_version_match="${match[1]}${match[2]:-".0"}${match[3]:-".0"}"

    if (( ! proj_prs_l_is_f )); then
      local _RET=0

      if (( proj_prs_l_is_u )); then
        confirm_ "remove release version: ${pink_cor}$release_version_match${reset_cor}?"
      else
        confirm_ "use release version as label: ${pink_cor}$release_version_match${reset_cor}?"
      fi
      _RET=$?

      if (( _RET == 130 || _RET == 2 )); then return 130; fi;
      if (( _RET == 0 )); then
        choose_labels+=("$release_version_match")
        is_version_label=1
        release_version="$release_version_match"
      fi
    else
      choose_labels+=("$release_version_match")
      is_version_label=1
  ...
  if (( ! proj_prs_l_is_f && ! proj_prs_l_is_u )); then
    local _RET=0
    ...
    if (( _RET == 0 )); then
      local all_labels_out="$(gh api repos/$repo_name/labels --paginate --jq '.[].name' 2>/dev/null | sort -rf)"
  ...
  if [[ -z "${choose_labels[@]}" || " ${choose_labels[*]} " == *" <new label...> "* ]]; then
    local new_label=""
    new_label="$(input_type_ "type new label name")"
    if (( $? == 130 )); then return 130; fi
    if [[ -n "$new_label" ]]; then
      choose_labels=("${(@)choose_labels:#'<new label...>'}" "$new_label")

      if [[ -z "$release_version" && "$new_label" =~ ([0-9]+)(\.[0-9]+)?(\.[0-9]+)? ]]; then
        release_version="${match[1]}${match[2]:-".0"}${match[3]:-".0"}"
      fi
    else
      choose_labels=("${(@)choose_labels:#'<new label...>'}")
    fi
  else
    if [[ -z "$release_version" && "${choose_labels[*]}" =~ ([0-9]+)(\.[0-9]+)?(\.[0-9]+)? ]]; then
      release_version="${match[1]}${match[2]:-".0"}${match[3]:-".0"}"
    fi
  fi
  ...
  if [[ -n "${choose_labels[@]}" ]]; then
    if (( proj_prs_l_is_u )); then
      print " ${purple_cor}remove labels:${reset_cor} $choose_labels"
    else
      print " ${purple_cor}add labels:${reset_cor} $choose_labels"
    fi
  fi

  local is_remove_all_labels=0
  local is_remove_some_labels=0
  local is_remove_version_labels=0

  if (( ! proj_prs_l_is_f )); then
    confirm_ "also remove all other labels?" "yes" "no" "no"
    local _RET=$?

    if (( _RET == 130 )); then return 130; fi;
    if (( _RET == 0 )); then
      is_remove_all_labels=1
      is_remove_some_labels=1
      is_remove_version_labels=1
    else
      confirm_ "also remove any version label?" "yes" "no" "no"
      _RET=$?
      ...
      if (( _RET == 0 )); then
        ...
        confirm_ "also remove some labels?" "yes" "no" "no"
        _RET=$?
  ...
  for label in "${choose_labels[@]}"; do
    local label_found="$(gh api repos/$repo_name/labels --paginate --jq ".[] | select(.name==\"${label}\") | .name" 2>/dev/null)"

    # if [[ -z "$(gh label list --repo "$proj_repo" --search "$label" 2>/dev/null)" ]]; then

    if [[ -z "$label_found" ]]; then
      if (( proj_prs_l_is_u )); then
        print " ${red_cor}✗ label not found: ${blue_cor}$label${reset_cor}" >&2
        choose_labels=()
      else
        if gh label create --repo "$proj_repo" "$label" --yes &>/dev/null; then
          print " ${green_cor}✓ label created: ${blue_cor}$label${reset_cor}"
        else
          print " ${red_cor}✗ label failed: ${blue_cor}$label${reset_cor}" >&2
          choose_labels=()
        fi
      fi
    fi
  ...
  if [[ -z "${choose_labels[@]}" ]] && (( ! is_remove_all_labels && ! is_remove_version_labels )); then
    return 1;
  fi

  local prs_labels_ignore_original=()
  local prs_labels_ignore=()
  local to_be_removed_labels=()

  if [[ -n "${PUMP_PR_LABELS_IGNORE[$i]}" ]]; then
    prs_labels_ignore_original=("${(@s:,:)PUMP_PR_LABELS_IGNORE[$i]}")
    prs_labels_ignore=("${(@s:,:)PUMP_PR_LABELS_IGNORE[$i]}")
  fi

  local pr_list_version=""
  local is_version_processed=0

  if (( is_version_label )) && [[ -n "$release_version" ]] && (( proj_prs_l_is_ql )); then
    pr_list_version="$(list_prs_ -omc "" "label:$release_version" "$proj_repo")"
  fi

  local tickets_out="$(filter_jira_keys_by_release_ $i "$jira_proj" "$jira_release" "$search_key")"
  local tickets=("${(@f)tickets_out}")

  local ticket=""
  for ticket in "${tickets[@]}"; do
    local jira_key="$(echo $ticket | awk '{print $1}')"
    
    if [[ -z "$jira_key" ]]; then continue; fi

    local pr_list_bump=""
    local pr_list=""

    if [[ -n "$release_version" ]]; then
      # search for bump version PRs
      if (( proj_prs_l_is_qd )); then
        pr_list_bump="$(list_prs_ -omb "$release_version" "in:title,body" "$proj_repo" "prs for $release_version")"
      else
        pr_list_bump="$(list_prs_ -omb "$release_version" "in:title" "$proj_repo" "prs for $release_version")"
      fi
    fi

    if (( proj_prs_l_is_qd )); then
      pr_list="$(list_prs_ -odm "$jira_key" "in:title,body" "$proj_repo" "prs for $jira_key")"
    else
      pr_list="$(list_prs_ -odm "$jira_key" "in:title" "$proj_repo" "prs for $jira_key")"

      if [[ -z "$pr_list" ]]; then
        local jira_base_url="${PUMP_JIRA_BASE_URL[$i]}"
        local jira_link="$jira_key"

        if [[ -n "$jira_base_url" ]]; then
          jira_link="https://${jira_base_url}/browse/$jira_key"
          jira_link=$'\e]8;;'"$jira_link"$'\a'"$jira_key"$'\e]8;;\a'
        fi

        print " ${yellow_cor}✗ no pull request${reset_cor} for jira: ${blue_cor}$jira_link${reset_cor}" >&2
      fi
    fi

    if [[ -z "$pr_list" && -z "$pr_list_bump" ]]; then continue; fi

    if [[ -n "$pr_list" && -n "$pr_list_bump" ]]; then
      pr_list="$pr_list"$'\n'"$pr_list_bump"
    elif [[ -z "$pr_list" && -n "$pr_list_bump" ]]; then
      pr_list="$pr_list_bump"
    fi

    local pr_list_all=""
    
    if (( is_version_processed )); then
      pr_list_all="$pr_list"
    else
      pr_list_all="$pr_list_version"$'\n'"$pr_list"
      is_version_processed=1
    fi

    typeset -A labeled_prs

    local line=""
    for line in "${(@f)pr_list_all}"; do
      local pr_number="" pr_title="" pr_url="" pr_branch="" pr_state=""
      IFS=$TAB read -r pr_number pr_title pr_url pr_branch pr_state _ <<< "$line"

      if [[ -z "$pr_number" ]]; then continue; fi
      if (( labeled_prs[$pr_number] )); then continue; fi

      local pr_status="$([[ "$pr_state" == "OPEN" ]] && echo "opened" || echo "merged")"
      ...
      if (( is_remove_all_labels || is_remove_version_labels || is_remove_some_labels )); then
        label=""
        for label in "${existing_labels[@]}"; do
          if [[ -z "$label" ]]; then continue; fi

          if [[ " ${prs_labels_ignore[*]} " == *" $label "* ]]; then
            continue;
          fi
          ...
          local is_to_remove=0

          if [[ "$label" =~ ^[0-9]+\.[0-9]+\.[0-9]+$ ]]; then
            if (( is_remove_version_labels )); then
              is_to_remove=1
            fi
          else
            if (( is_remove_all_labels )); then
              is_to_remove=1
            elif (( is_remove_some_labels )); then
              if [[ " ${to_be_removed_labels[*]} " == *" $label "* ]]; then
                is_to_remove=1
              else
                confirm_ "pr ${pr_link}: remove ${label}?" "yes" "no" "no"
                _RET=$?
                if (( _RET == 130 )); then return 130; fi;
                if (( _RET == 0 )); then
                  is_to_remove=1
                else
                  confirm_ "ignore future removals of ${label}?"
                  _RET=$?
                  if (( _RET == 130 )); then return 130; fi;
                  if (( _RET == 0 )); then
                    prs_labels_ignore+=("$label")
                  fi
                fi
              fi
            fi
          ...
          if (( is_to_remove )); then
            if [[ " ${to_be_removed_labels[*]} " != *" $label "* ]]; then
              to_be_removed_labels+=("$label")
            fi

            if gh pr edit $pr_number --repo "$proj_repo" --remove-label "$label" &>/dev/null; then
              print " ${magenta_cor}✗ label remov${reset_cor} in $pr_status pr ${blue_cor}$pr_link${reset_cor}: ${pink_cor}$label${reset_cor}"

              existing_labels=("${(@)existing_labels:#$label}")

              local label_found="$(gh api repos/$repo_name/labels --paginate --jq ".[] | select(.name==\"${label}\") | .name" 2>/dev/null)"
              # local label_found=$(gh pr list --repo "$proj_repo" --label "$label" --limit 1 2>/dev/null)

              if [[ -z "$label_found" ]]; then
                if gh label delete --repo "$proj_repo" "$label" --yes &>/dev/null; then
                  print " ${magenta_cor}✗ label deleted${reset_cor} in all prs: ${pink_cor}$label${reset_cor}"
                fi
              fi
            fi
          fi
      ...
      for label in "${choose_labels[@]}"; do
        if (( proj_prs_l_is_u )); then
          if [[ " ${existing_labels[*]} " != *" $label "* ]]; then
            ...
            # do not remove if PR title is Revert, ask first
            if [[ "$label" =~ ^[0-9]+\.[0-9]+\.[0-9]+$ ]] && [[ "$pr_title" == Revert* ]]; then
              confirm_ -a "Revert PR $pr_link - confirm removing label: ${pink_cor}$label${reset_cor}" "yes" "no" "no"
              _RET=$?
              if (( _RET == 130 || _RET == 2 )); then continue; fi
            fi

            if (( _RET == 0 )); then
              if gh pr edit $pr_number --repo "$proj_repo" --remove-label "$label" &>/dev/null; then
                print " ${green_cor}✓ label remov${reset_cor} in $pr_status pr ${blue_cor}$pr_link${reset_cor}: ${pink_cor}$label${reset_cor}"
              else
                print " ${red_cor}✗ label error${reset_cor} in $pr_status pr ${blue_cor}$pr_link${reset_cor}: ${pink_cor}$label${reset_cor}" >&2
              fi
            fi
          fi
        else
          if [[ " ${existing_labels[*]} " == *" $label "* ]]; then
            print " ${green_cor}✓ label exist${reset_cor} in $pr_status pr ${blue_cor}$pr_link${reset_cor}: ${pink_cor}$label${reset_cor}"
          else
            local _RET=0

            # do not add if PR title is Revert, ask first
            if [[ "$label" =~ ^[0-9]+\.[0-9]+\.[0-9]+$ ]] && [[ "$pr_title" == Revert* ]]; then
              confirm_ -a "revert pr $pr_link - confirm adding label: ${pink_cor}$label${reset_cor}" "yes" "no" "no"
              _RET=$?
              if (( _RET == 130 || _RET == 2 )); then continue; fi
            fi
            ...
            if (( _RET == 0 )); then
              if gh pr edit $pr_number --repo "$proj_repo" --add-label "$label" &>/dev/null; then
                print " ${green_cor}✓ label added${reset_cor} in $pr_status pr ${blue_cor}$pr_link${reset_cor}: ${pink_cor}$label${reset_cor}"
              else
                print " ${red_cor}✗ label error${reset_cor} in $pr_status pr ${blue_cor}$pr_link${reset_cor}: ${pink_cor}$label${reset_cor}" >&2
              fi
            fi
      ...
      labeled_prs[$pr_number]=1
    done # for line in "${(@f)pr_list}"; do

    if (( proj_prs_l_is_debug )); then
      if ! confirm_ "processed $jira_key - continue?"; then
        return 1;
      fi
    fi
  done # for ticket in "${tickets[@]}"; do

  if [[ "$prs_labels_ignore" != "$prs_labels_ignore_original" ]]; then
    if confirm_ "save ignored removal of labels for future prs: ${prs_labels_ignore[*]}" "yes" "no" "no"; then
      local prs_labels_ignore_comma=""
      
      prs_labels_ignore_comma="$(printf ",%s" "${prs_labels_ignore[@]}")"
      prs_labels_ignore_comma="${prs_labels_ignore_comma:1}"
      
      update_config_ $i "PUMP_PR_LABELS_IGNORE" "$prs_labels_ignore_comma"
    fi
  fi
...
function proj_prs_r_() {
  ...
  local pr_list="$(gum spin --title="fetching open pull requests... $repo_name" -- gh pr list --repo "$proj_repo" \
    ...
    --author $current_user \
    --json number,title,isDraft,headRefName,baseRefName,url,commits \
    --jq '.[] | {number, title, isDraft, headRefName, baseRefName, url, commits: [.commits[].oid]} // empty'
  )"
  ...
  echo "$pr_list" | jq -c '.' | while read -r pr; do
    ...
    local pr_commits_out="$(jq -r '.commits' <<<"$pr")"
    ...
    local pr_commits=("${(@f)pr_commits_out}")

    if [[ -z "$pr_commits" ]]; then
      print " ${yellow_cor}skipped:${reset_cor} $pr_link_full" >&2
      continue;
    fi
    ...
    if (( ! proj_prs_r_is_f )) && (( pr_is_draft )); then
      confirm_ "pull request is on draft, confirm ${label_work}? ${pr_link_full}" "${label_work}" "skip"
      local RET=$?
      if (( RET == 130 )); then return 130; fi
      if (( RET == 1 )); then
        print " ${yellow_cor}skipped:${reset_cor} $pr_link_full" >&2
    ...
    if (( proj_prs_r_is_x )); then
      if (( is_merge_commit )); then
        if ! gum spin --title="merging... $pr_link_full" -- git -C "$temp_folder" merge "$remote_name/$pr_base_branch" --no-edit; then
          git -C "$temp_folder" merge --abort &>/dev/null
          print " ${red_cor}aborted:${reset_cor} $pr_link_full" >&2
          continue;
        fi
      else
        if ! gum spin --title="rebasing... $pr_link_full" -- git -C "$temp_folder" rebase "$remote_name/$pr_base_branch"; then
      ...
      if git -C "$temp_folder" push $remote_name $pr_branch --force --quiet &>/dev/null; then
        if (( is_merge_commit )); then
          print " ${cyan_cor} merged:${reset_cor} $pr_link_full" >&2
        else
          print " ${cyan_cor}rebased:${reset_cor} $pr_link_full" >&2
        fi
      else
        print " ${red_cor}aborted:${reset_cor} $pr_link_full" >&2
      fi
      ...
      if (( is_merge_commit )); then
        if gum spin --title="merging... $pr_link_full" -- zsh -c '
        ...
        if gum spin --title="rebasing... $pr_link_full" -- zsh -c '
          git -C "$1" rebase "$2/$3" &&
...
function approve_pr_() {
  ...
  if [[ "$pr_title" =~ (WIP|DRAFT|DO NOT MERGE) ]]; then
    print " ${hi_gray_cor}pr has 0 ✓ but is draft in progress: $pr_link_full${reset_cor}"
    return 0;
  fi

  if (( has_dnm_label )); then
    print " ${hi_gray_cor}pr has label do not merge, skipping: $pr_link_full${reset_cor}"
    return 0;
  fi
  ...
  if (( approval_count < pr_approval_min )) || (( approve_pr_is_x )); then
    if (( user_has_approved )); then
      print " ${green_cor}pr has $approval_count ✓ and you also approved it: $pr_link_full${reset_cor}"
    else
      local is_main=0;

      if [[ "$pr_author" != "$current_user" ]]; then
        if (( ! approve_pr_is_f )) && [[ "$pr_branch" != "main" ]]; then
          confirm_ "pr by $pr_author has $approval_count ✓: $pr_link_full" "approve" "skip"
          local RET=$?
          if (( RET == 130 )); then return 130; fi
          if (( RET == 0 )); then
            is_authorized=1;
      ...
      if (( is_authorized || approve_pr_is_f )) && [[ "$pr_author" != "$current_user" ]]; then
        if gh pr review $pr_number --approve --repo "$proj_repo" &>/dev/null; then
          (( approval_count++ ))
          if (( is_main )); then
            print " ${green_cor}pr has $approval_count ✓ and we auto approved it!: $pr_link_full${reset_cor}"
          else
            print " ${hi_green_cor}pr has $approval_count ✓ and you just approved it: $pr_link_full${reset_cor}"
          fi
        else
          print " ${red_cor}pr has $approval_count ✗ but failed to be approve: $pr_link_full${reset_cor}"
        fi
      elif [[ "$pr_author" != "$current_user" ]]; then
        print " ${red_cor}pr has $approval_count ✗ and you did not approve!: $pr_link_full${reset_cor}"
      else
        print " ${blue_cor}pr has $approval_count ✗ but you authored this pr: $pr_link_full${reset_cor}"
      fi
    ...
    if (( user_has_approved )); then
      ...
      if [[ "$pr_author" == "$current_user" ]]; then
        print " ${blue_cor}pr has $approval_count ✗ and you authored this pr: $pr_link_full${reset_cor}"
      else
        print " ${yellow_cor}pr has $approval_count ✗ but you did not approve!: $pr_link_full${reset_cor}"
      fi
    fi
  fi
...
# -b means including bump prs
# -o means only open prs
# -m means only merged prs
# -c means only closed prs
function list_prs_() {
  set +x
  eval "$(parse_flags_ "$0" "omc" "dbf" "$@")"
  (( list_prs_is_debug )) && set -x

  local search_text="$1"
  local search_where="$2"
  local proj_repo="$3"
  local label="$4"

  if (( ! list_prs_is_o && ! list_prs_is_m && ! list_prs_is_c )); then
    query_prs_ "$search_text" "$search_where" "$proj_repo" "$label" "${@:5}"
  else
    if (( list_prs_is_o )); then
      query_prs_ -o "$search_text" "$search_where" "$proj_repo" "$label" "${@:5}"
    fi

    if (( list_prs_is_m )); then
      query_prs_ -m "$search_text" "$search_where" "$proj_repo" "$label" "${@:5}"
    fi

    if (( list_prs_is_c )); then
      query_prs_ -c "$search_text" "$search_where" "$proj_repo" "$label" "${@:5}"
    fi
  fi
}
...
function query_prs_() {
  set +x
  eval "$(parse_flags_ "$0" "ocmdbf" "" "$@")"
  (( query_prs_is_debug )) && set -x

  local search_text="$1"
  local search_where="${2:-"in:title"}"
  local proj_repo="$3"
  local label="${4:-"pull requests"}"
  ...
  if (( query_prs_is_o )); then pr_state="open"; limit=""; fi
  if (( query_prs_is_m )); then pr_state="merged"; limit="100"; fi
  if (( query_prs_is_c )); then pr_state="closed"; limit="100"; fi

  if (( ! query_prs_is_f )) && (( query_prs_is_m || query_prs_is_c )) && [[ -z "$search_text" ]]; then
    print " ${yellow_cor}warning:${reset_cor} searching merged/closed prs without search text may return too many results, consider adding search text or use -o to search open prs only or use -f to ignore this warning" >&2
    return 1;
  fi
  ...
  if [[ -n "$search_text" ]]; then
    if (( query_prs_is_b )); then
      search_text="bump AND \"$search_text\" $search_where"
    else
      search_text="\"$search_text\" $search_where"
    fi
  fi

  if command -v gum &>/dev/null; then
    pr_list="$(gum spin --title="fetching ${pr_state} ${label}..." -- gh pr list --repo "$proj_repo" \
      --state="$pr_state" "${flags[@]}" \
      --search "$search_text" \
...
function proj_prs_s_() {
  ...
  local pr_list="$(gum spin --title="fetching prs... $repo_name" -- gh pr list \
    --repo "$proj_repo" \
    --limit 100 --state open \
    --json number,title,url,author,assignees \
    --jq '.[] | {number, title, url, author: .author.login, assignees} // empty'
  )"
  ...
  echo "$pr_list" | jq -c '.' | while read -r pr; do
    ...
    if [[ "$author" == app/* ]]; then
      print " pr is authored by ${author}: $pr_link_full"
      continue;
    fi

    if [[ -z "$assignees" ]]; then
      if gh pr edit $pr_number --add-assignee "$author" --repo "$proj_repo" &>/dev/null; then
        print " ${green_cor}pr is assigned to ${author}: $pr_link_full"
      else
        print " ${red_cor}pr could not be assigned to ${author}: $pr_link_full"
      fi
    else
      print " pr is assigned to ${assignees}: $pr_link_full"
    fi
  done
}
...
function select_pr_() {
  ...
  select_prs_ -un "$search_text" "$proj_repo" "$header" "${@:4}"
}

function select_prs_() {
  set +x
  eval "$(parse_flags_ "$0" "un" "ocmdf" "$@")"
  ...
  local arg_count=0
  if is_arg_ "$1"; then (( arg_count++ )); fi
  if is_arg_ "$2"; then (( arg_count++ )); fi
  if is_arg_ "$3"; then (( arg_count++ )); fi

  shift $arg_count

  local pr_list="$(list_prs_ "$search_text" "in:title" "$proj_repo" "$header" "${@:4}")"
...
function proj_bkp_() {
  ...
  if (( single_mode )); then
    ...
    local folder=""
    folder="$(choose_one_ -t "folder" "${(@f)dirs}")"
    if (( $? == 130 )); then return 130; fi
    if [[ -z "$folder" ]]; then return 1; fi
...
function proj_dbkp_() {
  ...
  eval "$(parse_args_ "$proj_cmd bkp" "folder:fo" "$@")"
  shift $arg_count
  ...
  local dirs_out="$(get_folders_ -p $i "$backups_folder" 2>/dev/null)"
  ...
  local dirs=""
  dirs="$(choose_multiple_ "folders" "${(@f)dirs_out}")"
  if (( $? == 130 )); then return 130; fi
  if [[ -z "$dirs" ]]; then return 1; fi
...
function convert_mode_() {
  ...
  if (( old_single_mode )); then
    ...
    for branch_folder in "${BRANCHES[@]}"; do
      ...
      if is_folder_git_ -ro "$folder" &>/dev/null; then
...
function proj_exec_() {
  ...
  eval "$(parse_args_ "$proj_cmd exec" "script:to,*" "$@")"
  shift $arg_count

  if ! check_proj_ -x $i; then return 1; fi
  ...
  if [[ -n "$script" ]]; then
    files=(
      "${proj_script_folder}/${script}".sh(N)
      "${proj_script_folder}/${script}".zsh(N)
      "${proj_script_folder}/${script}".bash(N)
      "${proj_script_folder}/${script}".ksh(N)
    )
  else
    files=(
  ...
  if [[ -z "$files" ]]; then
    if (( proj_exec_is_h )); then
      proj_print_help_ "$proj_cmd exec"

      print "  ---"
      print "  create shell scripts and placed them in: ${green_cor}$proj_script_folder${reset_cor}"

      return 0;
    fi

    if [[ -n "$script" ]]; then
      print " no shell scripts found: $script" >&2
    else
      print " no shell scripts found" >&2
    fi

    print " create it in: ${green_cor}$proj_script_folder${reset_cor}" >&2
    return 1;
  fi

  if (( proj_exec_is_h )) && [[ -z "$script" ]]; then
    proj_print_help_ "$proj_cmd exec"
  ...
  # print " ${script_cor}$script $i $@${reset_cor}"

  if (( proj_exec_is_debug )); then
    zsh -i $script $i "$@" --debug
  else
    zsh -i $script $i "$@"
  fi
}
...
function proj_version_() {
  ...
  if [[ -z "$web_url" ]]; then
    ...
    if (( RET == 0 )); then
      ...
      if (( $? == 130 )); then return 130; fi
      if [[ -n "$web_url" ]]; then
        update_config_ $i "PUMP_VERSION_WEB" "$web_url" &>/dev/null # must be silent here
      fi
    else
      update_config_ -d $i "PUMP_VERSION_WEB" "0"
    fi
  elif [[ "$web_url" == "0" ]]; then
    web_url=""
  else
  ...
  if [[ -z "$version_cmd" ]]; then
    ...
    if (( RET == 0 )); then
      ...
      if [[ -n "$web_url" ]]; then
        update_config_ -d $i "PUMP_VERSION_CMD" "0"
      else
        update_config_ $i "PUMP_VERSION_WEB" ""
        proj_version_ $i "$@"
        return $?;
      fi
    fi
  ...
  if (( proj_version_is_w )) || [[ -z "$version_cmd" ]]; then
    if [[ -n "$web_url" && -n "$meta_name" ]]; then
      curl -H 'Cache-Control: no-cache' -H 'Pragma: no-cache' -fsSL "${web_url}?_=$(date +%s)" | grep -o "<meta name=\"$meta_name\" content=\"[^\"]*\"" | sed -E 's/.*content="([^"]*)"/\1/'
      return $?;  
    fi
...
function proj_deploy_() {
  set +x
  eval "$(parse_flags_all_ "$0" "a" "$@")"
  (( proj_deploy_is_debug )) && set -x

  local i="$1"
  local proj_cmd="${PUMP_SHORT_NAME[$i]}"

  if (( proj_deploy_is_h )); then
    proj_print_help_ "$proj_cmd deploy"
    return 0;
  fi
  ...
  if ! check_proj_ -fe $i; then return 1; fi
  ...
  local env_list="${PUMP_ENV_LIST[$i]}"

  if [[ -n "$env_list" && "$env_list" != "0" ]]; then
    eval "$(parse_args_ "$proj_cmd deploy" "env:eo,branch:to" "$@")"
    shift $arg_count

    if [[ -z "$env" ]]; then
      local env_options=("${(@ps:$TAB:)env_list}")

      env="$(choose_one_ -i "environment" "${env_options[@]}")"
      if (( $? == 130 )); then return 130; fi
      if [[ -z "$env" ]]; then return 1; fi
    fi
  else
    eval "$(parse_args_ "$proj_cmd deploy" "branch:to" "$@")"
    shift $arg_count
  fi

  if [[ -n "$branch" ]]; then
    if ! is_branch_existing_remote_ "$branch" "$folder"; then
      print " fatal: branch not found: $branch" >&2
      return 1;
    fi
  else
    local proj_pwd="$(find_proj_by_folder_ "$PWD" 2>/dev/null)"

    if [[ "$proj_cmd" == "$proj_pwd" ]]; then
      local my_branch="$(get_my_branch_ "$PWD" 2>/dev/null)"
      branch="$(determine_target_branch_ -a "" "$my_branch" "$PWD")"
    else
      branch="$(determine_target_branch_ -a "" "" "$folder")"
    fi
  ...
  local interval="${PUMP_INTERVAL:-20}"

  while true; do
    if [[ -n "$env" ]]; then
      proj_exec_ $i deploy "$env" "$branch"
    else
      proj_exec_ $i deploy "$branch"
    fi
    RET=$?
    
    if (( RET != 0 || ! proj_gha_is_a )); then break; fi
...
function proj_dtag_() {
  ...
  if [[ -z "$tag" ]]; then
    local tags_out="$(git -C "$folder" tag)"
    local tags=("${(@f)tags_out}")
    ...
    local selected_tags_out=""
    selected_tags_out="$(choose_multiple_ "tags to delete" "${tags[@]}")"
    if (( $? == 130 )); then return 130; fi

    local selected_tags=("${(@f)selected_tags_out}")
...
function get_branch_status_() {
  ...
  if (( get_branch_status_is_a )); then # all files
    local output="$(git -C "$folder" diff --name-only --cached 2>/dev/null)"
    ...
    local output="$(git -C "$folder" diff --name-only 2>/dev/null)"
    if (( $? == 0 )); then files+=("${(@f)output}"); fi
    ...
    local output="$(git -C "$folder" status --porcelain 2>/dev/null | awk '{ print $2 }')"
    if (( $? == 0 )); then files+=("${(@f)output}"); fi
...
function proj_rel_() {
  ...
  if [[ "$proj_cmd" == "$CURRENT_PUMP_SHORT_NAME" ]] && is_folder_git_ &>/dev/null; then
    eval "$(parse_args_ "$proj_cmd rel" "branch:bz,tag:to,title:to" "$@")"
  else
    eval "$(parse_args_ "$proj_cmd rel" "branch:b,tag:to,title:to" "$@")"
  fi
  ...
  if ! clone_repo_ -bo "$proj_repo" "$folder"; then
    return 1;
  fi
  ...
  print " preparing $lbl_release of ${blue_cor}$proj_cmd${reset_cor} from ${hi_green_cor}$remote_branch${reset_cor} branch..."

  local proj_pwd="$(find_proj_by_folder_ "$PWD" 2>/dev/null)"

  if [[ "$proj_pwd" == "$proj_cmd" ]] && is_folder_git_ "$PWD" &>/dev/null; then
  ...
  if [[ -z "$tag" ]]; then
    git -C "$folder" switch "$branch" --quiet 1>/dev/null
  ...
  if (( ! proj_rel_is_f )) && ! confirm_ "${lbl_release} title: ${pink_cor}$title${reset_cor} with tag: ${pink_cor}$tag${reset_cor}?"; then
    return 0;
  fi
  ...
  if gh release view "$tag" --repo "$proj_repo" &>/dev/null; then
    if (( ! proj_rel_is_f )) && ! confirm_ "tag already exists, delete and $lbl_release ${pink_cor}$tag${reset_cor} again?"; then
      return 1;
    fi
...
function proj_rels_() {
  ...
  eval "$(parse_args_ "$proj_cmd rels" "search:to,limit:nz:100" "$@")"
  shift $arg_count
  ...
  if [[ -n "$search" ]]; then
    gh release list --repo "$proj_repo" --limit="9999" --order desc --json tagName,name,isPrerelease,createdAt,isDraft \
      --jq '.[] | "\(.name)'"$TAB"'\(.tagName)'"$TAB"'\(.isPrerelease | if . then "Pre-release" else "Latest" end)'"$TAB"'\(.createdAt)'"$TAB"'\(.isDraft)"' \
    | grep -i -- "$search" \
    | while IFS=$TAB read -r tag name type posted is_draft; do
    ...
    gh release list --repo "$proj_repo" --limit="$limit" --order desc --json tagName,name,isPrerelease,createdAt,isDraft \
      --jq '.[] | "\(.name)'"$TAB"'\(.tagName)'"$TAB"'\(.isPrerelease | if . then "Pre-release" else "Latest" end)'"$TAB"'\(.createdAt)'"$TAB"'\(.isDraft)"' \
    | while IFS=$TAB read -r tag name type posted is_draft; do
      posted="$(date -j -f "%Y-%m-%dT%H:%M:%SZ" "$posted" "+%m/%d/%Y %H:%M" 2>/dev/null || echo "$posted")"
...
function proj_drel_() {
  ...
  local tags_out=""
  if command -v gum &>/dev/null; then
    tags_out="$(gum spin --title="fetching tags..." -- gh release list --repo "$proj_repo" | awk '{print $1 "\t" $2}')"
  else
    print " fetching tags..."
    tags_out="$(gh release list --repo "$proj_repo" | awk '{print $1 "\t" $2}')"
  fi
  local tags=("${(@f)tags_out}")
  ...
  local selected_tags_out="$(choose_multiple_ "tags to delete" "${tags[@]}")"
  local selected_tags=("${(@f)selected_tags_out}")
  if [[ -z "$selected_tags" ]]; then return 1; fi
...
function proj_jira_find_branch_() {
  ...
  local branch_found=""
  branch_found="$(select_branch_ -jliq "$jira_key" "$prompt_label" "$proj_folder")"
  if (( $? == 130 )); then return 130; fi
...
function proj_jira_() {
  ...
  if (( proj_jira_is_a || proj_jira_is_c || proj_jira_is_d || proj_jira_is_e || proj_jira_is_r || proj_jira_is_t )) && (( ! proj_jira_is_s )); then
    print " fatal: -a, -c, -d, -e, -r, -t flags must be used with -s flag" >&2
    print " run: ${hi_yellow_cor}$proj_cmd jira -h${reset_cor} to see usage" >&2
    return 1;
  fi

  if (( proj_jira_is_s && ! proj_jira_is_ss && ! proj_jira_is_a && ! proj_jira_is_c && ! proj_jira_is_d && ! proj_jira_is_e && ! proj_jira_is_r && ! proj_jira_is_t )); then
    print " fatal: did you mean -ss flag?" >&2
    print " run: ${hi_yellow_cor}$proj_cmd jira -h${reset_cor} to see usage" >&2
    return 1;
  fi

  if (( proj_jira_is_ss )) && (( proj_jira_is_a || proj_jira_is_c || proj_jira_is_d || proj_jira_is_e || proj_jira_is_r || proj_jira_is_t )); then
  ...
  if [[ -n "$jira_status" ]] && (( ! proj_jira_is_ss && ! proj_jira_is_a && ! proj_jira_is_c && ! proj_jira_is_d && ! proj_jira_is_e && ! proj_jira_is_r && ! proj_jira_is_t )); then
  ...
  if (( proj_jira_is_s )); then
    ...
    if [[ -z "$jira_key" ]]; then
      local my_branch="$(get_my_branch_)"
      jira_key="$(extract_jira_key_ "$my_branch" "$PWD")"
  ...
  # opening work item and cloning it
  ...
  if (( single_mode )); then
    if ! is_folder_git_ "$proj_folder" &>/dev/null; then
      ...
      if (( proj_jira_is_f )); then
        if ! proj_clone_ -jf $i; then return 1; fi
      else
        if ! proj_clone_ -j $i; then return 1; fi
      fi

      proj_folder="${PUMP_FOLDER[$i]}"
    fi

    local branch_found="$(select_branch_ -jliq "$jira_key" "to open" "$proj_folder")"
    if (( $? == 130 )); then return 130; fi
    ...
    local flags=()
    if (( proj_jira_is_f )); then
      if ! proj_clone_ -jf $i "$jira_key" "" "$work_type"; then return 1; fi
    else
      if ! proj_clone_ -j $i "$jira_key" "" "$work_type"; then return 1; fi
    fi
  fi
...
function check_jira_() {
  ...
  if (( check_jira_is_i )); then
    ...
    if [[ -z "${PUMP_JIRA_BASE_URL[$i]}" ]]; then
      local jira_base_url="$(gum spin --title="preparing jira..." -- acli jira auth status 2>/dev/null | awk -F': ' '/Site:/ { print $2 }')"
      update_config_ $i "PUMP_JIRA_BASE_URL" "$jira_base_url"
    fi
  fi
  ...
  if (( check_jira_is_s )); then
    ...
    if [[ -z "$STATUS_COLOR_MAP" ]]; then
      local jira_statuses_out="$(check_jira_statuses_ $i)"
      local jira_statuses=("${(@f)jira_statuses_out}")
    ...
    # PUMP_JIRA_TODO
    if (( check_jira_is_d || check_jira_is_ss )); then
    ...
    # PUMP_JIRA_IN_PROGRESS
    if (( check_jira_is_o || check_jira_is_ss )); then
      local jira_in_progress="${PUMP_JIRA_IN_PROGRESS[$i]}"
    ...
    # PUMP_JIRA_IN_REVIEW
    if (( check_jira_is_r || check_jira_is_ss )); then
    ...
    # PUMP_JIRA_IN_TEST
    if (( check_jira_is_t || check_jira_is_ss )); then
    ...
    # PUMP_JIRA_ALMOST_DONE
    if (( check_jira_is_a || check_jira_is_ss )); then
    ...
    # PUMP_JIRA_DONE
    if (( check_jira_is_e || check_jira_is_ss )); then
    ...
    # PUMP_JIRA_CANCELED
    if (( check_jira_is_c || check_jira_is_ss )); then
    ...
    # PUMP_JIRA_BLOCKED
    if (( check_jira_is_b || check_jira_is_ss )); then
...
function check_jira_statuses_() {
  ...
  jira_statuses="$(gum spin --title="fetching configuration..." -- curl -s \
    -u "$current_jira_user_email:$jira_api_token" \
    -H "Accept: application/json" \
...
function get_label_for_status_() {
  set +x
  eval "$(parse_flags_ "$0" "adcsvrtebuf" "" "$@")"
  ...
  elif (( get_label_for_status_is_ss )); then
    if [[ -z "$jira_status" ]]; then
  ...
  elif (( get_label_for_status_is_tt )); then
  ...
  if (( get_label_for_status_is_u )); then
    echo "$jira_status"
  else
    echo "$label"
  fi
...
function proj_jira_release_() {
  ...
  if (( proj_jira_release_is_s && ! proj_jira_release_is_ss && ! proj_jira_release_is_t && ! proj_jira_release_is_r && ! proj_jira_release_is_a && ! proj_jira_release_is_c && ! proj_jira_release_is_d )); then
    print " fatal: did you mean -ss flag?" >&2
    print " run: ${hi_yellow_cor}$proj_cmd jira release -h${reset_cor} to see usage" >&2
    return 1;
  fi

  if (( proj_jira_release_is_ss )) && (( proj_jira_release_is_t || proj_jira_release_is_r || proj_jira_release_is_a || proj_jira_release_is_c || proj_jira_release_is_d )); then
    print " fatal: -ss flag cannot be used with -a, -c, -r, -t flags" >&2
    print " run: ${hi_yellow_cor}$proj_cmd jira release -h${reset_cor} to see usage" >&2
    return 1;
  fi

  local i="$(find_proj_index_ -x "$proj_cmd")"
  ...
  if (( proj_jira_release_is_vv )); then
    ...
    local tickets_out="$(filter_jira_keys_by_release_ $i "$jira_proj" "$jira_release" "$key" "$jira_status" "$@")"
    local tickets=("${(@f)tickets_out}")
...
function select_jira_key_by_release_() {
  ...
  local tickets_out="$(filter_jira_keys_by_release_ -n $i "$jira_proj" "$jira_release" "$search_key" "$@")"

  local ticket=""
  ticket="$(choose_one_ "work item $label" "${(@f)tickets_out}")"
  if (( $? == 130 )); then return 130; fi
...
function filter_jira_keys_by_release_() {
  ...
  local tickets="$(gum spin --title="fetching work items..." -- \
...
function update_jira_status_() {
  ...
  elif (( update_jira_status_is_tt )); then
  ...
  elif (( update_jira_status_is_tt )) && [[ "${jira_status:u}" != "${jira_in_test:u}" ]]; then
...
function select_jira_ticket_() {
  ...
  local title="fetching work items in open sprints"

  if (( select_jira_ticket_is_n )); then
    sprint_query="Sprint IS NOT EMPTY"
    title="fetching work items in any sprint"
  fi
  ...
  if [[ -z "$tickets" ]]; then
    tickets="$(gum spin --title="fetching work items in the backlog..." -- \
      acli jira workitem search \
...
function blami_() {
  set +x
  eval "$(parse_simple_flags_ "$0" "sc" "" "$@")"
  (( blami_is_debug )) && set -x

  local file="$1"

  local folder="$(find_git_folder_ "$file")"
  if [[ -z "$folder" ]]; then return 1; fi

  print " ${green_cor}== blame ==${reset_cor} ${yellow_cor}$file${reset_cor}"

  if (( blami_is_c )); then
    git -C "$folder" blame --line-porcelain "$file" | sed -n 's/^author //p' | sort | uniq -c | sort -nr
  elif (( blami_is_s )); then
    git -C "$folder" --no-pager log --use-mailmap --use-mailmap --no-decorate --follow --pretty=format:"%ad %an %h %s" --date=short -n 100 -- "$file" | awk '{print $2}' | sort | uniq -c | sort -nr
  else
    git -C "$folder" --no-pager log --use-mailmap --use-mailmap --no-decorate --follow --pretty=format:"%ad %an %h %s" --date=short -n 100 -- "$file"
  fi
}

function workflow_run_() {
  set +x
  eval "$(parse_flags_ "$0" "sbvtc" "" "$@")"
  ...
  local format="$3"
  local cor="$4"
  local run="$5" # optional

  if [[ -n "$run" ]] && is_flag_ "$run"; then
    return 1;
  fi

  if [[ -z "$cor" ]]; then
    cor="${GHA_COLOR_MAP[${run:u}]}"
  ...
  if (( workflow_run_is_s )); then
    result="$(gum spin --title="checking workflow status... $workflow" -- gh run list --repo "$proj_repo" \
      ...
      --jq "[.[] | select((.displayTitle | contains(\"$run\")) and (.conclusion == \"success\" or .conclusion == null))] | .[0] | \"\(.databaseId)${TAB}\(.url)${TAB}\(.displayTitle)${TAB}\(.headBranch)${TAB}\(.updatedAt)${TAB}\(.conclusion // empty)\""
    )"
  else
    result="$(gum spin --title="checking workflow status... $workflow" -- gh run list --repo "$proj_repo" \
      --workflow="$workflow" --limit 99 \
      --json databaseId,url,displayTitle,headBranch,updatedAt,conclusion \
      --jq "[.[] | select(.displayTitle | contains(\"$run\"))] | .[0] | \"\(.databaseId)${TAB}\(.url)${TAB}\(.displayTitle)${TAB}\(.headBranch)${TAB}\(.updatedAt)${TAB}\(.conclusion // empty)\""
    )"
  fi
  ...
  if [[ -z "$result" ]]; then
    ...
    if [[ -n "$run" ]]; then
      if (( workflow_run_is_s )); then
        result="$(gum spin --title="checking workflow status... $workflow" -- gh run list --repo "$proj_repo" \
          --workflow="$workflow" --limit 99 \
          --json databaseId,url,displayTitle,headBranch,updatedAt,conclusion \
          --jq "[.[] | select(.conclusion == \"success\" or .conclusion == null)] | .[0] | \"\(.databaseId)${TAB}\(.url)${TAB}\(.displayTitle)${TAB}\(.headBranch)${TAB}\(.updatedAt)${TAB}\(.conclusion // empty)\""
        )"
      else
        result="$(gum spin --title="checking workflow status... $workflow" -- gh run list --repo "$proj_repo" \
          --workflow="$workflow" --limit 99 \
    ...
    gum style --border=rounded --padding="0 1" --margin="0 1" --width="60" --align=left "$error" --border-foreground "$(ansi_to_gum_ ${hi_gray_cor})" >&2
  ...
  if [[ -n "$tag" ]]; then
    pkg_version="$(get_pkg_field_online_ "version" "$tag" "$proj_repo" 2>/dev/null)"
    tag_branch="$(gh api repos/$repo_name/releases/tags/${tag} 2>/dev/null | jq -r '.target_commitish // empty' 2>/dev/null)"
    commit_hash="$(gh api repos/$repo_name/git/ref/tags/${tag} 2>/dev/null | jq -r '.object.sha // empty' 2>/dev/null)"
  fi

  local formatted_updated_at=""
  if formatted_updated_at="$(date -d "$updated_at" "+%m/%d/%Y %H:%M" 2>/dev/null)"; then
    updated_at="$formatted_updated_at"
  elif epoch="$(TZ=UTC date -j -f "%Y-%m-%dT%H:%M:%SZ" "$updated_at" "+%s" 2>/dev/null)"; then
    updated_at="$(date -r "$epoch" "+%m/%d/%Y %H:%M")"
  fi
  ...
  box_content+="${gray_cor}$commit_hash${reset_cor}"

  if (( workflow_run_is_t || workflow_run_is_b || workflow_run_is_v || workflow_run_is_c )); then
    local values=()

    (( workflow_run_is_c )) && values+=("$commit_hash")
    (( workflow_run_is_t )) && values+=("$tag")
    (( workflow_run_is_b )) && values+=("$tag_branch")
    (( workflow_run_is_v )) && values+=("$pkg_version")
  ...
  gum style --border=rounded --padding="0 1" --margin="0 1" --width="60" --align=left "$box_content" --border-foreground "$(ansi_to_gum_ ${cor})" >&2
}

function workflow_runs_() {
  set +x
  eval "$(parse_flags_ "$0" "" "sbvtc" "$@")"
  (( workflow_runs_is_debug )) && set -x
  ...
  if [[ -z "$@" ]]; then
    ...
    # no run argument
    workflow_run_ "$proj_repo" "$workflow" "$format" "$cor" "" "${workflow_runs_is_flags[@]}"
    return $?;
  fi
  ...
  if [[ -z "$GHA_COLOR_MAP" ]]; then
    local run=""
    for run in "$@"; do
      if [[ -z "$run" ]] || is_flag_ "$run"; then continue; fi

      local cor=""

      if [[ "$run" == "prod" ]] || [[ "$run" == *:prod* ]]; then
        cor="${bold_pink_cor}"
      else
        cor="${colors[$(( (RANDOM % ${#colors[@]}) + 1 ))]}"
      fi
  ...
  for run in "$@"; do
    ...
    workflow_run_ "$proj_repo" "$workflow" "$format" "" "$run" "${workflow_runs_is_flags[@]}"
    _RET=$?
...
function proj_gha_() {
  set +x
  eval "$(parse_flags_ "$0" "adwf" "sbvtc" "$@")"
  ...
  if (( proj_gha_is_d && proj_gha_is_f )); then
    eval "$(parse_args_ "$proj_cmd gha" "format:m,search:a:to" "$@")"

  elif (( proj_gha_is_d )); then
    eval "$(parse_args_ "$proj_cmd gha" "search:a:to" "$@")"
  
  elif (( proj_gha_is_f )); then
    eval "$(parse_args_ "$proj_cmd gha" "workflow:to,format:m,search:a:to" "$@")"
  ...
  if [[ -z "$workflow" ]]; then
    ...
    if (( proj_gha_is_d )); then
      local is_save=1
      is_save=0
      confirm_ -a "save \"$workflow\" as the default deployment workflow for ${proj_cmd}?"
      local _RET=$?
  ...
  while true; do
    workflow_runs_ "$proj_repo" "$workflow" "$format" "${search[@]}" "$@"
    
    if (( $? != 0 )); then
      if (( proj_gha_is_d )) && [[ -n "$workflow" ]]; then
        PUMP_GHA_DEPLOY[$i]=""
        proj_gha_ -d "$proj_cmd" "$interval" "${search[@]}" "$@"
...
function abort() {
  ...
  local operation="$(get_current_git_operation_ "$folder" 2>/dev/null)"
...
function get_current_git_operation_() {
  ...
  local git_dir="$(git -C "$folder" rev-parse --absolute-git-dir 2>/dev/null)"
...
function chp() {
  ...
  if [[ -z "$hash" ]]; then
    ...
    local commits_out="$(git -C "$folder" --no-pager log --use-mailmap --no-merges --oneline -100)"
    local commits=("${(@f)commits_out}")
...
function revert() {
  ...
  if (( revert_is_h )); then
    ...
    print "  ${hi_yellow_cor}  -n${reset_cor} : --no-commit"
    print "  ${hi_yellow_cor}  -s${reset_cor} : --signoff"
    print "  ${hi_yellow_cor}  -q${reset_cor} : --quit"
    return 0;
  fi
  ...
  if (( revert_is_q )); then
    ...
    if [[ -n "$parent_number" ]]; then
      flags+=(-m $parent_number)
    fi

    if [[ -z "$hash" ]]; then
      local base_branch="$(get_base_branch_ -l "$branch" "$folder")"
      local comparison="-100"

      if [[ -n "$base_branch" ]]; then
        local short_base_branch="$(get_short_name_ "$base_branch" "$folder")"
        local my_branch="$(get_my_branch_ -e "$folder")"
        local my_short_branch="$(get_short_name_ "$my_branch" "$folder")"

        if [[ "$short_base_branch" != "$my_short_branch" ]]; then
          comparison="$base_branch..$my_branch"
        fi
      fi

      local commits_out="$(git -C "$folder" --no-pager log $comparison --use-mailmap --no-decorate)"
      local commits=("${(@f)commits_out}")
...
function conti() {
  ...
  if [[ "$operation" == "rebase" ]]; then
    HUSKY=0 GIT_EDITOR=true git -C "$folder" rebase --continue "$@"
  elif [[ "$operation" == "merge" ]]; then
    HUSKY=0 GIT_EDITOR=true git -C "$folder" merge --continue "$@"
  elif [[ "$operation" == "revert" ]]; then
    HUSKY=0 GIT_EDITOR=true git -C "$folder" revert --continue "$@"
  elif [[ "$operation" == "cherry-pick" ]]; then
    HUSKY=0 GIT_EDITOR=true git -C "$folder" cherry-pick --continue "$@"
  fi
...
function glog() {
  set +x
  eval "$(parse_flags_ "$0" "argmndtse" "0123456789" "$@")"
  (( glog_is_debug )) && set -x

  if (( glog_is_h )); then
    print "  ${hi_yellow_cor}glog ${yellow_cor}[<format>] [<folder>]${reset_cor} : see log commits using base branch as the starting point"
    print "  ${hi_yellow_cor}  -<number>${reset_cor} : limit log commits e.g.: glog -10"
    print "  ${hi_yellow_cor}  -a${reset_cor} : no base branch as starting point"
    print "  ${hi_yellow_cor}  -d${reset_cor} : --decorate"
    print "  ${hi_yellow_cor}  -e${reset_cor} : --reverse"
    print "  ${hi_yellow_cor}  -g${reset_cor} : --graph"
    print "  ${hi_yellow_cor}  -m${reset_cor} : --merges (show only merge commits)"
    print "  ${hi_yellow_cor}  -nm${reset_cor} : --no-merges (exclude merge commits)"
  ...
  eval "$(parse_args_ "$0" "format:to,folder:fz" "$@")"
  ...
  if (( ! glog_is_a )); then
    base_branch="$(get_base_branch_ -l "$branch" "$folder")"
  fi
  ...
  if [[ -n "$base_branch" ]]; then
    ...
    if [[ "$short_base_branch" == "$my_short_branch" ]]; then
      print " ${yellow_cor}warning: base branch is the same as current branch: $my_branch${reset_cor}" >&2
    else
      comparison="$base_branch..$my_branch"
    fi
    ...
    if [[ -n "$format" ]]; then
      if [[ -n "$search" ]]; then
        git -C "$folder" --no-pager log $comparison --use-mailmap --no-decorate --pretty="$format" "${flags[@]}" --grep="$search" "$@"
      else
        git -C "$folder" --no-pager log $comparison --use-mailmap --no-decorate --pretty="$format" "${flags[@]}" "$@"
      fi
    else
      if [[ -n "$search" ]]; then
        git -C "$folder" --no-pager log $comparison --use-mailmap --no-decorate "${flags[@]}" --grep="$search" "$@"
      else
        git -C "$folder" --no-pager log $comparison --use-mailmap --no-decorate "${flags[@]}" "$@"
      fi
    fi
  ...
  if [[ -n "$format" ]]; then
    if [[ -n "$search" ]]; then
      git -C "$folder" --no-pager log $base_branch --use-mailmap --no-decorate --pretty="$format" "${flags[@]}" --grep="$search" "$@"
    else
      git -C "$folder" --no-pager log $base_branch --use-mailmap --no-decorate --pretty="$format" "${flags[@]}" "$@"
    fi
  elif [[ -n "$search" ]]; then
    git -C "$folder" --no-pager log $base_branch --use-mailmap --no-decorate "${flags[@]}" --grep="$search" "$@"
  else
    git -C "$folder" --no-pager log $base_branch --use-mailmap --no-decorate "${flags[@]}" "$@"
  fi
}
...
function blame() {
  set +x
  eval "$(parse_simple_flags_ "$0" "sc" "" "$@")"
  (( blame_is_debug )) && set -x

  if (( blame_is_h )); then
    print "  ${hi_yellow_cor}blame ${yellow_cor}[<glob>]${reset_cor} : blame files/folders"
    print "  ${hi_yellow_cor}  -s${reset_cor} : show only the summary of the blame"
    print "  ${hi_yellow_cor}  -c${reset_cor} : show the count of commits per author (summary)"
    return 0;
  fi
  ...
  for file in "${files[@]}"; do
    if [[ -f "$file" || -d "$file" ]]; then
      ...
      # look up the files in current folder and subfolders
      local found_files=($(gum spin --title="finding..." -- find . -type f -name "$file"))

      if (( ${#found_files[@]} >= 1 )); then
        local files_chosen=""
        files_chosen="$(choose_multiple_ -i "files to delete" "${found_files[@]}")"
        if (( $? == 130 )); then return 130; fi
        ...
        print " file or folder not found: $file" >&2
        RET=1
      fi
    fi
  done
...
function pull() {
  ...
  if (( RET != 0 )); then
    ...
    if [[ -n "$files" ]]; then
      if (( ! is_quiet )) && [[ -n "$PUMP_MERGE_TOOL" ]] && command -v "$PUMP_MERGE_TOOL" &>/dev/null; then
        local cmd="$(basename "$PUMP_MERGE_TOOL")"

        git -C "$folder" mergetool --tool=$cmd "$files"
        RET=$?
      fi
    fi
  fi
...
function glr() {
  ...
  if ! is_folder_git_ "$folder" &>/dev/null; then
    ...
    folder="$(find_git_folder_ "$proj_folder")"
    if [[ -z "$folder" ]]; then return 1; fi
  fi
...
function co() {
  ...
  if [[ -n "$operation" ]]; then
    print " fatal: cannot switch branches while $operation is in progress" >&2
    return 1
  fi
  ...
  if (( co_is_p && co_is_r )); then
    ...
    local output=""
    output="$(select_pr_ -od "$branch" "$proj_repo" "pull request to detach")"
    if (( $? == 130 )); then return 130; fi
    if (( $? == 0 )); then
  ...
  if (( co_is_a )); then
    ...
    if [[ -n "$branch" ]]; then
      ...
      branch_choice="$(select_branch_ -acy "" "to switch" "$folder")"
    fi
    if (( $? == 130 )); then return 130; fi
  ...
  if (( co_is_l )); then
    ...
    if [[ -n "$branch" ]]; then
      ...
      branch_choice="$(select_branch_ -lcy "" "to switch" "$folder")"
    fi
    if (( $? == 130 )); then return 130; fi
    if [[ -z "$branch_choice" ]]; then return 1; fi
  ...
  if (( co_is_b || co_is_c )); then
    ...
    if (( co_is_b )); then
      base_branch="$(determine_target_branch_ -dbm "$branch" "" "$folder")"
      if (( $? == 130 )); then return 130; fi
  ...
  if (( co_is_x )); then
    ...
    if [[ -n "$base_branch" ]]; then
      if ! git -C "$folder" switch "$base_branch" --quiet &>/dev/null; then
        git -C "$folder" switch "$base_branch"
  ...
  if [[ -n "$branch" ]]; then
    ...
    branch_choice="$(select_branch_ -lcqy "" "to switch" "$folder")"
    if (( $?  == 130 )); then return 130; fi
...
function switch_branch_() {
  ...
  if [[ -z "$new_branch" ]]; then
    print " fatal: switch_branch_ missing branch argument" >&2
    return 1;
  fi

  if [[ -z "$folder" ]]; then
    print " fatal: switch_branch_ missing folder argument" >&2
    return 1;
  fi
  ...
  local jira_key="$(extract_jira_key_ "$new_branch" "$folder")"

  if git -C "$folder" switch $c_flag "$new_branch" "$@" &>/dev/null; then
    git -C "$folder" fetch --quiet &>/dev/null

    get_pump_jira_title_ "$jira_key" "$folder" &>/dev/null

    return 0;
  fi

  print " did not match a remote or local branch known to git: $new_branch" >&2
...
function rebase() {
  ...
  if (( rebase_is_xo )); then
    flags+=(--strategy-option=patience)
    flags+=(--strategy-option=ours)
  fi
  if (( rebase_is_xt )); then
    flags+=(--strategy-option=patience)
  ...
  if ! git -C "$folder" rebase $base_branch "${flags[@]}"; then
    ...
    if [[ -n "$files" && -n "$PUMP_MERGE_TOOL" ]] && command -v "$PUMP_MERGE_TOOL" &>/dev/null; then
      local cmd="$(basename "$PUMP_MERGE_TOOL")"
...
function prune() {
  ...
  local local_tags_out="$(git -C "$folder" tag)"
  local local_tags=("${(@f)local_tags_out}")

  local remote_tags_out="$(git -C "$folder" ls-remote --tags $remote_name)"
  local remote_tags=("${(@f)remote_tags_out}")
...
function pro() {
  set +x
  eval "$(parse_flags_ "$0" "aerulndisc" "fH" "$@")"
  (( pro_is_debug )) && set -x

  if (( pro_is_h )); then
    print "  ${hi_yellow_cor}pro <name>${reset_cor} : open project"
    ...
    print "  ${hi_yellow_cor}pro -i ${yellow_cor}[<name>]${reset_cor} : display config settings"
    print "  ${hi_yellow_cor}pro -c ${yellow_cor}[<name>]${reset_cor} : clean project, delete project folders"
    ...
    print "  ---"
    print "  ${hi_yellow_cor}pro -f ${reset_cor}: skip confirmation"
  ...
  if (( pro_is_d )); then
    local i="$(find_proj_index_ -x "$project")"
    [[ -n "$i" ]] || return 1;
  ...
  if (( pro_is_e )); then
    local i="$(find_proj_index_ -oe "$project" "project to edit")"
    (( i )) || return 1;

    project="${PUMP_SHORT_NAME[$i]}"

    save_proj_ -e $i "$project"
    return $?;
  fi
  ...
  if [[ "$project" == "$CURRENT_PUMP_SHORT_NAME" ]]; then
    if [[ "$OLDPWD/" != "$PWD/"* && "$PWD/" == "$CURRENT_PUMP_FOLDER/"* ]]; then
      local jira_key="$(read_pump_value_ "JIRA_KEY" "$proj_folder")"
      local jira_title="$(get_pump_jira_title_ "$jira_key" "$proj_folder")"
      ...
      if [[ -n "$jira_key" && -n "$jira_title" ]]; then
        ...
        print " ${pink_cor}${jira_link}${reset_cor} ${purple_cor}${jira_title}${reset_cor}" 1>/dev/tty
      fi

      if git rev-parse HEAD &>/dev/null; then
        print -n " " 1>/dev/tty
        git --no-pager log --use-mailmap --oneline --no-decorate -1 2>/dev/null 1>/dev/tty || true
      fi
    fi
...
function proj_handler() {
  ...
  local sub_cmds=("bkp" "clone" "del" "deploy" "gha" "jira" "prs" "pull" "rel" "rels" "rev" "revs" "run" "setup" "tag" "tags" "exec" "version")
  
  if (( single_mode )); then
    sub_cmds=("${(@)sub_cmds:#del}")
  fi

  if [[ " ${sub_cmds[*]} " != *" $1 "* ]]; then
    if (( proj_handler_is_h )); then
      if (( single_mode )); then
        print "  ${hi_yellow_cor}${proj_cmd}${reset_cor} : open project folder"
        print "  ${hi_yellow_cor}  -of${yellow_cor} [<folder>]${reset_cor} : open folder within the project"
      else
      ...
      if (( single_mode )); then
        print "  ${hi_yellow_cor}  -c${reset_cor} : clean project, delete old branches if jira cli is installed"
      else
        print "  ${hi_yellow_cor}  -c${reset_cor} : clean project, delete old folders if jira cli is installed"
      fi
      print "  ${hi_yellow_cor}  -m${reset_cor} : print main folder path"
      ...
      if (( single_mode )); then
        ...
        print "  ${hi_yellow_cor}${proj_cmd} bkp [<folder>]${reset_cor} : create backup of a project folder"
      fi
      ...
      if (( ! single_mode )); then
        print "  ${hi_yellow_cor}${proj_cmd} clone <branch> [<target>]${reset_cor} : clone branch off of target branch"
        print "  ${hi_yellow_cor}  -w <workflow_run>${reset_cor} : clone a workflow run branch"
      fi
      print "  ---"

      if (( ! single_mode )); then
        print "  ${hi_yellow_cor}${proj_cmd} del${yellow_cor} <glob>${reset_cor} : delete files/folders in project subfolders"
        print "  ${hi_yellow_cor}  -a${reset_cor} : include any folders in the project"
        print "  ${hi_yellow_cor}  -f${reset_cor} : skip confirmation"
        print "  ---"
      fi

      print "  ${hi_yellow_cor}${proj_cmd} deploy${yellow_cor} [<env>] [<branch>]${reset_cor} : deploy branch to environment ${env_list//$TAB/, }"
      ...
      print "  ${hi_yellow_cor}  -f${yellow_cor} <format>${reset_cor} : format using printf with display options:"
      print "  ${hi_yellow_cor}  -b${reset_cor} : display branch"
      print "  ${hi_yellow_cor}  -v${reset_cor} : display version"
      print "  ${hi_yellow_cor}  -t${reset_cor} : display tag"
      print "  ${hi_yellow_cor}  -c${reset_cor} : display commit hash"
      print "  ---"
      ...
      print "  ${hi_yellow_cor}${proj_cmd} prs${yellow_cor} [<search>]${reset_cor} : list all pull requests"
      ...
      print "    --"
      print "  ${hi_yellow_cor}  -u${yellow_cor} [<search>]${reset_cor} : unlabel prs based on unreleased jira releases"
      print "  ${hi_yellow_cor}  -ur${yellow_cor} [<search>]${reset_cor} : unlabel prs based on released jira releases"
      print "    --"
      print "  ${hi_yellow_cor}  -f${reset_cor} : skip confirmation"
      print "  ${hi_yellow_cor}  -qd${reset_cor} : include desc when searching for prs (default is to search title only)"
      print "  ${hi_yellow_cor}  -ql${reset_cor} : include labels when searching for prs (version label must be set)"
      print "  ---"
      if (( single_mode )); then
        print "  ${hi_yellow_cor}${proj_cmd} pull${reset_cor} : update folders with its configured upstream"
      else
        print "  ${hi_yellow_cor}${proj_cmd} pull${reset_cor} : update local branches with its configured upstream"
      fi
      print "  ${hi_yellow_cor}  -ff${reset_cor} : --force"
      print "  ${hi_yellow_cor}  -fo${reset_cor} : --ff-only"
      print "  ${hi_yellow_cor}  -p${reset_cor} : --prune"
      ...
      print "  ${hi_yellow_cor}${proj_cmd} rev${yellow_cor} [<pr_or_branch>]${reset_cor} : open review by pr or branch"
      print "  ${hi_yellow_cor}  -b${yellow_cor} [<branch>]${reset_cor} : open review by branch only"
      print "  ${hi_yellow_cor}  -x${yellow_cor} [<branch>]${reset_cor} : exact branch, no lookup"
      print "  ${hi_yellow_cor}  -j${yellow_cor} [<jira_key>]${reset_cor} : open review by work item"
      print "  ${hi_yellow_cor}  -r${yellow_cor} [<jira_key>]${reset_cor} : open review by jira release"
      print "  ${hi_yellow_cor}  -e${reset_cor} : check out local reviews"
      print "  ${hi_yellow_cor}  -d${reset_cor} : delete local reviews"
      print "  ---"

      print "  ${hi_yellow_cor}${proj_cmd} revs${reset_cor} : check out local reviews"
      print "  ${hi_yellow_cor}  -b${yellow_cor} [<branch>]${reset_cor} : check out local review by branch"
      print "  ${hi_yellow_cor}  -d${reset_cor} : delete all local reviews"
      print "  ---"

      if [[ -n "${PUMP_RUN_DEV[$i]}" ]]; then
        print "  ${hi_yellow_cor}${proj_cmd} run [<folder>]${reset_cor} : run PUMP_RUN_DEV script from ${proj_cmd}'s config file"
      else
        print "  ${hi_yellow_cor}${proj_cmd} run [<folder>]${reset_cor} : run dev or start script from ${proj_cmd}'s package.json"
      fi
      print "  ${hi_yellow_cor}${proj_cmd} run <script> [<folder>]${reset_cor} : run any script from ${proj_cmd}'s package.json"
      if [[ -n "${PUMP_RUN_STAGE[$i]}" ]]; then
        print "  ${hi_yellow_cor}${proj_cmd} run stage [<folder>]${reset_cor} : run PUMP_RUN_STAGE script from ${proj_cmd}'s config file"
      fi
      if [[ -n "${PUMP_RUN_QA[$i]}" ]]; then
        print "  ${hi_yellow_cor}${proj_cmd} run qa [<folder>]${reset_cor} : run PUMP_RUN_QA script from ${proj_cmd}'s config file"
      fi
      if [[ -n "${PUMP_RUN_PROD[$i]}" ]]; then
        print "  ${hi_yellow_cor}${proj_cmd} run prod [<folder>]${reset_cor} : run PUMP_RUN_PROD script from ${proj_cmd}'s config file"
      fi
      print "  ---"

      if [[ -n "${PUMP_SETUP[$i]}" && "${PUMP_SETUP[$i]}" != "0" ]]; then
        print "  ${hi_yellow_cor}${proj_cmd} setup [<folder>]${reset_cor} : run PUMP_SETUP script from ${proj_cmd}'s config file"
      else
        print "  ${hi_yellow_cor}${proj_cmd} setup [<folder>]${reset_cor} : run setup or install script from ${proj_cmd}'s package.json"
      fi
      print "  ---"
    ...
    if (( proj_handler_is_m )); then
      ...
      local folder="$(find_git_folder_ -m "$proj_folder")"
      if [[ -z "$folder" ]]; then return 1; fi
  ...
  if (( proj_handler_is_of && single_mode )); then
    local folder_arg="$1"

    local folder=""
    folder="$(proj_handler_folder_ -of $i "$folder_arg")"
    local RET=$?
  ...
  if (( proj_handler_is_o || proj_handler_is_f )); then
    ...
    print " ${red_cor}fatal: invalid option: $flag${reset_cor}" >&2
    print " --"
    $proj_cmd -h
  ...
  if (( single_mode )); then
    eval "$(parse_args_ "$proj_cmd" "" "$@")"
  else
    eval "$(parse_args_ "$proj_cmd" "folder:to" "$@")"
  fi

  shift $arg_count

  folder="$(proj_handler_folder_ $i "$folder")"
  local RET=$?
...
function commit() {
  ...
  if (( ! is_amend || commit_is_j )); then
    ...
    if (( commit_is_j )); then
      ...
      if [[ -z "$jira_title" && -z "$message" ]]; then
        jira_title="$(get_pump_jira_title_ -ns "$jira_key")"
      fi
    fi
  fi
  ...
  if [[ -z "$message" ]]; then
    ...
    if (( ! commit_is_f )); then
      if (( ! is_amend )) || [[ -n "$message" ]]; then
        ...
        message="$(input_type_mandatory_ -k "" "commit message" 255 "$message")"
        if (( $? == 130 )); then return 130; fi
      fi
    fi
...
function help() {
  ...
  print ""
  print "  ${gray_cor}for more customization, edit file:${reset_cor}"
  print "  ${gray_cor}$PUMP_CONFIG_FILE${reset_cor}"
  print ""
  print "  ${gray_cor}type ${hi_yellow_cor}-h ${gray_cor}after any command to see help, example: ${hi_yellow_cor}pro -h${reset_cor}"
  print "  ${gray_cor}for more help, visit: https://github.com/fab1o/pump-zsh/wiki${reset_cor}"
  print ""
}
...
function nlist() {
  ...
  if ! command -v npm &>/dev/null; then
    print " fatal: npm is not installed" >&2
    return 1;
  fi
...
typeset -gA PUMP_PR_LABELS_IGNORE
typeset -gA PUMP_GO_BACK
typeset -gA PUMP_ENV_LIST
...
typeset -g CURRENT_PUMP_RUN_DEV=""
...
typeset -g CURRENT_PUMP_PR_LABELS_IGNORE=""
typeset -g CURRENT_PUMP_GO_BACK=""
typeset -g CURRENT_PUMP_ENV_LIST=""
