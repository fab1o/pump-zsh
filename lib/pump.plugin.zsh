    ...
    for line in "${(@f)pr_list}"; do
      ...
      if (( is_remove_all_labels || is_remove_version_labels )); then
        ...
        for label in "${existing_labels[@]}"; do
          ...
          if (( is_removed )); then
            local prs=$(gh pr list --repo "$proj_repo" --label "$label" 2>/dev/null)

            if [[ -z "$prs" ]]; then
              if gh label delete --repo "$proj_repo" "$label" &>/dev/null; then
                print " ${magenta_cor}✓ label delet${reset_cor} in all prs: ${pink_cor}${label}${reset_cor}"
              fi
            else
              print " ${magenta_cor}✓ label remov${reset_cor} in pr ${blue_cor}$pr_link${reset_cor}: ${pink_cor}${label}${reset_cor}"
            fi
          fi
        done
      fi
      ...
      for label in "${choose_labels[@]}"; do
        if [[ " ${existing_labels[*]} " == *" $label "* ]]; then
          print " ${green_cor}✓ label exist${reset_cor} in pr ${blue_cor}$pr_link${reset_cor}: ${pink_cor}$label${reset_cor}"
        else
          if gh pr edit "$pr_number" --repo "$proj_repo" --add-label "$label" &>/dev/null; then
            print " ${green_cor}✓ label added${reset_cor} in pr ${blue_cor}$pr_link${reset_cor}: ${pink_cor}$label${reset_cor}"
          else
            print " ${red_cor}✗ label error${reset_cor} in pr ${blue_cor}$pr_link${reset_cor}: ${pink_cor}$label${reset_cor}" >&2
          fi
        fi
      done
