...
function filter_one_() {
  ...
  local cor=""
  if (( filter_one_is_a )); then cor="${bold_red_cor}"; fi
...
function choose_one_() {
  ...
  local cor=""
  if (( choose_one_is_a )); then cor="${bold_red_cor}"; fi
  ...
  PS3="${purple_cor}${cor}choose $header: ${reset_cor}"
...
function choose_multiple_() {
  ...
  local cor=""
  if (( choose_multiple_is_a )); then cor="${bold_red_cor}"; fi
  ...
  local choices=()
  PS3="${purple_cor}${cor}choose multiple $header, then choose \"done\" to finish ${choices[*]}${reset_cor}"
...
function select_branch_() {
  set +x
  eval "$(parse_flags_ "$0" "" "alrixscjoqe" "$@")"
  (( select_branch_is_debug )) && set -x

  local search_arg="$1"

  local filtered_branches="$(list_branches_ "$@")"
...
function delplus() {
  ...
  (( delplus_is_debug )) && set -x

  if (( delplus_is_h )); then
    print "  ${hi_yellow_cor}delplus <folder> <glob>${reset_cor} : delete all files/folders given by <glob> in folder <folder>"
  ...
  if [[ -z "$folder" ]]; then
    ...
    print " run: ${hi_yellow_cor}delplus -h${reset_cor} to see usage" >&2
    return 1;
  fi

  if [[ ! -d "$folder" ]]; then
    print " fatal: folder is not a valid directory: $folder" >&2
    print " run: ${hi_yellow_cor}delplus -h${reset_cor} to see usage" >&2
    return 1;
  fi
  ...
  local folders=($(find "$folder" -maxdepth 2 -type d -not -path '*/.*' 2>/dev/null))
...
function pr() {
  ...
  if (( pr_is_h )); then
    ...
    print "  --"
    print "  ${hi_yellow_cor}pr -l ${yellow_cor}[<title>] [<folder>] [<label...>]${reset_cor} : create pull request with labels"
    return 0;
  fi
...
function run() {
  ...
  if (( run_is_h )); then
    ...
    print "  ${hi_yellow_cor}setup <pro> [<folder>]${reset_cor} : run a project's PUMP_RUN in a folder"
    print "  --"
...
function proj_run_() {
  ...
  if [[ -n "$3" && "$3" != -* ]]; then
    eval "$(parse_args_ "run" "proj:po,script:to,folder:to" "$@")"
  else
    if [[ "$2" == "prod" || "$2" == "stage" || "$2" == "qa" ]]; then
      eval "$(parse_args_ "run" "proj:po,script:to" "$@")"
    else
      eval "$(parse_args_ "run" "proj:po,folder:to" "$@")"
    fi
  fi
  shift $arg_count
...
function setup() {
  ...
  if (( setup_is_h )); then
    ...
    print "  ${hi_yellow_cor}setup <pro> [<folder>]${reset_cor} : run a project's PUMP_SETUP in a folder"
    if [[ -n "$CURRENT_PUMP_SHORT_NAME" ]]; then
      if [[ -n "$CURRENT_PUMP_SETUP" ]]; then
    ...
    print "  --"
    print "  configure PUMP_SETUP in your ${hi_gray_cor}$PUMP_CONFIG_FILE${reset_cor} file"
    return 0;
  fi
...
function clone_repo_() {
  ...
  local header="cloning... ${blue_cor}$repo_name${reset_cor}"

  if [[ -n "$branch" ]]; then
    header="cloning... ${green_cor}$branch${reset_cor} on ${blue_cor}$repo_name${reset_cor}"
  fi
...
function proj_rev_() {
  ...
  if is_folder_git_ "$full_rev_folder" &>/dev/null; then
    ...
    if ! clone_repo_ -bo "$proj_repo" "$full_rev_folder" "$branch_user"; then
...
function proj_clone_() {
  ...
  if (( single_mode )); then
    ...
    if [[ -z "$git_folder" ]]; then
      ...
      if ! clone_repo_ -bo "$proj_repo" "$temp_folder" "preparing to clone..."; then
  ...
  if ! is_folder_git_ "$folder_to_clone" &>/dev/null; then
    if ! clone_repo_ -o "$proj_repo" "$folder_to_clone" "$branch"; then
      print " ${red_cor}fatal: failed to clone repository: $proj_repo${reset_cor}" >&2
      return 1;
    fi
  fi
  ...
  if [[ -n "$branch" && "$first_branch" != "$target_branch_user" && "$branch" != "$first_branch" && "$branch" != "main" && "$branch" != "master" ]]; then
    ...
    if [[ -z "$target_branch_user" ]]; then
      ...
      if [[ -z "$target_branch_user" ]]; then
        ...
        if (( $? == 130 )); then
          del -fx -- "$folder_to_clone" &>/dev/null
          return 130;
        fi
      fi
    fi
...
function proj_prs_l_() {
  ...
  if (( ! proj_prs_l_is_f )); then
    confirm_ "remove ALL other labels from prs?" "yes" "no" "no"
    local _RET=$?
    ...
    if (( _RET == 0 )); then
      ...
      confirm_ "remove version labels from prs?" "yes" "no" "no"
      _RET=$?
      ...
      if (( _RET == 0 )); then
        ...
        confirm_ "remove some labels from prs?" "yes" "no" "no"
        _RET=$?
  ...
  for ticket in "${tickets[@]}"; do
    ...
    local pr_list="" pr_list2=""

    pr_list="$(list_prs_ -odm "$key" "in:title,body" "$proj_repo" "prs for $key")"
    
    if [[ -n "$release_version" ]]; then
      # search for bump version PRs
      pr_list2="$(list_prs_ -omb "$release_version" "in:title,body" "$proj_repo" "prs for $release_version")"
    fi

    if [[ -z "$pr_list" && -z "$pr_list2" ]]; then
      continue;
    fi
    ...
    for line in "${(@f)pr_list}"; do
      ...
      if (( is_remove_all_labels || is_remove_version_labels || is_remove_some_labels )); then
        ...
        for label in "${existing_labels[@]}"; do
          ...
          if [[ " ${choose_labels[*]} " == *" $label "* ]] || [[ " ${prs_labels_ignore[*]} " == *" $label "* ]]; then
            continue;
          fi
          ...
          if (( is_removed )); then
            ...
            if [[ -z "$prs_with_label" ]]; then
              if gh label delete --repo "$proj_repo" "$label" &>/dev/null; then
      ...
      for label in "${choose_labels[@]}"; do
        if [[ " ${existing_labels[*]} " == *" $label "* ]]; then
          ...
          # do not add PR title is to Revert, ask first
          local _RET=0
...
function proj_prs_r_() {
  ...
  echo "$pr_list" | jq -c '.' | while read -r pr; do
    ...
    local pr_url="$(gh pr view "$pr_number" --repo "$proj_repo" --json url -q .url 2>/dev/null)"
    local pr_link=$'\e]8;;'"$pr_url"$'\a'"$pr_number"$'\e]8;;\a'
...
function proj_rel_() {
  ...
  if ! clone_repo_ -bo "$proj_repo" "$folder" "preparing $lbl_release of ${blue_cor}$proj_cmd${reset_cor} from ${yellow_cor}$remote_branch${reset_cor} branch..."; then
    local repo_name="$(get_repo_name_ "$proj_repo" 2>/dev/null)"
    print " fatal: failed to clone ${repo_name}" >&2
    return 1;
  fi
  ...
  if (( single_mode )); then
    if ! is_folder_git_ "$proj_folder" &>/dev/null; then
      ...
      if (( proj_jira_is_f )); then
        if ! proj_clone_ -jf "$proj_cmd"; then return 1; fi
      else
        if ! proj_clone_ -j "$proj_cmd"; then return 1; fi
      fi
    ...
    if [[ -n "$found_proj_folder" ]] && is_folder_git_ "$found_proj_folder" &>/dev/null; then
      ...
      local flags=()
      if (( proj_jira_is_f )); then
        if ! proj_clone_ -jf "$proj_cmd" "$jira_key" "" "$work_type"; then return 1; fi
      else
        if ! proj_clone_ -j "$proj_cmd" "$jira_key" "" "$work_type"; then return 1; fi
      fi
    fi
  fi
...
function get_label_for_status_() {
  set +x
  eval "$(parse_flags_ "$0" "adcsvrtebSf" "" "$@")"
...
function chp() {
  ...
  if [[ -z "$hash" ]]; then
    ...
    local commits="$(git -C "$folder" --no-pager log --use-mailmap --no-merges --oneline -100)"
...
function glog() {
  set +x
  eval "$(parse_flags_ "$0" "argmndtsR" "0123456789" "$@")"
  (( glog_is_debug )) && set -x
  ...
  for file in "${files[@]}"; do
    if [[ -f "$file" || -d "$file" ]]; then
      ...
      if (( ${#found_files[@]} >= 1 )); then
        local new_files="$(choose_multiple_ -i "files to delete" "${found_files[@]}")"
        if (( $? == 130 )); then return 130; fi
...
function workflow_run_() {
  ...
  if (( workflow_run_is_s )); then
    result="$(gum spin --title="checking workflow status... $workflow" -- gh run list --repo "$proj_repo" \
      ...
      --jq "[.[] | select((.displayTitle | contains(\"$arg\")) and (.conclusion == \"success\" or .conclusion == null))] | .[0] | \"\(.databaseId)${TAB}\(.url)${TAB}\(.displayTitle)${TAB}\(.headBranch)${TAB}\(.updatedAt)${TAB}\(.conclusion // empty)\""
    )"
  else
    result="$(gum spin --title="checking workflow status... $workflow" -- gh run list --repo "$proj_repo" \
      --workflow="$workflow" --limit 99 \
  ...
  if (( workflow_run_is_b && workflow_run_is_v )); then
    echo "$tag_branch|$pkg_version"
  ...
  if (( workflow_run_is_v )); then
    echo "$pkg_version"

    return 0;
  fi

  # if [[ -n "$arg" && -n "$pkg_version" ]]; then
  #   echo "- *$arg* is \`$pkg_version\`"
  # fi
...
function workflow_runs_() {
  ...
  if [[ -z "$@" ]]; then
    ...
    workflow_run_ "$proj_repo" "$workflow" "" "$cor" "${workflow_runs_is_flags[@]}"
    return $?;
  fi
  ...
  for arg in "$@"; do
    ...
    workflow_run_ "$proj_repo" "$workflow" "$arg" "${workflow_runs_is_flags[@]}"
    local _RET=$?
...
function co() {
  ...
  if (( co_is_b || co_is_c )); then
    ...
    if (( co_is_b )); then
      base_branch="$(determine_target_branch_ -dbe "$branch" "$folder")"
  ...
  if [[ -n "$branch" ]]; then
    ...
    branch_choice="$(select_branch_ -lcq "" "to switch" "$folder" "$my_branch" "$my_branch_short")"
    if (( $?  == 130 )); then return 130; fi
...
function pro_c_() {
  ...
  remove_proj_folders_ "$proj_cmd" "$proj_folder" "${@:2}"
  if (( $? == 130 )); then return 130; fi
...
function pro() {
  set +x
  eval "$(parse_flags_ "$0" "aerulndisc" "fxH" "$@")"
  (( pro_is_debug )) && set -x

  if (( pro_is_h )); then
    print "  ${hi_yellow_cor}pro -c <name>${reset_cor} : remove old project folders"
  ...
  if (( pro_is_r )); then
    ...
    if (( pro_is_l || pro_is_i || pro_is_u )) && (( ! pro_is_H )) && (( ! pro_is_f )); then
      max_arg=0
    elif (( pro_is_n )); then
      max_arg=2
...
function proj_handler() {
  ...
  if [[ " ${sub_cmds[*]} " != *" $1 "* ]]; then
    if (( proj_handler_is_h )); then
      ...
      (( ! single_mode )) && print "  ${hi_yellow_cor}  -w <workflow_run>${reset_cor} : clone a workflow run branch"
      print "  --"
      ...
      print "  ${hi_yellow_cor}  -v${yellow_cor} [<search...>]${reset_cor} : display version or tag"
      print "  --"
      ...
      print "  ${hi_yellow_cor}  -r${yellow_cor} [<jira_key>]${reset_cor} : open review by unreleased jira release"
  ...
  if (( proj_handler_is_o && proj_handler_is_f )); then
    ...
    local folder="$(proj_handler_folder_ -of $i "$@")"
    RET=$?
...
function proj_handler_folder_() {
  ...
  if (( single_mode )); then
    ...
    if [[ -z "$dirs" && -n "$folder_arg" ]]; then
      print " not a valid folder within $proj_cmd: ${hi_cyan_cor}$folder_arg${reset_cor}" >&2
...
function help_general_() {
  ...
  printf "  ${yellow_cor}%-$spaces${reset_cor} = %s \n" "del" "delete files"
  printf "  ${yellow_cor}%-$spaces${reset_cor} = %s \n" "delplus" "delete files in multiple folders"
