...
function proj_clone_() {
  ...
  if (( ! skip_clone )); then
    ...
    if command -v gum &>/dev/null; then
      if ! gum spin --title="running git clone..." -- git clone --filter=blob:none "$proj_repo" "$folder_to_clone"; then
        print " ${red_cor}fatal: failed to clone $proj_repo ${reset_cor}" >&2
        return 1;
      fi
    else
  ...
  if (( ! skip_clone )); then
    ...
    if [[ -z "$remote_branch_arg" ]]; then
      local remote_name=$(get_remote_origin_ "$folder_to_clone")
      print " branch created: ${yellow_cor}${branch_arg}${reset_cor} but not in $remote_name" >&2
    else
...
function proj_prs_() {
  ...
  if (( proj_prs_is_a_a )); then
    ...
    if (( RET == 0 )) && confirm_ -a "approve all prs every $PUMP_INTERVAL min?"; then
      while true; do
        ...
        proj_prs_aa_ -a "$proj_repo" "$pr_approval_min" "$search_term" "$current_user"
      done
    fi
  ...
  if (( proj_prs_is_a )); then
    ...
    for pr in "${prs[@]}"; do
      ...
      aprove_pr_ "$pr_number" "$pr_title" "$proj_repo" "$pr_approval_min" "$current_user" "$pr_approval_min"
    done
...
function proj_prs_aa_() {
  ...
  for pr in "${pr_list[@]}"; do
    ...
    aprove_pr_ "$pr_number" "$pr_title" "$proj_repo" "$pr_approval_min" "$current_user" "$proj_prs_aa_is_a"
  done
}
...
function aprove_pr_() {
  ...
  if (( approval_count < pr_approval_min )); then
    if (( user_has_approved )); then
      ...
      local is_approved=$is_auto
      if (( ! is_approved )) && [[ "$pr_author" != "$current_user" ]]; then
...
function co() {
  ...
  if (( co_is_p && co_is_r )); then
    ...
    local pr
    pr=(${(s:|:)$(select_pr_ "$branch_arg" "$repo" "pull request to detach")})
    if (( $? == 130 )); then return 130; fi
...
function proj_handler_open_() {
  ...
  if [[ -z "$(find "$folder_to_open" -maxdepth 1 -type f)" && -n "$(get_folders_ "$folder_to_open")" ]]; then
    ...
    local folder=($(choose_one_ -i "folder" "${dirs[@]}"))
