...
function update_proj_repo_() {
  ...
  if (( single_mode )); then
    ...
    local dirs_output="$(get_folders_ $i "$proj_folder" 2>/dev/null)"
...
function check_proj_() {
  set +x
  eval "$(parse_flags_ "$0" "rfmptjea" "qv" "$@")"
  ...
  if (( check_proj_is_a )); then
    if [[ -z "${PUMP_PR_APPROVAL_MIN[$i]}" ]]; then
      local pr_approval_min="$(input_number_ "minimum number of approvals for pull requests" 2 1)"
      if (( $? == 130 || $? == 2 )); then return 130; fi

      update_config_ $i "PUMP_PR_APPROVAL_MIN" $pr_approval_min
    fi

    if [[ -z "${PUMP_JIRA_ALMOST_DONE[$i]}" ]]; then
      local jira_almost_done="$(input_type_mandatory_ "jira status to mark issues as almost done" "Ready for Production" 50)"
      if (( $? == 130 || $? == 2 )); then return 130; fi
...
function get_proj_releases_() {
  set +x
  eval "$(parse_flags_ "$0" "" "" "$@")"
  (( get_proj_releases_is_debug )) && set -x
  ...
  if command -v gum &>/dev/null; then
    current_user="$(gum spin --title="retrieving ira..." -- acli jira auth status | awk -F': ' '/Email:/ { print $2 }' 2>/dev/null)"
    jira_base_url="$(gum spin --title="retrieving jira..." -- acli jira auth status | awk -F': ' '/Site:/ { print $2 }' 2>/dev/null)"

    jira_releases="$(gum spin --title="retrieving releases..." -- curl -s \
    ...
    current_user="$(acli jira auth status | awk -F': ' '/Email:/ { print $2 }' 2>/dev/null)"
    jira_base_url="$(acli jira auth status | awk -F': ' '/Site:/ { print $2 }' 2>/dev/null)"
    
    jira_releases="$(curl -s \
    -u "$current_user:$jira_api_token" \
    -H "Accept: application/json" \
    "https://${jira_base_url}/rest/api/3/project/${jira_proj}/versions" \
    | jq -r 'map(select(.released == false and .archived == false)) 
...
function select_proj_release_() {
  ...
  local jira_releases="$(get_proj_releases_ $i "$jira_proj" "$jira_api_token")"
...
function pr() {
  ...
  if [[ -n "$all_labels" ]]; then
    ...
    if (( ! pr_is_f )) && [[ -z "$choose_labels" ]]; then
      local choose_labels_out="$(choose_multiple_ "labels" "${all_labels[@]}")"
      if (( $? == 130 )); then return 130; fi
...
function proj_prs_() {
  ...
  if ! check_proj_ -frma $i; then return 1; fi
  ...
  if (( proj_prs_is_l_l )); then
    if ! check_proj_ -jt $i; then return 1; fi

    local jira_proj="${PUMP_JIRA_PROJECT[$i]}"
    local jira_api_token="${PUMP_JIRA_API_TOKEN[$i]}"

    local releases_out="$(get_proj_releases_ $i "$jira_proj" "$jira_api_token")"
...
function proj_prs_l_() {
  ...
  if ! check_proj_ -jrt $i; then return 1; fi
  ...
  if (( ! proj_prs_l_is_f )); then
    ...
    if (( _RET == 0 )); then
      ...
      local choose_labels_out="$(choose_one_ "labels" "${all_labels[@]}")"
      if (( $? == 130 )); then return 130; fi
  ...
  if [[ " ${choose_labels[*]} " == *" <new label...> "* ]]; then
    ...
    if [[ -z "$new_label" ]]; then return 1; fi
  ...
  local tickets="$(get_jira_keys_by_release_ -a $i "$jira_proj" "$jira_release" "$search_term" "${files[@]}")"
...
function proj_prs_aa_() {
  ...
  if command -v gum &>/dev/null; then
    pr_list="$(gum spin --title="fetching prs... $repo_name" -- gh pr list --repo "$proj_repo" --draft=false --json number,title,headRefName --jq ".[] | \"\(.number)${TAB}\(.title)${TAB}\(.headRefName)\"" | grep -i "$search_term"  2>/dev/null)"
    pr_list=("${(@f)pr_list}")
  else
    print " fetching prs... $repo_name"
    TAB=$'\t'
    pr_list="$(gh pr list --repo "$proj_repo" --draft=false --json number,title,headRefName --jq ".[] | \"\(.number)${TAB}\(.title)${TAB}\(.headRefName)\"" | grep -i "$search_term"  2>/dev/null)"
    pr_list=("${(@f)pr_list}")
  fi
...
function proj_jira_find_branch_() {
  ...
  local branch_found=""
  if (( proj_jira_is_c )); then
    branch_found="$(select_branch_ -jli "$jira_key" "$prompt_label" "$proj_folder")"
  else
...
function proj_jira_release_() {
  ...
  if ! check_proj_ -jmftr $i; then return 1; fi
  ...
  if (( proj_jira_release_is_v_v )); then
    ...
    local jira_release="$(select_proj_release_ $i "$jira_proj" "$jira_api_token")"
    if [[ -z "$jira_release" ]]; then return 1; fi

    print " ${purple_cor}release:${reset_cor} $jira_release"

    local tickets="$(get_jira_keys_by_release_ $i "$jira_proj" "$jira_release" "$search_term" "${files[@]}")"
    local tickets=("${(@f)tickets}")
    ...
    if ! check_proj_ -a $i; then return 1; fi
...
function select_jira_key_by_release_() {
  ...
  local tickets="$(get_jira_keys_by_release_ $i "$jira_proj" "$jira_release" "$search_term" $@)"
...
function get_jira_keys_by_release_() {
  ...
  if (( ! get_jira_keys_by_release_is_a )); then
...
function select_jira_key_() {
  ...
  local label=""
  local query_status="status!=\"$jira_done\" AND status!=\"Canceled\""
...
function get_maybe_jira_tickets_() {
  ...
  if (( single_mode )); then
    ...
    maybe_jira_keys=($(git -C "$folder" branch --list "*$search_jira_key*" --format="%(refname:short)" \
        | grep -v 'detached' \
        | grep -v 'HEAD' \
        | sort -fu
      ))
...
function pro() {
  ...
  if (( pro_is_c )); then
    ...
    if ! check_proj_ -afm $i; then return 1; fi
    ...
    if (( single_mode )); then
      print " removing old branches from project: ${blue_cor}$proj_arg${reset_cor}"
    else
      print " removing old folders from project: ${blue_cor}$proj_arg${reset_cor}"    
    fi
...
function pro_c_() {
  ...
  for maybe_jira_key in "${maybe_jira_keys[@]}"; do
    ...
    if [[ -n "$key" ]]; then
      ...
      if [[ "${jira_status:u}" == "${jira_done:u}" \
          || "${jira_status:u}" == "${jira_almost_done:u}" \
          || "${jira_status:u}" == "CANCELED" ]]; then
...
function proj_handler() {
  ...
  if [[ " ${sub_cmds[*]} " != *" $1 "* ]]; then
    if (( proj_handler_is_h )); then
      ...
      print "  ${hi_yellow_cor}  -c${yellow_cor} [<key>]${reset_cor} : close work item"
      print "  ${hi_yellow_cor}  -sr${yellow_cor} [<key>]${reset_cor} : move work item to \"In Review\""
      print "  ${hi_yellow_cor}  -st${yellow_cor} [<key>]${reset_cor} : move work item to \"In Test\""
      print "  ${hi_yellow_cor}  -ss${yellow_cor} [<key>] [<status>]${reset_cor} : move work item to a status"
      print "  ${hi_yellow_cor}  -v${yellow_cor} [<key>]${reset_cor} : view status of work item"
