...
function proj_clone_() {
  ...
  if [[ -n "$branch_arg" && "$branch_arg" != "$my_branch" ]]; then
    if git -C "$folder_to_clone" switch -c "$branch_arg" &>/dev/null || git -C "$folder_to_clone" switch "$branch_arg" &>/dev/null; then
  ...
  if (( ! skip_clone )); then
    if [[ -n "$branch_arg" && "$branch_arg" != "main" && "$branch_arg" != "master" ]]; then
...
function proj_prs_() {
  ...
  if (( proj_prs_is_a_a )); then
    ...
    if (( RET == 0 )) && confirm_ -a "approve prs every $PUMP_INTERVAL min?"; then
      while true; do
        print "sleeping for $PUMP_INTERVAL minutes..."
        sleep $(( 60 * PUMP_INTERVAL ))
...
function co() {
  ...
  if [[ -z "$base_branch" ]] && (( ! co_is_e )); then
    ...
    if [[ -z "$branch_choice" ]]; then
      if [[ -n "$short_branch_arg" ]]; then
        if co -e "$folder" "$short_branch_arg" ${@:2}; then
          return 0;
        fi
  ...
  if ! git -C "$folder" switch -c "$branch_arg" "$base_branch" ${@:3}; then
    ...
    if ! git -C "$folder" switch -c "$branch_arg" "$remote_name/$base_branch" ${@:3}; then
      return 1;
    fi
  fi
...
function rebase() {
  ...
  if (( ! rebase_is_p && rebase_is_a )); then
    git -C "$folder" rebase --abort $@ &>/dev/null
    return $?;
  fi

  if (( ! rebase_is_p && rebase_is_c )); then
    if ! git -C "$folder" add .; then return 1; fi
...
function merge() {
  ...
  if (( ! merge_is_p && merge_is_a )); then
    git -C "$folder" merge --abort $@ &>/dev/null
    return $?;
  fi

  if (( ! merge_is_p && merge_is_c )); then
    if ! git -C "$folder" add .; then return 1; fi
...
function proj_handler() {
  ...
  if [[ " ${sub_cmds[*]} " != *" $1 "* ]]; then
    if (( proj_handler_is_h )); then
      ...
      print "  ${hi_yellow_cor}${proj_cmd} prs -aa ${yellow_cor}[<search_term>]${reset_cor} : approve pull requests every $PUMP_INTERVAL min"
