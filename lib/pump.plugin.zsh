...
function get_proj_special_folder_() {
  ...
  local folder_name="$(basename -- "$proj_folder")"
  local folder_single_mode="${parent_folder}/.${folder_name}-$folder"
  local folder_multiple_mode="${proj_folder}/.$folder"

  local folder_single_mode_content="$(find "$folder_single_mode" -maxdepth 1 -type d -name 'rev.*' 2>/dev/null)"
  local folder_multiple_mode_content="$(find "$folder_multiple_mode" -maxdepth 1 -type d -name 'rev.*' 2>/dev/null)"

  if (( single_mode )); then
    if [[ -n "$folder_single_mode_content" ]]; then
      echo "$folder_single_mode"
      return 0;
    fi
  fi

  if [[ -n "$folder_multiple_mode_content" ]]; then
    echo "$folder_multiple_mode"
    return 0;
  fi
...
function save_proj_folder_() {
  ...
  if (( save_proj_folder_is_a || save_proj_folder_is_r )); then
    ...
    if (( save_proj_folder_is_a )); then
      if find_proj_by_folder_ $PWD; then
        # ask to use pwd
...
function save_proj_() {
  ...
  if [[ -n "$old_single_mode" ]] && (( old_single_mode != ${PUMP_SINGLE_MODE[$i]} )); then
    ...
    if [[ -n "$git_proj_folder" || -n "$pkg_proj_folder" ]]; then
      if create_backup_ -s $i "${PUMP_FOLDER[$i]}"; then
...
function remove_proj_() {
  ...
  if (( remove_proj_is_r )); then
    local revs_folder=$(get_proj_special_folder_ $i "revs")
...
function reset1() {
  ...
  git -C "$folder" log -1 --pretty=format:'%s'
...
function proj_jira_() {
  ...
  if (( proj_jira_is_c || proj_jira_is_v )); then
    ...
    if (( single_mode )); then
      ...
      local dirs=("${(@f)$(fl -qp "$proj_folder" "$jira_key")}")
      if [[ -z "$dirs" ]]; then
...
function proj_dbkp_() {
  ...
  local backups_folder="${parent_folder}/.backups"
  ...
  local dirs=("${(@f)$(fl -qps "$backups_folder" "$folder_name")}")
...
function proj_bkp_() {
  ...
  if (( single_mode )); then
    create_backup_ $i "$proj_folder" "$single_mode"
  else
      local dirs=("${(@f)$(fl -qps "$proj_folder")}")
      if [[ -z "$dirs" ]]; then
      ...
      create_backup_ $i "${proj_folder}/${folder}" "$single_mode"
  fi
}
...
function create_backup_() {
  ...
  if [[ ! -d "$folder" ]]; then
    print " fatal: not a valid folder argument for backup: $folder" >&2
    return 1;
  fi
  ...
  if (( ! single_mode )); then
    folder_name="$(basename -- "$parent_folder")-$folder_name"
  ...
  if command -v gum &>/dev/null; then
    ...
    if (( create_backup_is_s && create_backup_is_g )); then
      gum spin --title="cleaning .git folder in ${folder_name}..." -- \
        find "$folder" -type d -name ".git" -prune -exec rm -rf '{}' +
    
    elif (( create_backup_is_g )) && [[ -d "${folder}/.git" ]]; then
      gum spin --title="creating backup for .git..." -- \
        rsync -a --remove-source-files -- "${folder}/.git/" "${proj_backup_folder}/.git_backup/"
      rm -rf -- "${folder}/.git"
    fi

    if (( create_backup_is_s )); then
      gum spin --title="creating backup and deleting folder ${folder_name}..." -- \
        rsync -a --remove-source-files -- "${folder}/" "${proj_backup_folder}/"
    else
      gum spin --title="creating backup of ${folder_name}..." -- \
        rsync -a -- "${folder}/" "${proj_backup_folder}/"
    fi
    RET=$?
...
function proj_handler() {
  ...
  if [[ " ${sub_cmds[*]} " != *" $1 "* ]]; then
    if (( proj_handler_is_h )); then
      ...
      (( single_mode )) && print "  ${yellow_cor}${proj_cmd} -b <branch>${reset_cor} : open project and switch to branch"
  ...
  local dirs=("${(@f)$(fl -qp "$proj_folder" "${folder:-$1}")}")
  if [[ -n "$dirs" ]]; then
...
function fl() {
  set +x
  eval "$(parse_flags_ "$0" "qpoOnts" "" "$@")"
  (( fl_is_debug )) && set -x

  if (( fl_is_h )); then
    print "  ${yellow_cor}fl ${low_yellow_cor}[<folder>]${reset_cor} : list folders"
    print "  ${yellow_cor}fl -p${reset_cor} : order by priority: special folders first"
    print "  ${yellow_cor}fl -on${reset_cor} : order by name"
    print "  ${yellow_cor}fl -On${reset_cor} : order by name (reverse order)"
    print "  ${yellow_cor}fl -ot${reset_cor} : order by time"
    print "  ${yellow_cor}fl -Ot${reset_cor} : order by time (reverse order)"
    return 0;
  fi
