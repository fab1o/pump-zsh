...
function determine_base_branch_() {
  local folder="$1"
  ...
  local branches=("${(@f)$(printf "%s\n" "$default_branch" "$base_branch" "${my_branch:t}" | sort -u)}")
...
function get_my_branch_() {
  ...
  if [[ -z "$my_branch" ]] && (( get_my_branch_is_h )); then
    my_branch=$(git -C "$folder" rev-parse --abbrev-ref HEAD 2>/dev/null)
  fi

  if [[ -z "$my_branch" ]]; then
    print " fatal: current branch is detached or not tracking a remote branch" >&2
    return 1;
  fi
...
function get_repo_() {
  ...
  print " fatal: could not determine remote repository" >&2
  return 1;
}
...
function select_pr_() {
  ...
  local pr_list=$(gh pr list --repo "$repo" --json number,title,headRefName --jq '.[] | "\(.number)\t\(.title)\t\(.headRefName)"' | grep -i "$search_text" 2>/dev/null)
...
function refix() {
  ...
  if (( amend )); then
    ...
    if [[ -z "$last_commit_msg" ]]; then
      print " ${red_cor}fatal: last commit message is empty, cannot ammend${reset_cor}" >&2
      return 1;
    fi
...
function read_commits_() {
  ...
  local base_branch=$(get_base_branch_ -f "$folder")
  ...
  git -C "$folder" log --no-graph --oneline --no-merges --pretty=format:'%H%x1F%s%x00' \
    ...
    if [[ -n "$commit_jira_key" ]]; then
      ...
      if [[ -z "$pr_title_jira_key" ]]; then
        ...
        if [[ -z "$pr_title_rest" ]]; then pr_title_rest=$(echo "$rest" | xargs -0 2>/dev/null); fi
      fi
    fi
...
function pr() {
  ...
  local pr_field=(${(s:|:)$(read_commits_ -t "$my_branch")})
...
function proj_clone_() {
  ...
  if (( ! skip_clone )); then
    ...
    if [[ -z "$default_branch" ]]; then
      default_branch=$(determine_base_branch_ "$git_proj_folder" "$branch_arg")
      if (( $? == 130 )); then return 130; fi
...
function proj_prs_() {
  ...
  if ! command -v gh &>/dev/null; then
    print " fatal: command requires gh: $proj_cmd prs" >&2
    print " install gh: ${blue_cor}https://github.com/cli/cli/${reset_cor}" >&2
    return 1;
  fi
  ...
  if (( proj_prs_is_a && proj_prs_is_s && proj_prs_is_u )) || (( proj_prs_is_s && proj_prs_is_u )); then
  ...
  if (( proj_prs_is_s )); then
    ...
    if confirm_ "auto set assignee for all prs every $_interval min" "auto" "no"; then
      proj_prs_ "$proj_cmd" -sak
  ...
  if (( proj_prs_is_a && proj_prs_is_u )); then
    if [[ -z "$pr_approval_min" ]]; then
      pr_approval_min=$(input_number_ "minimum number of approvals to auto approve pull requests" "2" 1)
      if (( $? == 130 || $? == 2 )); then return 130; fi

      update_config_ $i "PUMP_PR_APPROVAL_MIN" $pr_approval_min
    fi

    proj_prs_au_ "$proj_repo" "$pr_approval_min"
    print ""

    if confirm_ "auto approve prs every $_interval min" "auto" "no"; then
      while true; do
        print "sleeping for $_interval minutes..."
        sleep $(( 60 * _interval ))
  ...
  if (( proj_prs_is_u )); then
    print "  ${red_cor}fatal: invalid option: -u${reset_cor}" >&2
    print "  --" >&1
    $proj_cmd -h | grep -w --color=never -E '\bprs\b'
    return 1;
  fi
  ...
  if (( proj_prs_is_a )); then
    local prs=("${(@f)$(select_prs_ "$2" "$proj_repo" "pull requests to approve")}")
    if (( $? == 130 )); then return 130; fi
...
function proj_prs_au_() {
  ...
  local pr_list=("${(@f)$(gh pr list --repo "$proj_repo" --json number,title --jq '.[] | "\(.number)\t\(.title)"' 2>/dev/null)}")
  if [[ -z "$pr_list" ]]; then
    print " fatal: failed to fetch pull requests for repo: $proj_repo" >&2
    return 1;
  fi
  ...
  for pr in "${pr_list[@]}"; do
    ...
    if [[ "$pr_title" =~ (WIP|Wip|wip|DRAFT|Draft|draft|DO NOT MERGE|Do Not Merge|do not merge) ]]; then
      print " ${hi_gray_cor}pr $pr_number has 0 ✓ and is drafted, skipping:${reset_cor} $pr_link"
      continue
    fi
    ...
    if [[ "$pr_labels" =~ (DO NOT MERGE|Do Not Merge|do not merge) ]]; then
      print " ${hi_gray_cor}pr $pr_number has label do not merge, skipping:${reset_cor} $pr_link"
      continue
    fi
    ...
    if (( approval_count < pr_approval_min )); then
      if (( user_has_approved )); then
        print " pr $pr_number has $pr_approval_min ✓ and you also approved it: $pr_link"
      else
        if gh pr review $pr_number --approve --repo "$proj_repo" &>/dev/null; then
          print " ${cyan_cor}pr $pr_number has $pr_approval_min ✓ and you just approved it:${reset_cor} $pr_link"
        fi
      fi
    else
      if (( user_has_approved )); then
        print " pr $pr_number has $approval_count ✓ and you also approved it: $pr_link"
      else
        print " pr $pr_number has $approval_count ✓, but you did not approve: $pr_link"
      fi
    fi
  done
}
...
function renb() {
  ...
  local gh_branch=$(git -C "$folder" config --get branch.$my_branch.gh-merge-base 2>/dev/null)
  ...
  if (( $? == 0 )); then
    ...
    git -C "$folder" config --remove-section branch.$my_branch &>/dev/null
...
function glog() {
  set +x
  eval "$(parse_flags_ "$0" "abcmrfg" "" "$@")"
  (( glog_is_debug )) && set -x

  if (( glog_is_h )); then
    print "  ${yellow_cor}glog ${low_yellow_cor}[<branch>] [<folder>]${reset_cor} : see last commits of a given branch"
    ...
    print "  ${yellow_cor}glog -g${reset_cor} : --graph"
    print "  ${yellow_cor}glog -r${reset_cor} : --remotes"
    print "  ${yellow_cor}glog -f <format>${reset_cor} : --pretty=format:'<format>' e.g.: glog -f %s"
    print "  ${yellow_cor}glog -<number>${reset_cor} : number of commits to show e.g.: glog -9"
    return 0;
  fi
  ...
  if (( glog_is_b || glog_is_c || glog_is_m )); then
    ...
    local my_branch=$(get_my_branch_ -h "$folder" 2>/dev/null)
    if [[ -z "$my_branch" ]]; then return 1; fi
  ...
  if [[ -n "$format" ]]; then
      git -C "$folder" --no-pager log $branch_arg..$my_branch --no-merges --oneline --decorate --pretty=format:"$format" ${flags[@]} $@
    else
      git -C "$folder" --no-pager log $branch_arg..$my_branch --no-merges --oneline --decorate ${flags[@]} $@
    fi
  ...
  if [[ -n "$format" ]]; then
    git -C "$folder" --no-pager log $branch_arg --oneline --decorate --pretty=format:"$format" ${flags[@]} $@
  else
    git -C "$folder" --no-pager log $branch_arg --oneline --decorate -10 ${flags[@]} $@
  fi
}
...
function push() {
  ...
  if [[ -z "$PUMP_PUSH_SET_UPSTREAM" ]]; then
    confirm_ "auto create an upstream counterpart branch on ${yellow_cor}push${reset_cor} and ${yellow_cor}pushf${reset_cor} ?"
    local RET=$?
    if (( RET == 130 || RET == 2 )); then return 130; fi
...
function pushf() {
  ...
  if [[ -z "$PUMP_PUSH_SET_UPSTREAM" ]]; then
    confirm_ "auto create an upstream counterpart branch on ${yellow_cor}pushf${reset_cor} and ${yellow_cor}push${reset_cor} ?"
    local RET=$?
    if (( RET == 130 || RET == 2 )); then return 130; fi
...
function glr() {
  ...
  if command -v gum &>/dev/null; then
    gum spin --title="loading..." -- git -C "$folder" branch -r --sort=authordate \
...
function rebase() {
  set +x
  eval "$(parse_flags_ "$0" "lacpqwd" "mipq" "$@")"
...
function proj_handler() {
  ...
  set +x
  eval "$(parse_flags_exclusive_ "$0" "mefinru" "cprvdsmnbjaeu" "$@")"
  ...
  if [[ " ${sub_cmds[*]} " != *" $1 "* ]]; then
    if (( proj_handler_is_h )); then
      ...
      print "  ${yellow_cor}${proj_cmd} gha -a ${low_yellow_cor}[<workflow>]${reset_cor} : auto check status of workflow every $_interval min"
      print "  --" >&1
      ...
      print "  --" >&1
      print "  ${yellow_cor}${proj_cmd} prs${reset_cor} : open pull requests in github"
      print "  ${yellow_cor}${proj_cmd} prs -a ${low_yellow_cor}[<branch_or_pr>]${reset_cor} : approve pull requests"
      print "  ${yellow_cor}${proj_cmd} prs -au${reset_cor} : auto approve pull requests"
      print "  ${yellow_cor}${proj_cmd} prs -s${reset_cor} : set assignee for all pull requests"
      print "  ${yellow_cor}${proj_cmd} prs -sa${reset_cor} : auto set assignee for all prs every $_interval min"
      print "  --" >&1
      ...
      print "  --" >&1
      print "  ${yellow_cor}${proj_cmd} rev ${low_yellow_cor}[<pr_or_branch>]${reset_cor} : open code review by pr or branch"
